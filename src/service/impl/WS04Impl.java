...
public class WS04Impl implements WS04 {
	...
	public String Query_list(String enterorg,String AuthCode) throws Exception  {
		...
		if(AuthCodeChk.equals(AuthCode)){
			try
			 {
				SqlStr=" select "+
				" a.enterOrg,a.unitNo,a.unitName"+ 
				" from UNTMP_KEEPUNIT a where 1=1";		
			  if(enterorg.length()>0){
				SqlStr=SqlStr+" and a.enterorg = '"+enterorg+"'";
			  }				  
			  SqlStr+=" order by a.unitname COLLATE Chinese_Taiwan_Stroke_CS_AS";
			  ...
			  if(rs.next()){
					...
					while(rs.next()){
						HashMap rows=new HashMap();
					 	...
					 	rows.put("unitNo",checkGet(rs.getString("unitNo")));//
					 	rows.put("unitName",checkGet(rs.getString("unitName")));//
