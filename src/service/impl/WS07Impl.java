...
public class WS07Impl implements WS07 {
	...
	public String Query_list(String enterorg,String AuthCode) throws Exception  {
		...
		if(AuthCodeChk.equals(AuthCode)){
			try
			 {
				SqlStr=" select "+
				" a.enterOrg,a.keeperno,a.keepername"+ 
				" from UNTMP_KEEPER a where 1=1";		
			  if(enterorg.length()>0){
				SqlStr=SqlStr+" and a.enterorg = '"+enterorg+"'";
			  }
			  SqlStr+=" order by a.keepername COLLATE Chinese_Taiwan_Stroke_CS_AS";	
			  ...
			  if(rs.next()){
					...
					while(rs.next()){
						HashMap rows=new HashMap();
					 	...
					 	rows.put("keeperno",checkGet(rs.getString("keeperno")));//
