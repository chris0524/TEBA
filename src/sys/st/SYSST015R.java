 ...
 * 非公用建物清理情形表
 * 
 * @version 1.0, Aug 4, 2006
 * @author  clive.chang
 * @since   TAJK Project
 */

public class SYSST015R extends SuperBean{
	...
	public void genExcelFile(){
		...
		try {
			...
			String sTitle = "高雄市政府財政局 " + Integer.parseInt(getQ_year())+" 年度非公用房屋建築清理情形表";
			...
			String sql = " select decode(a.useRelation,'01','01','52','01','53','01','81','01','03','02','83','02','50','50','99') as useState, count(*) as recCount, sum(a.useArea) as holdArea, " +  
				...
				" from QRY_UNT_Manage a where a.propertyType='2' and a.ownership='1' and a.dataState='1' and a.verify='Y' and a.propertyKind='04' and nvl(a.grass,'N')<>'Y' and nvl(a.taxCredit,'N')<>'Y' " + preSQL + 
			...
			//出租、占用、閒置及待處理建物 - 變動之增加項目
			sql = " select decode(a.useRelation,'01','01','52','01','53','01','81','01','03','02','83','02','50','50','99') as useState, count(*) as recCount, sum(a.useArea) as holdArea, " +  
				" sum(nvl(a.bookValue,0)) as currentValue " +
				" from QRY_UNT_Manage a where a.propertyType='2' and a.ownership='1' and a.dataState='1' and a.verify='Y' and a.propertyKind='04' " +
			...
			sTitle = "說明：1.資料填列基準日為"+Integer.parseInt(getQ_year())+"年12月31日止。表列各價值欄，按建築支出費用或取得之原價登記填列。";				
			...
			FileOutputStream fout = new FileOutputStream(tempDirectory.getAbsoluteFile()+File.separator+"npbBuilding.xls");			
			wb.write(fout);
			fout.flush();
			fout.close();
			this.setExcelFileName(tempDirectory.getAbsoluteFile()+File.separator+"npbBuilding.xls");
