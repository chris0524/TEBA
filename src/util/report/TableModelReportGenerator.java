...
public class TableModelReportGenerator extends ReportGenerator {
	...
	protected JasperPrint getJasperPrint(java.util.Map parms) throws Exception {
		...
		if (Common.getFileExtension(jasperFile).equals("jrxml")) return JasperFillManager.fillReport(compileAndFillReport(jasperFile),parms,ds);
		else return JasperFillManager.fillReport(jasperFile,parms,ds);
	}
	protected java.util.List getJasperPrintList(java.util.Map parms) throws Exception {
		...
		if (TableModelList == null ){
			...
			if (Common.getFileExtension(jasperFile).equals("jrxml")) {
				list.add(JasperFillManager.fillReport(compileAndFillReport(jasperFile),parms,ds));
			} else {
				list.add(JasperFillManager.fillReport(jasperFile,parms,ds));
			}
		}else{			
			for (int i = 0; i < jasperFileList.size(); i++) {
				...
				if (Common.getFileExtension(jasperFilePath).equals("jrxml")) {
