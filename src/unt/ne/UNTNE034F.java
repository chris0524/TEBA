...
public class UNTNE034F extends UNTNE001Q{
		...
		while(counter > 0){
		    ...
		    sbSQL.append( " insert into UNTNE_Attachment2(" ).append(
				...
				" )Values(" ).append(
					...
					Common.sqlChar(this.getSerialNoUntneAttachment2()) ).append( "," ).append(
					Common.sqlChar(obj.getSerialNo1()) ).append( "," ).append(
					Common.sqlChar(this.getEquipmentName()) ).append( "," ).append(
					Common.sqlChar(ul._transToCE_Year(this.getBuyDate())) ).append( "," ).append(
					Common.sqlChar(this.getEquipmentUnit()) ).append( "," ).append(
					Common.sqlChar(this.getEquipmentAmount()) ).append( "," ).append(
					Common.sqlChar(this.getUnitPrice()) ).append( "," ).append(
					Common.sqlChar(this.getTotalValue()) ).append( "," ).append(
					Common.sqlChar("1") ).append( "," ).append(
					Common.sqlChar(this.getNotes()) ).append( "," ).append(
					...
					Common.sqlChar(this.getSourceKind()) ).append( "," ).append(
					Common.sqlChar(this.getNameplate()) ).append( "," ).append(
					Common.sqlChar(this.getSpecification()) ).append( ")" ).append(
					":;:");		
...
protected String[] getInsertSQL(){
	...
	execSQLArray[0]=" insert into UNTNE_Attachment1(" +
		...
		" )Values(" +
			...
			Common.sqlChar(this.getEquipmentName()) + "," +
			Common.sqlChar(ul._transToCE_Year(this.getBuyDate())) + "," +
			Common.sqlChar(this.getEquipmentUnit()) + "," +
			Common.sqlChar(this.getEquipmentAmount()) + "," +
			Common.sqlChar(this.getUnitPrice()) + "," +
			Common.sqlChar(this.getTotalValue()) + "," +
...
protected String[] getUpdateSQL(){
	...
	execSQLArray[0]=" update UNTNE_Attachment1 set " +
			" equipmentName = " + Common.sqlChar(this.getEquipmentName()) + "," +
			" buyDate = " + Common.sqlChar(ul._transToCE_Year(this.getBuyDate())) + "," +
			" equipmentUnit = " + Common.sqlChar(this.getEquipmentUnit()) + "," +
			" equipmentAmount = " + Common.sqlChar(this.getEquipmentAmount()) + "," +
			" unitPrice = " + Common.sqlChar(this.getUnitPrice()) + "," +
			" totalValue = " + Common.sqlChar(this.getTotalValue()) + "," +
		...
		" where 1=1 " + 
			...
			" and lotNo = " + Common.sqlChar(this.getLotNo()) +
			" and serialNo1 = " + Common.sqlChar(this.getSerialNo1()) +
...
public UNTNE034F  queryOne() throws Exception{
	UNTCH_Tools ul = new UNTCH_Tools();
	Database db = new Database();
	UNTNE034F obj = this;
	try {
