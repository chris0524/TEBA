...
public class UNTNE003F_2 extends UNTNE001Q{
...
public void update() throws Exception{
	...
	try {			
		if (!beforeExecCheck(getUpdateCheckSQL())){
			...
			db.excuteSQL_NoChange(getUpdateSQL());		
...
protected String[] getUpdateSQL(){
	...
	execSQLArray[0]=" update UNTNE_NONEXPDETAIL set " +
			...
			" place = N" + Common.sqlChar(place) + "," +
			" notes1 = " + Common.sqlChar(notes1) + "," +
			" notes = N" + Common.sqlChar(notes) + "," +
			...
			" originalusernote = " + Common.sqlChar(originalUserNote)+"," +
			" originalplace1 = " + Common.sqlChar(originalPlace1)+"," +
			" place1 = " + Common.sqlChar(place1)+"," +
			" barcode = "+ Common.sqlChar(barCode) + "," +
			" propertyname1 = N"+ Common.sqlChar(propertyName1) +
		" where 1=1 " + 
	...
	StringBuilder nonexpUpdate = new StringBuilder();
	nonexpUpdate.append("update UNTNE_NONEXP set ")
	            .append("articlename = N").append(Common.sqlChar(this.articleName))
	            .append(",nameplate = N").append(Common.sqlChar(this.nameplate))
	            .append(",specification = N").append(Common.sqlChar(this.specification))
	            .append(",storeno = ").append(Common.sqlChar(this.storeNo));
	nonexpUpdate.append(" where 1=1")
				.append(" and lotno = ").append(Common.sqlChar(this.lotNo))
				.append(" and enterorg = ").append(Common.sqlChar(this.enterOrg))
				.append(" and ownership = ").append(Common.sqlChar(this.ownership))
				.append(" and propertyno = ").append(Common.sqlChar(this.propertyNo))
				.append(" and differencekind = ").append(Common.sqlChar(this.differenceKind));
	
	execSQLArray[1] = nonexpUpdate.toString();
	return execSQLArray;
}
...
public UNTNE003F_2  queryOne() throws Exception{
	...
	try {
		String sql=" select a.enterOrg, b.organSName as enterOrgName, a.ownership, a.propertyNo,a.differenceKind, " +
					...
					" a.editTime, d.caseNo, d.otherPropertyUnit, d.otherMaterial, ISNULL(a.otherLimitYear,0) as otherLimitYear,a.originallimityear, " +
					" a.originalAmount, a.originalBV "+
					" ,a.barcode, a.propertyname1, " +
					
					" d.propertykind, d.fundtype, d.valuable, d.articlename, d.specification,d.buydate," +"\n"+
					" d.nameplate, d.cause, d.cause1, d.approvedate," +"\n"+
					" d.approvedoc, d.fundssource, d.fundssource1, d.grantvalue, d.accountingtitle," +"\n"+
					" d.sourcekind, d.sourcedate, d.sourcedoc," +"\n"+
					" d.storeno, d.cause, d.sourcekind," +"\n"+
					" (select z.storename from UNTMP_STORE z where 1=1 and d.storeno=z.storeno and d.enterorg=z.enterorg) as storeName, " +"\n"+
					" (select z.codename from SYSCA_CODE z where z.codekindid = 'CAA' and z.codeid = d.cause and z.codecon1 in (1,4)) as causeName," +"\n"+
					" (select codename from SYSCA_CODE z where codekindid = 'SKD' and z.codeid = d.sourcekind) as sourceKindName" +"\n"+
					...
					" inner join UNTNE_Nonexp d on 1=1 " +
		...
		if (rs.next()){
			...
			obj.setOriginalPlace1Name(placeMap.get(rs.getString("originalPlace1")));
			obj.setPlace1Name(placeMap.get(rs.getString("Place1")));
...
public ArrayList queryAll() throws Exception{
	...
	try {
		String sql= " select a.enterOrg," +
					" (select b.organSName from SYSCA_Organ b where a.enterOrg = b.organID) as organSName," +
					" a.ownership, a.propertyNo,a.differencekind,a.caseNo, " +"\n"+
					...
					" from UNTNE_NonexpDetail a, UNTNE_Nonexp e " +"\n"+
					" where a.ownership=e.ownership and a.enterOrg=e.enterOrg and e.propertyNo=a.propertyNo and e.lotNo=a.lotNo and e.differencekind=a.differencekind " +"\n"+
					"";
...
//				sql+=" and d.proofyear = " + Common.sqlChar(getQ_proofYear()) ;
		...
		sql+=" and a.verify='Y' ";
		sql+=" order by  a.enterOrg, a.dataState, a.differencekind, a.propertyNo, a.serialNo ";
			...
			if(rowArray[12].equals("N")){
				...
				if(enterOrg.equals(checkEnterOrg)){
					sbSQL.append("update UNTNE_NonexpDetail set ").append(
                				" originalMoveDate = " ).append( Common.sqlChar(setMoveDate) ).append( "," ).append(
								...
								" moveDate = " ).append( Common.sqlChar(ul._transToCE_Year(setMoveDate)) ).append( "," ).append(
...
//		sbHTML.append("<td class='TDLable' align='center'>使用處別</td>\n");
		...
		while (rs.next()) {
			...
			sbHTML.append("<td class='queryTDInput'>\n" ).append(
			 "<select class='field' type='select' name='keepUnit_").append( rs.getString("serialNo")).append("' onChange='form1.useUnit_").append(rs.getString("serialNo")).append(".value = this.value").append(";'>\n" ).append(
			  ...
			  "</td>\n");
			sbHTML.append("<td class='queryTDInput'>\n" ).append(
			  "<select class='field' type='select' name='keeper_").append( rs.getString("serialNo")).append("' onChange='form1.user_").append(rs.getString("serialNo")).append(".value = this.value").append(";'>\n" ).append(
			  "<script>getKeeper(form1.enterOrg, form1.keeper_").append( rs.getString("serialNo")).append(", form1.keeper_").append( rs.getString("serialNo")).append(", '").append(rs.getString("originalKeeper")).append("');</script>\n").append(
			  "</select>").append(
			  "</td>\n");

			sbHTML.append("<td class='queryTDInput'>\n" ).append(
			  "<select class='field' type='select' name='useUnit_").append( rs.getString("serialNo")).append("' onChange='form1.keepUnit_").append(rs.getString("serialNo")).append(".value = this.value").append(";'>\n" ).append(
			  ...
			  "</td>\n");
			sbHTML.append("<td class='queryTDInput'>\n" ).append(
			  "<select class='field' type='select' name='user_").append( rs.getString("serialNo")).append("' >\n").append(
			  "<script>getKeeper(form1.enterOrg, form1.user_").append( rs.getString("serialNo")).append(", form1.user_").append( rs.getString("serialNo")).append(", '").append(rs.getString("originalUser")).append("');</script>\n").append(
			  "</select>").append(
			  "</td>\n");
			...
			sbHTML.append("<td class='queryTDInput'>").append(
			  "<input class='field_Q' type='text' name='place1_").append( rs.getString("serialNo")).append("' size='5' maxlength='5' value='").append(Common.get(rs.getString("place1"))).append("'>").append(
			  "&nbsp;<input class='field_Q' type='button' name='btn_q_place_").append(rs.getString("serialNo")).append("' onclick=\"popPlace('").append(rs.getString("enterorg")).append("','place1_").append(rs.getString("serialNo")).append("','placeName_").append(rs.getString("serialNo")).append("')\" value='...' title='存置地點輔助視窗'>").append(
			  "&nbsp;[<input class='field_RO' type='text' name='placeName_").append(rs.getString("serialNo")).append("' size='20' maxlength='20' value=''>] ").append(
			  "</td>\n");
