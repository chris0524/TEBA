...
public class UNTNE003F_1 extends UNTNE001Q{
...
public UNTNE003F_1  queryOne() throws Exception{
	...
	try {
		...
		if (rs.next()){
			...
			obj.setPlaceName(placeMap.get(rs.getString("Place1")));
		...
		if("".equals(this.getObjPath())){
			this.setObjPath("功能選單 > > 單位財產系統 > > 物品查詢及維護 > >物品查詢" + this.getPropertyNo() +"-" + this.getSerialNo());
		}
		Common.insertRecordSql("" , this.getOrganID(), this.getUserID(), "UNTNE003F_1", this.getObjPath().replaceAll("&gt;", ">"),"query");
...
//				sql+=" and d.proofyear = " + Common.sqlChar(getQ_proofYear()) ;
			...
			if (!"".equals(this.getQ_notes())) {
				sql += " and replace(a.Notes, '-', space(0)) like '%" + getQ_notes().replace("-", "") +"%'";
			}
