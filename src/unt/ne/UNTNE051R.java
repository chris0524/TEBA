...
public class UNTNE051R extends SuperBean{
	String q_labelType;
	String q_enterOrg;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
        ...
        while(rs.next()){
        	...
        	data[3] = "編號：" + Common.get(rs.getString("propertyNo")) + "-" + Common.get(rs.getString("serialNo"));//propertyNo
        	data[4] = "年限：" + Common.get(rs.getString("limitYear"));//limitYear
        	data[5] = "名稱：" + Common.get(rs.getString("propertyname"));//propertyname
        	data[6] = "購買日期：" + Common.MaskDate(rs.getString("buyDate"));//buyDate
        	data[7] = "規格：" + Common.get(rs.getString("specification"));//specification
        	data[8] = "單位：" + Common.get(rs.getString("keepUnitName"));//keepUnitName
        	data[9] = "地點：" + Common.get(rs.getString("place"));//place
        	if("Y".equals(getQ_printKeeper())){
        		data[10] = "保管人：" + Common.get(rs.getString("keeperName"));//keeperName
        	}else{
        		data[10] = "";
        	}
        	data[11] = Common.get(rs.getString("barCode"));//barcode
