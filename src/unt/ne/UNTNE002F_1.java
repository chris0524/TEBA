...
public class UNTNE002F_1 extends UNTNE001Q{
...
public UNTNE002F_1  queryOne() throws Exception{
	UNTCH_Tools ul = new UNTCH_Tools();
	Database db = new Database();
	UNTNE002F_1 obj = this;
	try {
		String sql=" select distinct a.enterOrg, b.organSName as enterOrgName, a.ownership, a.caseNo,a.differenceKind,a.caseSerialNo, " +"\n"+
					" a.propertyNo, a.lotNo, a.serialNoS, a.serialNoE,a.otherPropertyUnit, a.otherMaterial, ISNULL(a.otherLimitYear,d.originallimitYear) as otherLimitYear, " +"\n"+
					" c.propertyName, c.propertyType, c.propertyUnit, c.material, isnull(c.limitYear,d.originallimitYear) as limitYear, " +"\n"+
...
//System.out.println("-- untne002f queryOne --\n"+sql);
		...
		if (rs.next()){
			...
			obj.setOtherLimitYear((Integer.parseInt(rs.getString("otherLimitYear"))==0)?"":Integer.toString(Integer.parseInt(rs.getString("otherLimitYear"))/12)); //調整後年限(月)
...
//		}
					...
					if(!"".equals(getQ_place1()))
						sql+=" and f.place1 = " + Common.sqlChar(getQ_place1());
