...
public class UNTPD006F extends SuperBean2{
...
public ArrayList queryAll() throws Exception{
	...
	try {
		...
		sql += " select "+
			   "a.enterorg, a.ownership, a.differencekind, a.differencekindname, a.propertyno, a.serialno,  (select b.propertyname from SYSPK_PROPERTYCODE b where (b.enterorg=a.enterorg or b.enterorg='000000000A') and b.propertyno=a.propertyno ) as propertyname, a.propertyname1," +
			   "a.bookamount, a.actualamount, a.bookvalue, a.keepunitname, a.keepername, a.checkresult,"+ 
			   "a.scrappednote, a.labelnote, a.movenote "+  
			   "from UNTPD_CHECKMOVABLE a where 1=1";
		
				if("init".equals(this.getState())){
					if(!"".equals(getQ_enterOrg2())){
						 sql += " and a.enterorg = " + Common.sqlChar(getQ_enterOrg2()) ;
					}
				} else {
			    ...
			    if(!"".equals(getQ_buyDateS()) && !"".equals(getQ_buyDateE()) ){
				     sql += " and a.buydate between "+ Common.sqlChar(new Datetime().changeTaiwanYYMMDD(getQ_buyDateS(),"2")) +" and "+ Common.sqlChar(new Datetime().changeTaiwanYYMMDD(getQ_buyDateE(),"2"));  
			    }
			    
			    if(!"".equals(getQ_barcode())){
				     sql += " and a.barcode = "+ Common.sqlChar(getQ_barcode()) ;
			    }
			    
			    if(!"".equals(getQ_checkResult())){
				     sql += " and a.checkresult = "+ Common.sqlChar(getQ_checkResult()) ;
			    }
			    
			    if(!"".equals(getQ_oldPropertyNo())){
				     sql += " and a.oldpropertyno = "+ Common.sqlChar(getQ_oldPropertyNo()) ;
			    }

			    if(!"".equals(getQ_oldSerialNoS()) && !"".equals(getQ_oldSerialNoE()) ){
				     sql += " and a.oldserialno between "+ Common.sqlChar(getQ_oldSerialNoS()) +" and "+ Common.sqlChar(getQ_oldSerialNoE());
			    }
			    ...
			    if("Y".equals(getQ_scrappednote())){
				     sql += " and a.scrappednote = "+ Common.sqlChar(getQ_scrappednote()) ;
			    }
			    
			    if("Y".equals(getQ_labelnote())){
				     sql += " and a.labelnote = "+ Common.sqlChar(getQ_labelnote()) ;
			    }
			    
			    if("Y".equals(getQ_movenote())){
				     sql += " and a.movenote = "+ Common.sqlChar(getQ_movenote()) ;
			    }
		
	
		ResultSet rs = db.querySQL(sql.toString(),true);

		System.out.println("rs:"+sql.toString());
...
//			 rs.beforeFirst();
			do{
				...
				String checkresult = Common.get(rs.getString("checkresult"));
...
//public UNTPD006F  queryOne() throws Exception{
//	Database db = new Database();
//	UNTPD006F obj = this;
//	try {
//		String sql=" select checkresult, scrappednote, labelnote, movenote, oddscause  "+
//			" from UNTPD_CHECKMOVABLE  where 1=1 " ;
...
protected String[] getUpdateSQL(){
	...
	if(getsKeySet()!=null){
		...
		for (int i=0; i<getcut; i++) {	//將挑選的資料組成一字串
			...
			execSQLArray[i]=" update UNTPD_CHECKMOVABLE set " +
