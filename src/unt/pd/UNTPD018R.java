...
*<br>--------------------------------------------------------
*/
public class UNTPD018R extends SuperBean{
	...
	private String differencekindname; //區分別
	...
	public void setDifferencekindname(String s) { this.differencekindname = checkSet(s);}
	...
	private StringBuilder doGetSQLforUNTPD_CHECKNONEXP(StringBuilder stb){
		stb.append(" select ")
		          .append("( select k.codename from SYSCA_CODE k where k.codeid ="+checkGet(getQ_differencekind())+" and k.codekindid = 'DFK') as differencekindname,")
		          .append("c.propertyno,")
		           .append("c.serialno,")
		          .append("c.propertyname,")
		          .append("c.propertyname1,")
		          .append("d.propertyunit,")
		          .append("c.bookamount,")
		          .append("c.actualamount,")
		          .append("c.keepunitname,")
		          .append("c.keepername,")
		          .append("c.notes,")
		          .append("c.closingdate")
		   .append(" from UNTPD_CHECKNONEXP c, SYSPK_PROPERTYCODE2 d ")
		   .append(" where 1=1 ")
		   .append(" and d.enterorg=c.enterorg ")
		   .append(" and d.propertyno=c.propertyno ");
		   if(!"".equals(getQ_enterOrg())){
			   stb.append(" and c.enterorg = '"+getQ_enterOrg()+"' " );
		   }
		   if(!"".equals(getQ_differencekind())){
			   stb.append(" and c.differencekind = '"+getQ_differencekind()+"' " );
		   }
		   if(!"".equals(getClosingDate())){
			   stb.append(" and c.closingdate <= '"+Datetime.changeTaiwanYYMMDD(getClosingDate(), "2")+"' ");
		   }
		   ...
		   stb.append(" order by c.propertyno, c.propertyunit  ");
		return stb;
	}
...
//	    Map DifferenceMap = TCGHCommon.getSysca_Code("DFK"); //財產區分別
	    Map PropertyMap = TCGHCommon.getSyspk_PropertyCode2(q_enterOrg); //財產名稱
	    try{
	    	...
	    	if(!"".equals(getQ_enterOrg())){
				...
				//使用者操作記錄
				Common.insertRecordSql(stb.toString(), this.getOrganID(), this.getUserID(), "UNTPD018R", this.getObjPath().replaceAll("&gt;", ">"));
			    ...
			    while(rs.next()){
				    ...
				    //資料截止日期
			    	if(rs.getString("closingdate") != null && rs.getString("closingdate").length() == 8){
			    		String rocClosingDate = new Datetime().changeTaiwanYYMMDD(rs.getString("closingdate"), "1");
				    	this.year=rocClosingDate.substring(0,3);
				    	this.month=rocClosingDate.substring(3, 5);
				    	this.day=rocClosingDate.substring(5, 7);
			    	...
			    	//物品名稱
			    	data[6] = (String)PropertyMap.get(rs.getString("propertyno"));
			    	...
			    	//帳面數量
			    	data[9] =Double.parseDouble(rs.getString("bookamount") == null? "0":rs.getString("bookamount"));
				    	
			    	//實際數量
			    	data[10] =Double.parseDouble(rs.getString("actualamount") == null? "0":rs.getString("actualamount"));
