...
public class UNTLA035F extends UNTLA027Q{


String useState1 ;	
String useState1Name ;
...
public String getUseState1Name(){ return checkGet(useState1Name); }
public void setUseState1Name(String s){ useState1Name=checkSet(s); }
...
//		System.out.println("縣市："+signNo1.substring(0,1));
//		System.out.println("鄉鎮市區："+signNo2.substring(1,3));
//		System.out.println("地段小段："+signNo3.substring(3,7));
//		System.out.println("母號："+signNo4);
//		System.out.println("子號："+signNo5);
	return execSQLArray;
}
...
protected String[] getUpdateSQL(){
	...
	execSQLArray[0]=" update UNTLA_ReduceDetail set " +
			...
			" useState1 = " + Common.sqlChar(useState1) + "," +
...
public UNTLA035F  queryOne() throws Exception{
	...
	try {
		String sql=" select (select b.organSName from SYSCA_Organ b where b.organID = a.enterOrg ) organSName," +
				...
				" (case a.useState1 when '01' then '空置' when '02' then '建物' when '03' then '農作' when '04' then '其他' end)as useState1Name, a.useState1," + 
				" (select c.propertyName from SYSPK_PropertyCode c where c.propertyNo= a.propertyNo ) propertyName," +
				" a.serialNo, a.propertyName1, a.signNo, a.cause, a.cause1, a.reduceDate, a.verify, a.propertyKind, a.fundType, a.valuable, a.taxCredit, a.area, a.holdNume, a.holdDeno, a.holdArea, a.bookNotes, a.accountingTitle, a.bookUnit, a.bookValue, a.useSeparate, a.useKind, a.proofDoc, a.field, a.sourceDate, a.useState, a.bulletinDate, a.valueUnit, a.notes, a.oldPropertyNo, a.oldSerialNo, a.editID, a.editDate, a.editTime, a.closing  "+
...
//	System.out.println("untla035f--queryOne--"+sql);
		...
		if (rs.next()){
			...
			obj.setUseState1Name(rs.getString("useState1Name"));
...
//System.out.println(sql);
		...
		while (rs.next()){
			...
			rowArray[8]=rs.getString("signNoName");
