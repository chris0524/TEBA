...
public class UNTLA005F extends UNTLA001Q{
...
protected String[][] getInsertCheckSQL(){
	String[][] checkSQLArray = new String[2][4];
 	checkSQLArray[0][0]=" select count(*) as checkResult from UNTLA_Value where 1=1 " + 
...
protected String[] getInsertSQL(){
	...
	String sql="select suitDateS,suitDateE from UNTLA_BulletinDate " +
		" where bulletinKind = '1'"+  
		" and bulletinDate = " + Common.sqlChar(ut._transToCE_Year(bulletinDate)) +  	
		"";		
	try {		
		rs = db.querySQL(sql);
		if (rs.next()){
		    setSuitDateS(ut._transToROC_Year(rs.getString("suitDateS")));
		    setSuitDateE(ut._transToROC_Year(rs.getString("suitDateE")));
		}
	} catch (Exception e) {
		e.printStackTrace();
	} finally {
	...
	execSQLArray[0]=" insert into UNTLA_Value(" +
			" enterOrg,"+
			" ownership,"+
			" propertyNo,"+
			" serialNo,"+
		...
		" )Values(" +
			...
			Common.sqlChar(ut._transToCE_Year(bulletinDate)) + "," +
			Common.sqlChar(valueUnit) + "," +
			Common.sqlChar(ut._transToCE_Year(suitDateS)) + "," +
			Common.sqlChar(ut._transToCE_Year(suitDateE)) + "," +
...
protected String[] getUpdateSQL(){
	String[] execSQLArray = new String[1];
	UNTCH_Tools ut = new UNTCH_Tools();
	execSQLArray[0]=" update UNTLA_Value set " +
			" valueUnit = " + Common.sqlChar(valueUnit) + "," +
			" suitDateS = " + Common.sqlChar(ut._transToCE_Year(suitDateS)) + "," +
			" suitDateE = " + Common.sqlChar(ut._transToCE_Year(suitDateE)) + "," +
...
protected String[] getDeleteSQL(){
	String[] execSQLArray = new String[1];
	UNTCH_Tools ut = new UNTCH_Tools();
	execSQLArray[0]=" delete UNTLA_Value where 1=1 " +
			" and enterOrg = " + Common.sqlChar(enterOrg) +
			" and ownership = " + Common.sqlChar(ownership) +
			" and propertyNo = " + Common.sqlChar(propertyNo) +
			" and serialNo = " + Common.sqlChar(serialNo) +
...
public UNTLA005F  queryOne() throws Exception{
	Database db = new Database();
	UNTLA005F obj = this;
	try {
		UNTCH_Tools ut = new UNTCH_Tools();
		String sql=" select a.enterOrg, a.ownership, a.propertyNo, a.serialNo, a.bulletinDate, a.valueUnit, a.suitDateS, a.suitDateE, a.notes, a.editID, a.editDate, a.editTime, a.differenceKind "+
			" from UNTLA_Value a where 1=1 " +
			" and a.enterOrg = " + Common.sqlChar(enterOrg) +
			" and a.ownership = " + Common.sqlChar(ownership) +
			" and a.propertyNo = " + Common.sqlChar(propertyNo) +
			" and a.serialNo = " + Common.sqlChar(serialNo) +
			" and differenceKind = " + Common.sqlChar(differenceKind) +
			" and a.bulletinDate = " + Common.sqlChar(ut._transToCE_Year(bulletinDate)) +
			"";
		...
		if (rs.next()){
			...
			obj.setSuitDateS(ut._transToROC_Year(rs.getString("suitDateS")));
			obj.setSuitDateE(ut._transToROC_Year(rs.getString("suitDateE")));
...
public ArrayList queryAll() throws Exception{
	...
	try {
		String sql=" select a.enterOrg, a.ownership, a.propertyNo, a.serialNo, a.bulletinDate, a.valueUnit, a.suitDateS, a.suitDateE "+
			" from UNTLA_Value a where 1=1 "+ 
			" and a.enterOrg = " + Common.sqlChar(enterOrg) +
			" and a.ownership = " + Common.sqlChar(ownership) +
			" and a.propertyNo = " + Common.sqlChar(propertyNo) +
		...
		if (getTotalRecord() > 0) {
			...
			rowArray[5]=rs.getString("valueUnit"); 
