...
public class UNTLA014F extends SuperBean{
...
protected String[] getInsertSQL(){
	...
	try {
		for (i=0; i<getcut; i++) {	
			...
			if (rs.next()){
			// getvalue 的值
			strgetValue = getValue(strKeys[0].toString(),strKeys[1].toString(),strKeys[2].toString(),strKeys[3].toString(),rs.getString("originalDate"),rs.getString("originalBasis"),rs.getString("originalUnit"));
			//insert 資料
...
public UNTLA014F  queryOne() throws Exception{
	Database db = new Database();
	UNTLA014F obj = this;
	try {
...
public ArrayList queryAll() throws Exception{
	...
	int counter=0;
	try {
		String sql = " select a.enterOrg ,d.propertyName ,a.ownership as ownership ,a.caseNo ,a.area ,a.holdNume ,a.holdDeno " + "\n"
		   		   ...
		   		   + "		  ,a.propertyNo ,a.serialNo ,a.signNo ,a.propertyKind ,a.holdArea " + "\n"
		   		   ...
		   		   + "		  and a.enterOrg = c.organID " + "\n"
		   		   + "		  and a.propertyNo = d.propertyNo " + "\n"
		   		   + "		  and (a.enterOrg)  not in (select enterOrg from UNTLA_ReduceDetail   ) " + "\n"  
			   	   + "		  and (a.ownership)  not in (select ownership from UNTLA_ReduceDetail   ) " + "\n"
			   	   + "		  and (a.propertyNo)  not in (select propertyNo from UNTLA_ReduceDetail   ) " + "\n"
			   	   + "		  and (a.serialNo)  not in (select serialNo from UNTLA_ReduceDetail   ) " + "\n"
...
//取得untla_value欄位資料
protected String[] getValue(String penterOrg,String pownership,String ppropertyNo,String pserialNo,String poriginalDate,String poriginalBasis,String poriginalUnit){	
    Database db = new Database();
	ResultSet rs;
