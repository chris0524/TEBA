...
public class UNTLA026R extends SuperBean{
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	String[] columns = new String[] {"ENTERORGNAME","OWNERSHIP","VALUABLE","PROPERTYKIND","FUNDTYPE","SIGNNO","SIGNDESC","ORIGINALBV","OLDBOOKVALUE","ADJUSTBOOKVALUE","NEWBOOKVALUE","SIGNNO1"};
    	...
    	String execSQL="select (select c.organAName from SYSCA_Organ c where a.enterOrg=c.organID) as enterOrgName,a.valuable,a.propertykind,(select f.codename  from SYSCA_CODE f where f.codekindid = 'FUD' and a.fundtype=f.codeid )as fundtype," +
    			...
    			"a.signno, a.originalBV,a.oldBookValue,a.adjustBookValue,a.newBookValue,a.signno as signno1 ";
    	execSQL=execSQL+" from UNTLA_AdjustDetail a";
    	execSQL=execSQL+" where 1=1 ";
    	execSQL+=" and a.cause='2' ";
    	...
    	if("1".equals(Common.get(getQ_ownership()))){
    		enterownership=getQ_bulletinDate().substring(1,3)+"年度市有土地評定現值差額清冊";
    		
    	}
    	else enterownership=getQ_bulletinDate().substring(1,3)+"年度國有土地評定現值差額清冊";
        ...
        while(rs.next()){
            ...
            data[19]=rs.getString("signNo1").substring(0,3);
