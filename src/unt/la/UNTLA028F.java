...
public class UNTLA028F extends UNTLA027Q{
...
protected String[][] getInsertCheckSQL(){
	String[][] checkSQLArray = new String[2][4];
 	checkSQLArray[0][0]=" select count(*) as checkResult from UNTLA_ReduceProof where 1=1 " + 
		...
		" and caseNo = " + Common.sqlChar(reduceCaseNo) + 
		"";
	checkSQLArray[0][1]=">";
	checkSQLArray[0][2]="0";
	...
	checkSQLArray[1][0]=" select count(*) as checkResult from UNTLA_MergeDivision where 1=1 " + 
		...
		" and mergeReduce is not null " + 
		"";
	checkSQLArray[1][1]=">";
	checkSQLArray[1][2]="0";
	checkSQLArray[1][3]="該案件之合併減損單已存在，不可再新增！";
...
protected String[] getInsertSQL(){
	...
	execSQLArray[1]=" update UNTLA_MergeDivision set " +
			" mergeReduce = " + Common.sqlChar(reduceCaseNo) + "," +
...
public UNTLA028F  queryOne() throws Exception{
	Database db = new Database();
	UNTLA028F obj = this;
	try {
		...
		if (rs.next()){
			...
			obj.setReduceCaseNo(reduceCaseNo);
