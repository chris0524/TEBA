...
public class UNTLA012F extends UNTLA012Q{
	...
	private Map getRecordMap(){
		...
		if(this.checkProofyearChanged(getEnterOrg(), getCaseNo(), getProofYear())){
			proofno = MaxClosingYM.getMaxProofNo("UNTLA_REDUCEPROOF",enterOrg,ownership,this.getProofYear());
		}
...
private boolean checkProofyearChanged(String enterorg, String caseno, String newproofyear){
	...
	try{
		String sql = "select proofyear from UNTLA_REDUCEPROOF where enterorg=" + Common.sqlChar(enterorg) +
...
//	setProofNo(MaxClosingYM.getMaxProofNo("UNTLA_REDUCEPROOF",enterOrg,ownership));
	...
	execSQLArray[0] = new SQLCreator()._obtainSQLforInsert("UNTLA_REDUCEPROOF", getPKMap(), getRecordMap());
	return execSQLArray;
}
...
protected String[] getUpdateSQL(){
	...
	execSQLArray[0] = new SQLCreator()._obtainSQLforUpdate("UNTLA_REDUCEPROOF", getPKMap(), getRecordMap());
...
protected String getDBVerify(){
	...
	String sql = "select verify from UNTLA_REDUCEPROOF "
							+ "where 1=1 "
...
private String updateTable(){
		...
		if(getReVerify().equals("Y")){
			...
			querySQL = "select propertyno, serialno," +
							" area, holdnume, holddeno, holdarea," +
							" BookValue, NetValue" +
			...
			try{
				...
				while(rs.next()){
					sql += " update UNTLA_LAND set" +
				        	  " area = " + Common.sqlChar(rs.getString("area")) + "," +
...
protected String[] getDeleteSQL(){ 
		String[] execSQLArray = new String[2];
		//刪除 UNTLA_ReduceProof TABLE
		execSQLArray[0] = new SQLCreator()._obtainSQLforDelete("UNTLA_REDUCEPROOF", getPKMap(), getRecordMap());
		//刪除 UNTLA_ReduceDetail TABLE
...
public UNTLA012F  queryOne() throws Exception{
	Database db = new Database();
	UNTCH_Tools ut = new UNTCH_Tools();
	UNTLA012F obj = this;
	try {
		String sql=" select a.*, b.organSName," +
			...
			" from UNTLA_ReduceProof a, SYSCA_Organ b where 1=1 " +
		...
		if (rs.next()){
			...
			obj.setPreenterdate(ut._transToROC_Year(rs.getString("preenterdate")));
...
public void approveRe()throws Exception{	
	...
	try {
		...
		while (rs.next()){
			...
			//------------------------------------------------------------------------------------------------			
			if(verify.equals("Y") && mergeDivision==null && reduceCount.equals("0") && adjustCount.equals("0") && reduceMax.equals("0") && adjustMax.equals("0")){				
