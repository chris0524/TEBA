...
public class UNTLA023R extends SuperBean{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	...
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOFNO","CASENO","MANAGENO"
    									,"SOURCEDATE","PROPERTYNO","PROPERTYNAME","SIGNNO","CAUSE","OLDHOLDAREA"
    									,"OLDBOOKVALUE","ADJUSTHOLDAREA1","ADJUSTBOOKVALUE1","ADJUSTHOLDAREA2"
    									,"ADJUSTBOOKVALUE2","NEWHOLDAREA","NEWBOOKVALUE","KIND","UNIT"
    									,"summonsNo","propertyKind","propertyKindName","ownershipTitle"};
    	
    	String execSQL = " select (select c.organAName from SYSCA_Organ c where a.enterOrg = c.organID) as enterOrgName ,a.writeDate ,a.writeUnit ,a.summonsNo " + "\n"
    				   + "		 ,a.proofDoc, a.proofNo " + "\n"
    				   + "		 ,a.caseNO ,a.manageNo ,b.sourceDate " + "\n"
    				   + "		 ,b.propertyNo, b.serialNo " + "\n"
    				   + "		 ,d.propertyName , b.signno " + "\n"
    				   + "		 ,b.cause ,b.cause1 ,b.oldHoldArea ,b.oldBookValue ,b.adjustHoldArea ,b.adjustBookValue " + "\n"
    				   + "		 ,b.newHoldArea ,b.newBookValue ,b.propertyKind " + "\n"
    				   + "		 ,(select x.codeName from sysca_code x where b.propertyKind = x.codeid and x.codekindid = 'PKD') as propertyKindName " + "\n"
    				   + "		 ,(select x.codeName from sysca_code x where b.ownership = x.codeid and x.codekindid = 'OWA') as ownershipName " + "\n"
    				   + " from UNTLA_AdjustProof a,UNTLA_AdjustDetail b,SYSPK_PropertyCode d" + "\n"
    	...
    	if("4".equals(q_kind)){
    		...
    		while(rs.next()){
                ...
                if("1".equals(data[10]))data[10]="公告地價調整";
                else if("2".equals(data[10]))data[10]="資產重估調整";
                else data[10]=causetemp;
                data[11]=Common.areaFormat(rs.getString("oldHoldArea"))+"　";
                tansToDouble= rs.getString("oldBookValue");
                if(tansToDouble!=null)data[12]= new Double(Double.parseDouble(tansToDouble));
                else data[12]=null;
                tansToDouble= rs.getString("adjustHoldArea");
                if(tansToDouble!=null){
                	Doubletemp= Double.parseDouble(tansToDouble);
                	if(Doubletemp<0){data[15]=Common.areaFormat(rs.getString("adjustHoldArea"))+"　";data[13]="0.00　";}
                	else {data[13]=Common.areaFormat(rs.getString("adjustHoldArea"))+"　";data[15]="0.00　";}
                }
                else {
                ...
                if(tansToDouble!=null){
                	Doubletemp= Double.parseDouble(tansToDouble);
                	if(Doubletemp<0){data[16]=new Double(Doubletemp);data[14]=new Double(0);}
                	else {data[14]=new Double(Doubletemp);data[16]=new Double(0);}
                }
                else {
                	data[14]=null;data[16]=null;
                }
                data[17]= Common.areaFormat(rs.getString("newHoldArea"))+"　";
                ...
                data[24] = Common.get(rs.getString("ownershipName")) + "財產增減值單";
