...
public class UNTLA030F extends UNTLA027Q{
...
protected String[][] getInsertCheckSQL(){
	String[][] checkSQLArray = new String[2][4];
 	checkSQLArray[0][0]=" select count(*) as checkResult from UNTLA_AddProof where 1=1 " + 
		...
		" and caseNo = " + Common.sqlChar(addCaseNo) + 
		"";
	checkSQLArray[0][1]=">";
	checkSQLArray[0][2]="0";
...
protected String[] getInsertSQL(){
	...
	execSQLArray[1]=" update UNTLA_MergeDivision set " +
			" mergeAdd = " + Common.sqlChar(addCaseNo) + "," +
...
protected String[] getDeleteSQL(){
	...
	for(int i=0;i<delTable.length;i++){
	    execSQLArray[i]=" delete " + delTable[i]+ " a where exists ( " +
			...
			" and b.caseNo = " + Common.sqlChar(addCaseNo) +
			" ) " +
			"";	
	}
...
public UNTLA030F  queryOne() throws Exception{
	Database db = new Database();
	UNTLA030F obj = this;
	try {
...
public ArrayList queryAll() throws Exception{    
	...
	int counter=0;
	try {
		String sql=" select a.enterOrg, ownership,(case a.ownership when '1' then '市有' when '2' then '國有' else '' end) ownershipName, a.caseNo, a.caseName,"+
		    " a.writeDate, a.enterDate, (case a.verify when '1' then '是' else '否' end) verify, (case a.closing when '1' then '是' else '否' end) closing "+
			" from UNTLA_AddProof a where 1=1 " +
			" and a.enterOrg = " + Common.sqlChar(enterOrg) +
			" and a.ownership = " + Common.sqlChar(ownership) +
			" and a.caseNo = " + Common.sqlChar(addCaseNo) +
