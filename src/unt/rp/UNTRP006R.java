...
public class UNTRP006R extends SuperBean {
		    ...
		    if(kindName != null){
				for(int i = 0 ; i < kindName.length; i++){
				    ...
				    while(rs.next()){
				    	...
				    	if(!"".equals(writedate)){
				    		data[2] = writedate.substring(0,3) + "年" + writedate.substring(3,5) + "月" + writedate.substring(5) + "日";
				    	}else{
				    		data[2] = "";	
				    	}
				    	//填造單位
				    	String writeunit = ut._queryData("writeunit")._from("UNTLA_REDUCEPROOF a ")._with(queryCondition+" and a.caseno='"+checkGet(rs.getString("caseNo"))+"' ")._toString();
				    	if(!"".equals(checkGet(writeunit))){
				    		data[3] = ut._queryData("unitname")._from("UNTMP_KEEPUNIT")._with(" and enterorg = '" + getQ_enterOrg() + "' and unitno = '" + writeunit + "'")._toString();
				    	}
				    	//印表人
				    	data[4] = ut._queryData("empname")._from("SYSAP_EMP a ")._with("and organid = '" + getOrganID() + "' and empid = '" + getEditID() + "'")._toString();
				    	//全銜
				    	data[5] = ReportUtil.getTitleByEnterOrgCode(this.getQ_enterOrg(), this.getQ_differenceKind());
				    	...
				    	// 單據編號
				    	data[8] = ut._queryData("proofYear")._from("UNTLA_REDUCEPROOF a ")._with(queryCondition+" and a.caseno='"+checkGet(rs.getString("caseNo"))+"' ")._toString() + 
			    				  "年" +
			    				  ut._queryData("proofDoc")._from("UNTLA_REDUCEPROOF a ")._with(queryCondition+" and a.caseno='"+checkGet(rs.getString("caseNo"))+"' ")._toString() +
			    				  "字第" +
			    				  ut._queryData("proofNo")._from("UNTLA_REDUCEPROOF a ")._with(queryCondition+" and a.caseno='"+checkGet(rs.getString("caseNo"))+"' ")._toString() +
			    				  "號";
				    	...
				    	//===========================================================
				    	//取得日期
				    	data[9] = Common.MaskDate(ut._transToROC_Year(checkGet(rs.getString("sourceDate")))) +"\n"+
				    			  Common.MaskDate(ut._transToROC_Year(checkGet(rs.getString("buydate"))));
				    	//財產編號 (含分號) 財產名稱
				    	data[10] = checkGet(rs.getString("propertyNo")) + "－" + checkGet(rs.getString("serialNo")) + "\n" + 
	...
	//			    	data[10] = ReportUtil.getStringWithOldSerialNo(this.getQ_enterOrg(), Common.get(data[10]), "原財產分號", Common.get(rs.getString("oldserialno")));
				    	...
				    	//別名/型式/廠牌
				    	if("".equals(checkGet(rs.getString("propertyName1")))){			    			
				    	}else{
				    		data[11] = checkGet(rs.getString("propertyName1"));
				    	}
				    	if("".equals(checkGet(rs.getString("specification"))) && "".equals(checkGet(rs.getString("nameplate")))){			    			
				    	}else{
				    		data[11] = checkGet(rs.getString("specification")) + "/" + checkGet(rs.getString("nameplate"));
				    	}
				    	...
				    	data[12] = unitName;
				    	//數量
				    	data[13] = checkGet(rs.getString("amount"));
				    	
				    	//總價 帳面價值
				    	data[14] = st._getMoneyFormat(checkGet(rs.getString("bookvalue"))) + "\n" +
				    	...
				    	//繳存地點
				    	data[16] = checkGet(rs.getString("returnPlace"));	
				    	...
				    	data[20] = st._getMoneyFormat(sum_bookValue) + "\n" +
				    	...
				    	//===========================================================
				    	//單據備註
				    	data[21] = ut._queryData("notes")._from("UNTLA_REDUCEPROOF a ")._with(queryCondition+" and a.caseno='"+checkGet(rs.getString("caseNo"))+"' ")._toString();
				    	
				    	//聯數群組判斷用
				    	data[22] = checkGet(rs.getString("caseNo"))+"_"+String.valueOf(i);
				    	//判斷是否列印備註用
				    	data[23] = q_note;
				    	
				    	//累計折舊  帳面總價
						data[24] = st._getMoneyFormat("".equals(checkGet(rs.getString("accumdepr")))?"0":rs.getString("accumdepr"));
						
						//累計折舊 總計
						data[25] = st._getMoneyFormat(sum_accumdepr);
		    ...
		    Collections.sort(rowData,
		            new Comparator<Object[]>() {
		                public int compare(Object[] o1, Object[] o2) {
		                    return o1[22].toString().compareTo(o2[22].toString());
		                }
		            });
	...
	//=================================================================
		...
		private StringBuilder doGetSQLforUNTLA_ReduceDetail(StringBuilder stb){
			stb.append("select ").append(
							...
							" a.bookvalue, ").append(
							" a.netvalue, ").append(										
							" ((select codename from SYSCA_CODE z where codekindid = 'CAA' and z.codeid = a.cause and codecon1 in (2,4)) + (case a.cause when '499' then ':' + a.cause1 else ' ' end)) as causeName, ").append(
							" null as returnPlace, ").append(
							" null as otherLimitYear, ").append(
							" null as otherpropertyunit, ").append(
							" null as cause2, ").append(
							" null as keeper, ").append(
