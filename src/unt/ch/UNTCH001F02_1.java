 ...
 * <br/>--------------------------------------------------------
 */
public class UNTCH001F02_1 extends UNTCH001Q {
	...
	protected void _setDefaultValue(){	
		String signNo = null;
		String area = null;
		String holdNume = null;
		String holdDeno = null;
		String holdArea = null;
		...
		if(_table_LA.equals(checkStr)){
			...
			area = this.getAreaLa();
			holdNume = this.getHoldNumeLa();
			holdDeno = this.getHoldDenoLa();
			holdArea = this.getHoldAreaLa();
			...
			area = this.getAreaBu();
			holdNume = this.getHoldNumeBu();
			holdDeno = this.getHoldDenoBu();
			holdArea = this.getHoldAreaBu();
			...
			originalArea = this.getAreaLa();
			originalHoldNume = this.getHoldNumeLa();
			originalHoldDeno = this.getHoldDenoLa();
			originalHoldArea = this.getHoldAreaLa();
		}
		...
		this.setCArea(getCArea());
		this.setSArea(getSArea());
		this.setArea(area);
		this.setHoldNume(holdNume);
		this.setHoldDeno(holdDeno);
		this.setHoldArea(holdArea);
	...
	public UNTCH001F02_1 _execQueryOneforDetail(){
		...
		try{
			...
			this.setEnterOrgName(ul._getOrganAName(this.getEnterOrg()));
			...
			if(_table_LA.equals(checkStr)){				
				...
				this.setAreaLa(this.getArea());
				this.setHoldNumeLa(this.getHoldNume());
				this.setHoldDenoLa(this.getHoldDeno());
				this.setHoldAreaLa(this.getHoldArea());
				...
				this.setAreaBu(this.getArea());
				this.setHoldNumeBu(this.getHoldNume());
				this.setHoldDenoBu(this.getHoldDeno());
				this.setHoldAreaBu(this.getHoldArea());
				this.setOriginalAmount("1");
				
				this.setPropertyUnit("平方公尺");
				
				this.setDtlBookAmountRO("1");
				this.setDtlBookValueRO(this.getBookValue());
				this.setDtlOriginalAmountRO("1");
				this.setDtlOriginalBVRO(this.getOriginalBV());
				this.setDtlNetValueRO(this.getNetValue());
				...
				this.setDtlBookAmountRO(this.getBookAmount());
				this.setDtlBookValueRO(this.getBookValue());
				this.setDtlNetValueRO(this.getNetValue());
				this.setDtlOriginalAmountRO("1");
				this.setDtlOriginalBVRO(this.getOriginalBV());
				...
				this.setOriginalDeprMethod("01");
				this.setPropertyUnit(ul._getPropertyUnit(this.getPropertyNo()));
				this.setDtlOriginalAmountRO(this.getOriginalAmount());
				this.setDtlOriginalBVRO(this.getOriginalBV());
				this.setDtlBookAmountRO(this.getBookAmount());
				this.setDtlBookValueRO(this.getBookValue());
			}
			...
			if("".equals(this.getObjPath())){
				this.setObjPath("功能選單 > > 單位財產系統 > > 財產查詢及維護 > >財產查詢" + this.getPropertyNo() +"-" + this.getSerialNo());
			}
			Common.insertRecordSql("" , this.getOrganID(), this.getUserID(), "UNTCH001F02_1", this.getObjPath().replaceAll("&gt;", ">"),"query");
		...
		private void queryProofNo(){
			String sql = "select proofno from UNTLA_ADDPROOF where 1=1" +
							" and enterorg = " + Common.sqlChar(this.getEnterOrg()) +
							" and ownership = " + Common.sqlChar(this.getOwnership()) +
							" and differencekind = " + Common.sqlChar(this.getDifferenceKind()) +
							" and caseno = " + Common.sqlChar(this.getCaseNo());
	...
	*/
	...
	public ArrayList execQueryAll() throws Exception{
		...
		try {
			...
			sbSQL.append("select ROW_NUMBER() over (order by datastate,differencekind,propertyno,serialno) as rownum,* from (").append(								
							"select").append(
								" 'LA' as type, ").append(	
								" enterOrg, ").append(
								" ownership, ").append(
					...
					" union ").append(
						" select").append(		
								...
								" z.originalBV, ").append(
								" z.datastate,").append(
								" z.propertyname1,").append(
								//" z.usernote,").append(
								" (select z.limityear from SYSPK_PROPERTYCODE z where z.propertyno = a.propertyno and z.enterorg = '000000000A') as limityear,").append(
								" z.otherlimityear, ").append(
								" z.nodeprset").append(
...
//					}
					...
					rowArray[fieldIndex++] = st._getMoneyFormat(rs.getString("originalBV"));
					rowArray[fieldIndex++] = checkGet(uctls._transToROC_Year(rs.getString("sourceDate")));
					rowArray[fieldIndex++] = Common.get(rs.getString("otherlimityear"));
					rowArray[fieldIndex++] = uctls._getDataStateName(rs.getString("datastate"));
					rowArray[fieldIndex++] = "Y".equals(Common.get(rs.getString("nodeprset"))) ? "是" : "";
		...
		private String getQueryCondition(String table){
			...
			if (!"".equals(getQ_engineeringNo())){
				result.append(" and a.engineeringNo  = ").append(Common.sqlChar(getQ_engineeringNo()));
			}
			...
			if (!"".equals(getQ_propertyName1())){
				if("MP".equals(table)){
					result.append(" and z.propertyName1 like ").append(Common.sqlChar("%" + getQ_propertyName1() + "%"));
				}else{
			...
			if (!"".equals(getQ_dataState())){
				if("MP".equals(table)){
					result.append(" and z.dataState = ").append(Common.sqlChar(getQ_dataState()));
				}else{
...
//			}
			...
			if (!"".equals(getQ_fundsSource()) && !("RT".equals(table) || "VP".equals(table))) {
				result.append(" and a.fundsSource = ").append(Common.sqlChar(getQ_fundsSource()));
			}
			
			if (!"".equals(getQ_detail_propertyName1())) {
				if ("MP".equals(table)) {
					result.append(" and z.propertyname1 like '%").append(this.getQ_detail_propertyName1()).append("%' ");
				} else {
					result.append(" and 1!=1 ");
				}
			}
