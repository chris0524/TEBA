 ...
 * <br/>--------------------------------------------------------
 */
public class UNTCH001F02_2 extends UNTCH001Q {
	...
	protected util.SuperBean2[] _checkPropertyNoType(){
		...
		if(_table_LA.equals(checkStr)){				sb[0] = new unt.la.UNTLA002F(this.getClass().getSimpleName());
	...
	protected void _setDefaultValue(){
		...
		this.setEditTime(util.Datetime.getHHMMSS());
		...
		this.setBuildFeeCB(getBuildFeeCB());
		this.setDeprUnitCB(getDeprUnitCB());
		this.setMoveDate(getMoveDate());
		this.setKeepUnit(getKeepUnit());
		this.setKeeper(getKeeper());
		this.setUseUnit(getUseUnit());
		this.setUserNo(this.getUserNo());
		this.setUserNote(getUserNote());
	...
	public UNTCH001F02_2 _execQueryOneforDetail(){
			...
			if (getTotalRecord() > 0) {
				...
				do {
					if (count > end)
						break;
					int fieldIndex = 0;
					String rowArray[]=new String[("true".equals(this.getIsAddProof()) ? 17 : 18)];
...
//					}
		...
		private String getQueryCondition(String table){
			...
			if("detail".equals(getQuerySelect())){
				...
				/* 不確定原始用意是什麼 , 但問題單855 提出此問題 ,QueryAll後 觸發QueryOne , 以下四個條件會被set, 此時在查詢or換頁 將會與"第一次"查詢筆數不同
