...
public class UNTBU023F extends SuperBean{
...
public void insert(){
	...
	try {
		for (i=0; i<getcut; i++) {	
			...
			if (rs.next()){
			//insert 資料
				sbSQL.append(" insert into UNTBU_AdjustDetail(" ).append( 
				  " enterOrg,").append(
				  " ownership,").append(
				  " caseNo,").append(
				  " propertyNo,").append(
...
public UNTBU023F  queryOne() throws Exception{
	Database db = new Database();
	UNTBU023F obj = this;
	try {
		String sql=" select a.enterOrg, a.ownership, a.caseNo, a.propertyNo, " +
			...
			" a.CArea,a.SArea,a.area, a.holdNume, a.holdDeno, a.holdArea,a.bookValue,  " +
			" a.sourceDate,  " +
...
public ArrayList queryAll() throws Exception{
	...
	try {
		String sql= " select a.enterOrg, d.propertyName,a.ownership, a.caseNo, "+
					...
					" a.CArea,a.SArea,a.area, a.bookValue, a.sourceDate, "+
					" from UNTBU_Building a ,SYSCA_Organ c ,SYSPK_PropertyCode d,SYSCA_SIGN q  " +
					" where 1=1 and a.verify='Y' and a.dataState='1' and a.enterOrg = c.organID and a.propertyNo = d.propertyNo "+
					" and SubStr(a.signNo,1,7)= q.signNo(+) "+
		...
		while (rs.next()){
			...
			rowArray[5]=rs.getString("signName") + rs.getString("signNo").substring(7,12) + '-' + rs.getString("signNo").substring(12); 
