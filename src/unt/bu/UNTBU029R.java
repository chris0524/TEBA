...
public class UNTBU029R extends SuperBean{
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	...
    	String execSQL="select c.organAName as enterOrgName,(case a.ownership when '1' then '市有' when '2' then '國有' else '' end) as ownership," +
    			"a.area," +
    			"a.holdNume, a.holdDeno," +
    			"a.holdArea," +
    			"(case a.proofDoc when null then '免繕狀' else a.proofDoc end) as proofDoc," +
    			"a.ownershipDate," +
    			"a.enterDate," +
    			"b.drawDate," +
    			"b.drawName," +
    			"(select g.codeName from SYSCA_Code g where  b.drawCause=g.codeID and g.codeKindID = 'DCA') as drawCause ";
    	execSQL=execSQL+" from UNTBU_Building a ";
    	if("1".equals(Common.get(getQ_drawState())))
    		execSQL=execSQL+" left join SYSCA_Organ c on a.enterOrg=c.organID,UNTBU_DrawProof b  where a.enterOrg=b.enterOrg and a.ownership=b.ownership and a.propertyNo=b.propertyNo and a.serialNo=b.serialNo ";
    	else 
    		execSQL=execSQL+" left join SYSCA_Organ c on a.enterOrg=c.organID left join UNTBU_DrawProof b on a.enterOrg=b.enterOrg and a.ownership=b.ownership and a.propertyNo=b.propertyNo and a.serialNo=b.serialNo ";
        ...
        while(rs.next()){
            ...
            data[2] = rs.getString("signNo").substring(0,3) + "0000";
            data[3] = rs.getString("signNo").substring(0,7);
            
            
            String SignNo = rs.getString("signNo");
            if(SignNo!=null){
            	data[4]=SignNo.substring(7,12)+"-"+SignNo.substring(12,15);
            ...
            data[7]=Common.valueFormat(rs.getString("holdNume"))+"/"+Common.valueFormat(rs.getString("holdDeno"))+"　";
