...
public class UNTBU019R extends SuperBean{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{    	
    	...
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOFNO","CASENO","MANAGENO"
    									,"PROPERTYKIND","VALUABLE","SOURCEDATE","PROPERTYNO","PROPERTYNAME"
    									,"SIGNNO","DOORPLATE","HOLDAREA","HOLDNUME","BOOKVALUE","CAUSE"
    									,"USESTATE","PROOFDOC","KIND","summonsNo","accountingTitle","ownershipTitle"
    									};
    	...
    	String execSQL = " select c.organAName as enterOrgName ,a.writeDate ,a.writeUnit " + "\n"
    				   ...
    				   + " 		 ,b.doorPlate4 as addrname4, b.holdArea,b.holdNume, b.holdDeno " + "\n"
    				   + " 		 ,b.bookValue " + "\n"
    				   + " 		 ,(select x.codename from sysca_code x where x.codekindid = 'CAC' and b.cause=x.codeid) as cause " + "\n"
    				   + " 		 ,b.cause1 ,(select x.codename from SYSCA_CODE x where x.codekindid = 'UST' and b.useState=x.codeid)useState " + "\n"
    				   + " 		 ,b.proofDoc ,a.summonsNo ,b.accountingTitle ,b.cause as oldCause " + "\n"
    	...
    	if("4".equals(q_kind)){
    		...
    		while(rs.next()){
                ...
                if("".equals(data[11]))data[11]="";
                data[12]=rs.getString("addrname1") + rs.getString("addrname2") + rs.getString("addrname3") + rs.getString("addrname4");
                areatemp= Common.get(rs.getString("holdArea"));
                data[13]= new Double(Double.parseDouble(areatemp));
                area = Double.parseDouble(areatemp);
                ...
                if(rs.getString("oldCause").equals("01")){
                	data[16] = rs.getString("cause")+Common.get(rs.getString("newEnterOrgName"));
                }else{
                	data[16] = rs.getString("cause");
                }
                causetemp= rs.getString("cause1");
                if("其他".equals(data[16])||data[16]==null)data[16]=causetemp;
                data[17] = rs.getString("useState");
                data[18] = rs.getString("proofDoc");
                if(data[18]==null||"".equals(data[18]))data[18]="免繕狀";
                data[19] = kindName[j-1];
                data[20] = Common.get(rs.getString("summonsNo"))+"　";
                data[21] = rs.getString("accountingTitle");
                data[22] = rs.getString("ownershipName")+"財產減損單";
