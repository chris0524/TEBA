...
public class UNTBU011R extends SuperBean{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	...
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOFNO","CASENO","MANAGENO","PROPERTYKIND","VALUABLE","SOURCEDATE","PROPERTYNO","PROPERTYNAME","SIGNDESC","DOORPLATE","ORIGINALBV","ORIGINALHOLDAREA","ORIGINALHOLDNUME","PROOFDOC","SOURCEKIND","USESTATE","SCRAPVALUE","LIMITYEAR","DEPRMETHOD","KIND","summonsNo","accountingTitle"};
    	String execSQL="select c.organAName as enterOrgName," +
		    			...
		    			" (select m.addrname from sysca_addr m where m.addrkind='3' and m.addrid=b.doorPlate3) as addrname3," +
		    			" b.doorPlate4 as addrname4," +
		    			" b.originalBV," +
		    			" b.originalHoldArea," +
		    			" b.originalHoldNume, b.originalHoldDeno," +
		    			" b.proofDoc," +
		    			...
		    			" b.scrapValue," +
		    			" (case d.limitYear when null then b.otherLimitYear else d.limitYear end)limitYear," +
		    			" b.deprMethod, " +
    	...
    	*/
        ...
        while(rs.next()){
            ...
            data[9] = rs.getString("propertyNo").substring(0,7) + "-" + rs.getString("propertyNo").substring(7) + "-" + rs.getString("serialNo");
            data[10] = rs.getString("propertyName");
            data[11] = getSignDescName(rs.getString("signNo").substring(0,7)) + rs.getString("signNo").substring(7,12) + '-' + rs.getString("signNo").substring(12) + "建號";
            if("-建號".equals(data[11]))data[11]="";
            data[12] = rs.getString("addrname1") + rs.getString("addrname2") + rs.getString("addrname3") + rs.getString("addrname4");
            ...
            tansToDouble= rs.getString("originalHoldArea");
            if(tansToDouble!=null){
            	data[14]= new Double(Double.parseDouble(tansToDouble));
            	area = Double.parseDouble(tansToDouble);
            }else {
            	data[14]=null;
            	area = 0;
            }
            ...
            data[16] = rs.getString("proofDoc");
            if("".equals(data[16])){
            	data[16]="免繕狀";
            }
            data[17] = rs.getString("causeName");
            data[18] = rs.getString("useState");
            ...
            data[20] = rs.getString("limitYear");
            data[21] = rs.getString("deprMethod");
            if("02".equals(data[21]))data[21]="直線法";
            else if("03".equals(data[21]))data[21]="直線法";
            else if("04".equals(data[21]))data[21]="直線法";
            else if("05".equals(data[21]))data[21]="報廢法";
            else data[21]="";
            data[22] = q_kind;
            data[23] = Common.get(rs.getString("summonsNo"))+"　";
            data[24] = rs.getString("accountingTitle");
            /**
            if(oldCaseNo.equals(newCaseNo)){
            	totalArea = totalArea + area;
            	data[25] = Common.areaFormat(totalArea+"");
            }else{
            	totalArea = 0;
            	totalArea = totalArea + area;
            	data[25] = Common.areaFormat(totalArea+"");
