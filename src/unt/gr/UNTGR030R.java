...
public class UNTGR030R extends SuperBean {
	...
	public DefaultTableModel getResultModel(Database db) throws Exception {
	    ...
	    try{
	    	...
	    	String[] cols = new String[]{ "F0", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "CKEY" };
	    	...
	    	String sameField = " select SUBSTRING(a.propertyno,1,1) as propertyNo1, SUBSTRING(a.propertyno,2,2) as propertyNo2, " +
			   				   ...
			   				   " (select b.propertyunit from SYSPK_PROPERTYCODE b where (b.enterorg = a.enterorg or b.enterorg = '000000000A') and b.propertyno = substring(a.propertyno,1,7)) as propertyUnit, ";
	    	...
	    	String lastYearSameField = " 0 as addAmount, 0 as addArea, 0 as reduceAmount, 0 as reduceArea, " +
					   				   " 0 as nowAmount, 0 as nowArea, 0 as nowBookValue ";
	    	...
	    	String addReduceSameField2 = " 0 as nowAmount, 0 as nowArea, 0 as nowBookValue ";
	   ...
	   // 	tableLast = "";
	    	...
	    	// [結存]-[去年] ----> [1-土地][2-建物][3-土地改良][4-動產][5-有價證劵][6-權利](珍貴確定不加入有價證卷和權利的部分)
...
//	    	}
	    	...
	    	*/
	    	...
	    	String nameOf503 = TCGHCommon.getLookup(" select propertyname from SYSPK_PROPERTYCODE " +
													...
													" and propertyno = '503' ");
	    	String unitOf503 = TCGHCommon.getLookup(" select propertyunit from SYSPK_PROPERTYCODE " +
												    " where (enterorg = " + Common.sqlChar(getQ_enterOrg()) + " or enterorg = '000000000A') " +
													" and propertyno = '503' ");
	    	java.util.Map<String, java.util.Map<String, double[]>> outMap = new java.util.TreeMap<String, java.util.Map<String, double[]>>(); 
	    	String sql = " select T.propertyNo1, T.propertyNo2, T.propertyNo3, T.propertyNo4, T.propertyName, T.propertyUnit, " +
	    				 ...
	    				 " SUM(ISNULL(T.nowAmount, 0)), SUM(ISNULL(T.nowArea, 0)), SUM(ISNULL(T.nowBookValue, 0)) " +
	    				 " from (" + sb.toString() + ") as T " +
	    			     " group by T.propertyNo1, T.propertyNo2, T.propertyNo3, T.propertyNo4, T.propertyName, T.propertyUnit " +
...
//	    			key2 = "5_03_ _ _" + (nameOf503.equals("")?" ":nameOf503) + "_" + (unitOf503.equals("")?" ":unitOf503);
//	    		}else{
...
//	    				   (Common.get(rs.getString(5)).equals("")?" ":Common.get(rs.getString(5))) + "_" + 
//	    				   (Common.get(rs.getString(6)).equals("")?" ":Common.get(rs.getString(6)));
//	    		}
    			key2 = Common.get(rs.getString(1)) + "_" + Common.get(rs.getString(2)) + "_" + 
				   (Common.get(rs.getString(3)).equals("")?" ":Common.get(rs.getString(3))) + "_" +  
				   (Common.get(rs.getString(4)).equals("")?" ":Common.get(rs.getString(4))) + "_" + 
	    		...
	    		t[4] += rs.getDouble(11);
	    		t[5] += rs.getDouble(12);
	    		t[6] += rs.getDouble(7) + rs.getDouble(9) - rs.getDouble(11);
	    		t[7] += rs.getDouble(8) + rs.getDouble(10) - rs.getDouble(12);
//	    		t[6] += rs.getDouble(13);
//	    		t[7] += rs.getDouble(14);
	    		t[8] += rs.getDouble(15);
	    		...
	    		d[4] += rs.getDouble(11);
	    		d[5] += rs.getDouble(12);
	    		d[6] += rs.getDouble(7) + rs.getDouble(9) - rs.getDouble(11);
	    		d[7] += rs.getDouble(8) + rs.getDouble(10) - rs.getDouble(12);
//	    		d[6] += rs.getDouble(13);
//	    		d[7] += rs.getDouble(14);
	    		d[8] += rs.getDouble(15);
	    	}	
	    	...
	    	for(Map.Entry<String, java.util.Map<String, double[]>> o : outMap.entrySet()){
	    		for(Map.Entry<String, double[]> dtl : o.getValue().entrySet()){
	    			...
	    			double area = Common.getNumeric(d[3]);
	    			double areaTotal = Common.getNumeric(d[7]);

	    			if( "A42020000G".equals(this.getQ_enterOrg()) && "110".equals(this.getQ_differenceKind()) && "111".equals(this.getQ_reportYear())) {
	    				if (("2".equals(rowArray[0]) && "01".equals(rowArray[1]) && "06".equals(rowArray[2]) && "01".equals(rowArray[3])) 
	    						|| "建物".equals(rowArray[0]) && "房屋建築及設備小計".equals(rowArray[1])) {
	    					area += 477.79;	 
	    					areaTotal += 477.79;
	    				}
	    			}
	    			...
	    			if ("1".equals(keys[0]) && !"11".equals(keys[1])) { //若為土地則要將面積換算為公頃
		    			rowArray[6] = Common.MoneyFormat(d[0], 0) + (isLandOrBuilding?"\n" + (d[1]==0?"0":Common.MoneyFormat(d[1]/10000, 6)):"");
	    				rowArray[7] = Common.MoneyFormat(d[2], 0) + (isLandOrBuilding?"\n" + (d[3]==0?"0":Common.MoneyFormat(d[3]/10000, 6)):"");
	    				rowArray[8] = Common.MoneyFormat(d[4], 0) + (isLandOrBuilding?"\n" + (d[5]==0?"0":Common.MoneyFormat(d[5]/10000, 6)):"");
	    				rowArray[9] = Common.MoneyFormat(d[6], 0) + (isLandOrBuilding?"\n" + (d[7]==0?"0":Common.MoneyFormat(d[7]/10000, 6)):"");
	    			} else {
		    			rowArray[6] = Common.MoneyFormat(d[0], 0) + (isLandOrBuilding?"\n" + (d[1]==0?"0":Common.MoneyFormat(d[1], 2)):"");
	    				rowArray[7] = Common.MoneyFormat(d[2], 0) + (isLandOrBuilding?"\n" + (area==0?"0":Common.MoneyFormat(area, 2)):"");
	    				rowArray[8] = Common.MoneyFormat(d[4], 0) + (isLandOrBuilding?"\n" + (d[5]==0?"0":Common.MoneyFormat(d[5], 2)):"");
	    				rowArray[9] = Common.MoneyFormat(d[6], 0) + (isLandOrBuilding?"\n" + (areaTotal==0?"0":Common.MoneyFormat(areaTotal, 2)):"");
	    			}
	    			
    				rowArray[10] = Common.MoneyFormat(d[8], 0);
	    			rowArray[11] = dtl.getKey().equals("Z")?"Z":o.getKey();
	    			
	    			if(!dtl.getKey().equals("Z")){//1651 只針對明細做加總
	    				totalAmount += d[8];
	    			}
	    	...
	    	if(outMap.size() > 0){
	    		...
	    		totArray[0] = "合計";
	    		totArray[10] = Common.MoneyFormat(totalAmount, 0);
	    		totArray[11] = "T";
...
//	    	}
	...
	private String queryNowBookValue(String sameField, String nowYearSameField) throws Exception {
		...
		sb.append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ")
	...
	private String queryUNTGR_UNTGR011RForNow(String nextyear, String sameField) throws Exception {
		...
		sb.append(sameField)
		  ...
		  .append(" ,isnull(a.oldamount, 0) as nowamount ,isnull(a.oldarea, 0) as nowarea ,isnull(a.oldbookvalue, 0) as nowbookvalue")
		...
		if(nowYear){
			...
			case 1:
				sb.append(isNeed?" union all (":"")
				  ...
				  .append(isBookValue?" a.bookvalue as nowBookValue ":" a.bookvalue as nowBookValue ")
			...
			case 2:
				sb.append(isNeed?" union all (":"")
				  .append(sameField).append(nowYearSameField)
				  .append(" 1 as nowAmount, a.holdarea as nowArea, ")
				  .append(isBookValue?" a.bookvalue as nowBookValue ":" a.netvalue as nowBookValue ")
			...
			case 4:
				sb.append(isNeed?" union all (":"")
				  ...
				  .append(" 1 as nowAmount, 0 as nowArea, ")
				  .append(isBookValue?" a.bookvalue as nowBookValue ":" a.netvalue as nowBookValue ")
				  .append(" from UNTMP_MOVABLE" + yyymm + " c, UNTMP_MOVABLEDETAIL" + yyymm + " a ")
				  .append(" where c.enterorg = a.enterorg and c.ownership = a.ownership and c.differencekind = a.differencekind and c.propertyno=a.propertyno and  c.lotno = a.lotno ")
