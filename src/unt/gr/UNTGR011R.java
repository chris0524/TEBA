 ...
 *<br>--------------------------------------------------------
 */
public class UNTGR011R extends SuperBean {
	...
	/**
	 * @param isDataState    : 若為true 則增加 datastate = q_datastate , 反之不限制datastate
	 * @param enterDateType  : case enterDateType when "1" then 入帳日期     when "2" then 增減值日期     when "3" then 減損日期
	 * @param isValuable     : 若為true 則增加 valuable = q_valuable , 反之不限制 valuable
	 * @return
	 * @throws Exception
	 */
	private String getConditionSQL(boolean isDataState, int enterDateType, boolean isValuable) throws Exception {
	...
	private String getConditionSQL2() throws Exception {
		StringBuffer conditionSb = new StringBuffer();
		if("N".equals(getQ_isorganmanager())){
			//1080305 TDCM 新增 :查詢條件，依所選的「含實中」、「及所屬」控制各table「入帳機關(enterOrg)」需符合的條件
			if("".equals(getQ_enterorgKind1())){
		...
		if(!"".equals(getQ_reportYear()))
			conditionSb.append(" and a.reportyear = ").append(Common.sqlChar(""  + (Common.getInt(getQ_reportYear()))));
		return conditionSb.toString();
	}
	...
	//問題單1741 大於105年報表不使用此getResultModel 請見 untgr011p.jsp
	public DefaultTableModel getResultModel(Database db) throws Exception {
	    ...
	    try{
	    	...
	    	String[] cols = new String[]{ "F0", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "CKEY"};
	    	...
	    	String lastYearSameField = " 0 as addAmount, 0 as addArea, 0 as reduceAmount, 0 as reduceArea, " +
					   				   " 0 as nowAmount, 0 as nowArea, 0 as nowBookValue, 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue ";
	    	...
	    	String addReduceSameField2 = " 0 as nowAmount, 0 as nowArea, 0 as nowBookValue, 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue ";
	    	
	    	// TODO 本年度 增加減損 nowBookvalue 應該為0
	    	String addReduceSameField3 = " 0 as nowAmount, 0 as nowArea, bookvalue as nowBookValue, 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue ";
	    ...
	    //	tableLast = "";
	    	...
	    	ResultSet rs = null;
	    	//#region 上年度結存數 [1-土地][2-建物][3-土地改良][4-動產][5-有價證劵][6-權利]   
	    	// 數量.上年度止結存數 一律抓初始檔
	    	String UNTGR_UNTGR011R = queryUNTGR_UNTGR011R(sameField, lastYearSameField);
	    	if (!"".equals(UNTGR_UNTGR011R)) {
...
//	    	}
	    	// [結存]-[本年] bookvalue from 主檔 TODO
	    	if (sb.length() > 0) {
	    		sb.append(" union all ").append(this.queryNowBookValue(sameField, nowYearSameField));
	    	}
	    	...
	    	for(int i=1; i<=3; i++){
	    		getMovableAddReduceSQL(sb, i, addReduceSameField1, addReduceSameField2, addReduceSameField3, true);
	    	}
	    	...
	    	String sql = " select T.propertyNo1, T.propertyNo2, T.propertyNo3, T.propertyNo4, T.propertyName, " +
	    				 ...
	    				 " SUM(ISNULL(T.nowBookValue, 0)), SUM(ISNULL(T.reNetValue, 0)), SUM(ISNULL(T.nowAccumDepr, 0)), SUM(ISNULL(T.nowNetValue, 0)) " +
	    	...
	    	rs = db.querySQL(sql);
	    	while(rs.next()){
	    		if("".equals(Common.get(rs.getString(1)))){// propertyno1
	    			continue;
	    		}
	    		if("".equals(Common.get(rs.getString(2)))){// propertyno2
	    			continue;
	    		}
	    		...
	    		if("1".equals(key1)){
	    			if("11".equals(Common.get(rs.getString(2)))){ //propertyno2
	    				key1 = "111";
	    			}
	    		}
...
//	    		}else{
	    		String key2 = Common.get(rs.getString(1)) + "_" + Common.get(rs.getString(2)) + "_" + 
...
//	    		}
	    		...
	    		if("".equals(Common.get(rs.getString(5)))){
	    			String name = TCGHCommon.getLookup(" select propertyname from SYSPK_PROPERTYCODE " +
							" where (enterorg = " + Common.sqlChar(getQ_enterOrg()) + " or enterorg = '000000000A') " +
							" and propertyno like " + Common.sqlChar(Common.get(rs.getString(1))+Common.get(rs.getString(2))+Common.get(rs.getString(3))+Common.get(rs.getString(4))+"%"));
	    		...
	    		t[0] += rs.getDouble(6); // oldamount
	    		t[1] += rs.getDouble(7); // oldarea
	    		t[2] += rs.getDouble(8); // addamount
	    		t[3] += rs.getDouble(9); // addarea
	    		t[4] += rs.getDouble(10); // reduceamount
	    		t[5] += rs.getDouble(11); // reducearea
	    		t[6] += rs.getDouble(6) + rs.getDouble(8) - rs.getDouble(10); // oldamount + addamount - reduceamount = 截至本年度結存數
	    		t[7] += rs.getDouble(7) + rs.getDouble(9) - rs.getDouble(11); // oldarea + addarea - reducearea = 截至本年度面積
//	    		t[6] += rs.getDouble(12); // nowamount
//	    		t[7] += rs.getDouble(13); // nowarea
	    		t[8] += rs.getDouble(14); // nowbookvalue
	    		t[9] += rs.getDouble(15); // reNetValue
	    		t[10] += rs.getDouble(16); // nowAccumDepr
	    		t[11] += rs.getDouble(17); // nowNetValue
	    		...
	    		d[0] += rs.getDouble(6); // oldamount
	    		d[1] += rs.getDouble(7); // oldarea
	    		d[2] += rs.getDouble(8); // addamount
	    		d[3] += rs.getDouble(9); // reducearea
	    		d[4] += rs.getDouble(10); // reduceamount
	    		d[5] += rs.getDouble(11); // reducearea
	    		d[6] += rs.getDouble(6) + rs.getDouble(8) - rs.getDouble(10); // oldamount + addamount - reduceamount 
	    		d[7] += rs.getDouble(7) + rs.getDouble(9) - rs.getDouble(11); // oldarea + addarea - reducearea
//	    		d[6] += rs.getDouble(12);
//	    		d[7] += rs.getDouble(13);
	    		d[8] += rs.getDouble(14); // nowbookvalue
	    		d[9] += rs.getDouble(15); // reNetValue
	    		d[10] += rs.getDouble(16); // nowAccumDepr
	    		d[11] += rs.getDouble(17); // nowNetValue
	    	}	
	    	rs.close();
	    	rs = null;
	    	
	    	double totalAmount = 0.0d;
	    	double total_reNetValue = 0.0d;
	    	double total_nowAccumDepr = 0.0d;
	    	double total_nowNetValue = 0.0d;
	    	for(Map.Entry<String, java.util.Map<String, double[]>> o : outMap.entrySet()){
	    		for(Map.Entry<String, double[]> dtl : o.getValue().entrySet()){
	    			...
	    			// 由於「財產目錄初始檔(UNTGR_untgr011r)」之面積皆為平方公尺，但報表土地面積為公頃，故取出資料時面積應除以10,000(1公頃=10,000平方公尺)，計列至小數點下六位止，四捨五入。
    				// 上年度止結存數
    				...
    				*/
    				//問題單1301: 土地合計應為公頃(四捨五入取至小數六位)，建物面積為平方公尺(四捨五入取至小數二位) 
    				if("1".equals(o.getKey())){
    					// 上年度止結存數
    				...
    				// 結存財產總值(重估價值) for 105以後(含) 
    				rowArray[10] = Common.MoneyFormat(d[9], 0);
    				// 結存財產總值(累積折舊) for 105以後(含) 
    				rowArray[11] = Common.MoneyFormat(d[10], 0);
    				// 結存財產總值(帳面價值) for 105以後(含) 
    				rowArray[12] = Common.MoneyFormat(d[11], 0);
	    			...
	    			total_reNetValue += d[9];
	    			if(!dtl.getKey().equals("Z")){ //問題單1646 只針對明細做加總
	    				total_nowAccumDepr += d[10];
	    			}
	    			total_nowNetValue += d[11];
	    	...
	    	if(outMap.size() > 0){
	    		...
	    		totArray[0] = "合計";
	    		totArray[9] = Common.MoneyFormat(totalAmount, 0);
	    		totArray[10] = Common.MoneyFormat(total_reNetValue, 0);
	    		totArray[11] = Common.MoneyFormat(total_nowAccumDepr, 0);
	    		totArray[12] = Common.MoneyFormat(total_nowNetValue, 0);
	    		totArray[13] = "T";
...
//	    	}
	...
	public String checkTableExist(String YYY){
		String[] tableName = new String[] {"UNTLA_LAND", "UNTRF_ATTACHMENT", "UNTBU_BUILDING", "UNTMP_MOVABLE", "UNTMP_MOVABLEDETAIL", "UNTVP_ADDPROOF", "UNTRT_ADDPROOF"}; 
		String[] tableChineseName = new String[] {"土地主檔", "土地改良物主檔", "建物主檔", "動產主檔", "動產批號明細檔", "有價證券主檔", "權利主檔" }; 
		
		String[] queryMonth = new String[1];
		queryMonth[0] = Common.formatFrontZero(String.valueOf(Common.getInt(YYY)-1), 3) + MM;
...
//		.append(" from UNTGR_UNTGR011R a where 1 = 1 ").append(getConditionSQL2());
//		return sb.toString();
//	}
	
	public List getEnterorg(List enterorgList, String condition){
		 Database db = new Database();
		 String sql = "select organid from SYSCA_ORGAN where "+condition;
	    try{
	    	ResultSet rs = db.querySQL(sql);
	    	while (rs.next()) {
	    		if(rs.getString("organid") != null){
	    			enterorgList.add(rs.getString("organid"));
	    		}
	    	}
	    	rs.close();
		}catch(Exception e){
	...
	public String queryUNTGR_UNTGR011R(String sameField, String lastYearSameField) throws Exception {
		StringBuilder sb = new StringBuilder();
		List enterorgList = new ArrayList();
		if("N".equals(getQ_isorganmanager())){
			enterorgList.add(getQ_enterOrg());
		}else if("Y".equals(getQ_isorganmanager())){
			if("120".equals(getQ_differenceKind())){
				enterorgList.add(getQ_enterOrg());
				enterorgList = getEnterorg(enterorgList, " organsuperior2 = "+Common.sqlChar(getQ_enterOrg()));
			}else if("110".equals(getQ_differenceKind())){
				if(CHECK_ENTERORG.equals(getQ_enterOrg())){
					enterorgList.add(getQ_enterOrg());
					enterorgList = getEnterorg(enterorgList, " ismanager = 'Y' ");
				}else{
					enterorgList.add(getQ_enterOrg());
					enterorgList = getEnterorg(enterorgList, " organsuperior1 = "+Common.sqlChar(getQ_enterOrg()));
				}
			}
		}
		
//		 " 0 as addAmount, 0 as addArea, 0 as reduceAmount, 0 as reduceArea, " +
//			   " 0 as nowAmount, 0 as nowArea, 0 as nowBookValue, 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue ";
		
		for(int i=0; i<enterorgList.size(); i++){
			if (sb.length() > 0) {
				sb.append(" union all ");
			}
			
			sb.append(sameField).append(" a.oldamount as oldAmount, a.oldarea as oldArea, ").append(lastYearSameField)
			.append(" from UNTGR_UNTGR011R a where 1 = 1 ")
			.append(" and a.enterorg = ").append(Common.sqlChar(enterorgList.get(i).toString()));
			if(!"".equals(getQ_ownership())){
			...
			if(!"".equals(getQ_reportYear())){
				sb.append(" and a.reportyear = ").append(Common.sqlChar(""  + (Common.getInt(getQ_reportYear()))));
			}
	...
	private String queryUNTGR_UNTGR011RForNow(String nextyear, String sameField) throws Exception {
		...
		for(int i=0; i<enterorgList.size(); i++){
			...
			sb.append(sameField)
			  ...
			  .append(" ,isnull(a.oldamount, 0) as nowAmount ,isnull(a.oldarea, 0) as nowArea ,isnull(a.oldbookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
			  .append(" from UNTGR_UNTGR011R a where 1 = 1 ")
			  .append(" and a.enterorg = ").append(Common.sqlChar(enterorgList.get(i).toString()));
			if(!"".equals(getQ_ownership())){
				sb.append(" and a.ownership = ").append(Common.sqlChar(getQ_ownership()));
			...
			if(!"".equals(getQ_reportYear())){
				sb.append(" and a.reportyear = ").append(Common.sqlChar(nextyear));
			}
	...
	/**
	 * nowBookValue from 主檔
	 */
	private String queryNowBookValue(String sameField, String nowYearSameField) throws Exception {
		StringBuilder sb = new StringBuilder();
		List enterorgList = new ArrayList();
		if("N".equals(getQ_isorganmanager())){
			enterorgList.add(getQ_enterOrg());
		...
		sb.append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  .append(" from UNTLA_LAND a where 1 = 1 ").append(getConditionSQL(true, 0, true))
		  .append(" union all")
		  .append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  .append(" from UNTRF_ATTACHMENT a where 1 = 1 ").append(getConditionSQL(true, 0, true))
		  .append(" union all")
		  .append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  .append(" from UNTBU_BUILDING a where 1 = 1 ").append(getConditionSQL(true, 0, true))
		  .append(" union all")
		  .append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  .append(" from UNTMP_MOVABLEDETAIL a")
		  .append(" left join UNTMP_MOVABLE b on a.enterorg=b.enterorg and a.ownership=b.ownership and a.differencekind=b.differencekind and a.propertyno=b.propertyno and a.lotno=b.lotno")
		  .append(" where 1 = 1 ").append(getConditionSQL(true, 0, false))
		  .append(" and b.valuable = ").append(Common.sqlChar(getQ_valuable()))
		  .append(" union all")
		  .append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  ...
		  .append(sameField).append(nowYearSameField)
		  .append(" 0 as nowAmount ,0 as nowArea ,isnull(a.bookvalue, 0) as nowBookValue ,0 as reNetValue ,0 as nowAccumDepr ,0 as nowNetValue ")
		  .append(" from UNTVP_ADDPROOF a where 1 = 1 ")
		  .append(getConditionSQL(true, 0, false));
		return sb.toString();
	}
	...
	private String queryYearTotalAccumDepr(String yyyymm, String samefields) {
		...
		for(int i=0; i<enterorgList.size(); i++){
			...
			sb.append(samefields)
			  .append(" 0 as oldAmount ,0 as oldArea ,0 as addAmount ,0 as addArea ,0 as reduceAmount ,0 as reduceArea ")
			  .append(" ,0 as nowAmount ,0 as nowArea ,0 as nowBookValue ,0 as reNetValue ,isnull(CASE a.deprunitcb WHEN 'Y' THEN a.scalednewaccumdepr ELSE a.newaccumdepr END, 0) as nowAccumDepr ,0 as nowNetValue ")
	...
	/**
	 * 來源為主檔者的 上年度結存 or 本年度結存  依照 nowYear參數決定
	 * @param sb
	 * @param tableType
	 * @param yyymm
	 * @param nowYear             :  true 本年度結存  ; false 上年度結存
	 * @param sameField
	...
	public void getYearCloseSQL(StringBuilder sb, int tableType, String yyymm, boolean nowYear,
			String sameField, String nowYearSameField, String lastYearSameField, boolean isNeed, boolean isBookValue) throws Exception {
		if(nowYear){
			// 結存財產總值(來源為主檔備份檔，本年度結存)
			switch(tableType){
			case 1:
				sb.append(isNeed?" union all (":"")
				  .append(sameField).append(nowYearSameField)
				  .append(" 1 as nowAmount, a.holdarea as nowArea ");
				if (!this.isQYearOver105()) {
					// 105年度以前 的規則
					sb.append(isBookValue?" ,a.bookvalue as nowBookValue ":" ,a.bookvalue as nowBookValue ")
					  .append(", 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue");
				} else {
					// 105年度以後(含)的規則
					sb.append(" ,a.bookvalue as nowBookValue , 0 as reNetValue, a.accumdepr as nowAccumDepr, a.bookvalue as nowNetValue");
				}
				sb.append(" from UNTLA_LAND" + yyymm + " a where 1 = 1 ").append(getConditionSQL(true, 0, true))
				  .append(isNeed?")":"");
				break;
			case 2:
				sb.append(isNeed?" union all (":"")
				...
				if (!this.isQYearOver105()) {
					// 105年度以前 的規則
					sb.append(isBookValue?" a.bookvalue as nowBookValue ":" a.netvalue as nowBookValue ")
					  .append(", 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue");
				} else {
					// 105年度以後(含)的規則
					sb.append(" ,a.bookvalue as nowBookValue , 0 as reNetValue, a.accumdepr as nowAccumDepr, a.netvalue as nowNetValue");
				}
				sb.append(" from UNTBU_BUILDING" + yyymm + " a where 1 = 1 ").append(getConditionSQL(true, 0, true))
				  .append(isNeed?")":"");
				break;
			case 3:
				sb.append(isNeed?" union all (":"")
				  .append(sameField).append(nowYearSameField)
				  .append(" 1 as nowAmount, 0 as nowArea, ");
				if (!this.isQYearOver105()) {
					// 105年度以前 的規則
					sb.append(isBookValue?" a.bookvalue as nowBookValue ":" a.netvalue as nowBookValue ")
					  .append(", 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue");
				...
				sb.append(" from UNTRF_ATTACHMENT" + yyymm + " a where 1 = 1 ").append(getConditionSQL(true, 0, true))
				  .append(isNeed?")":"");
				break;
			case 4:
				sb.append(isNeed?" union all (":"")
				...
				sb.append(" from UNTMP_MOVABLEDETAIL" + yyymm + " a left join UNTMP_MOVABLE" + yyymm + " c ")
				  .append(" on c.enterorg = a.enterorg and c.ownership = a.ownership and c.differencekind = a.differencekind and c.lotno = a.lotno and c.propertyno=a.propertyno ")
				  .append(" where 1=1 ")
				  .append(getConditionSQL(true, 0, false))
			...
			case 5:
				sb.append(isNeed?" union all (":"")
				  .append(sameField).append(nowYearSameField)
				  .append(" a.bookamount as nowAmount, 0 as nowArea, ");
				if (!this.isQYearOver105()) {
					// 105年度以前 的規則
					sb.append(isBookValue?" a.bookvalue as nowBookValue ":" a.bookvalue as nowBookValue ")
					  .append(", 0 as reNetValue, 0 as nowAccumDepr, 0 as nowNetValue");
				...
				sb.append(" from UNTVP_ADDPROOF" + yyymm + " a where 1 = 1 ").append(getConditionSQL(true, 0, false))
				  .append(isNeed?")":"");
				break;
			case 6:
				sb.append(isNeed?" union all (":"")
				...
				sb.append(" from UNTRT_ADDPROOF" + yyymm + " a where 1 = 1 ").append(getConditionSQL(true, 0, false))
				  .append(isNeed?")":"");
				break;
			}
		}else{
			// 數量.上年度止結存數(來源為主檔備份檔者，上年度結存)
			switch(tableType){
			case 1:
				sb.append(isNeed?" union all (":"")
	 ...
	 * @param sb
	 * @param landType
	 * @param addReduceSameField1
	 * @param addReduceSameField2
	 * @param isNeed
	 * @throws Exception
	 */
	 ...
	 * @param addReduceSameField1
	 * @param addReduceSameField2
	 * @param addReduceSameField3
	 * @param isNeed
	 * @throws Exception
	 */
	public void getMovableAddReduceSQL(StringBuilder sb, int movableType, String addReduceSameField1, String addReduceSameField2, String addReduceSameField3, boolean isNeed) throws Exception {
		switch(movableType){
		case 1:
			sb.append(isNeed?" union all (":"")
			  .append(addReduceSameField1)
			  .append(" case when a.propertyno like '503%' then a.originalamount else 1 end  as addAmount, 0 as addArea, 0 as reduceAmount, 0 as reduceArea, ")
			  .append(addReduceSameField2)//問題單1262 由 addReduceSameField3 改 addReduceSameField2
		...
		case 3:
			sb.append(isNeed?" union all (":"")
			  .append(addReduceSameField1)
			  .append(" 0 as addAmount, 0 as addArea, case when a.propertyno like '503%' then a.adjustbookamount else 1 end  as reduceAmount, 0 as reduceArea, ")
			  .append(addReduceSameField2)
