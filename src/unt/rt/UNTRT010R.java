...
public class UNTRT010R extends UNTRT008Q{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
	...
	int oldBookValueSum=0;
	int adjustBookValueSum=0;
	int newBookValueSum=0;
	int adjustBookValue = 0;
	
	String sumHoldArea;
	String sumSetArea;
	String sumOldBookValue;
	String sumAdjustBookValue;
	String sumNewBookValue;
	String caseNo;
	String oldCaseNo;
	...
	public String getSumOldBookValue(){ return checkGet(sumOldBookValue); }
	public void setSumOldBookValue(String s){ sumOldBookValue=checkSet(s); }
	public String getSumAdjustBookValue(){ return checkGet(sumAdjustBookValue); }
	public void setSumAdjustBookValue(String s){ sumAdjustBookValue=checkSet(s); }
	public String getSumNewBookValue(){ return checkGet(sumNewBookValue); }
	public void setSumNewBookValue(String s){ sumNewBookValue=checkSet(s); }
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOF","CASENO","MANAGENO","OWNERSHIP","PROPERTYKIND","FUNDTYPE","BUYDATE","PROPERTYNO","PROPERTYNAME","PROPERTYNAME1","SOURCEKIND","MEAT","PROOFDOC1","HOLDNUME1_HOLDDENO2","REGISTERCAUSE","REGISTERDATE","OLDBOOKVALUE","ADJUSTBOOKVALUE","NEWBOOKVALUE","SETPERIOD","COMMONOBLIGEE","SETPERSON","PAYDATE","INTEREST","RENT","NOTES1","subReportDataSource","SUMHOLDAREA","SUMSETAREA","SUMOLDBOOKVALUE","SUMADJUSTBOOKVALUE","SUMNEWBOOKVALUE","CAUSE","Q_KIND","caseNoPage","summonsNo"};

    	String execSQL="select a.enterOrg as shareEnterOrg, a.ownership as shareOwnership, a.propertyNo as sharePropertyNo, a.serialNo as shareSerialNo, " +
    					...
    					" a.meat, a.proofDoc1, a.holdNume1, a.holdDeno2, a.adjustType, " +
    					" (select g.codeName from SYSCA_Code g where 1=1 and a.registerCause=g.codeID(+) and g.codeKindID(+) = 'RCA') as registerCause, " +
    					" a.registerDate, nvl(a.oldBookValue,0) as oldBookValue, nvl(a.adjustBookValue,0) as adjustBookValue, nvl(a.newBookValue,0) as newBookValue, a.setPeriod, a.commonObligee, a.setPerson, a.payDate, a.interest, a.rent, a.notes1, " +
    					" (select h.codeName from SYSCA_Code h where 1=1 and a.cause=h.codeID(+) and h.codeKindID(+) = 'CAE') as causeName, a.cause1, a.cause, " +
    					" a.summonsNo " +
    					" from UNTRT_AdjustProof a,SYSPK_PropertyCode b, SYSCA_Organ c,SYSCA_Code d "+
    	...
    	if("4".equals(q_kind)){
            ...
            while(rs.next()){
                ...
                data[17] = rs.getString("registerCause");
               	data[18] = Common.MaskDate(rs.getString("registerDate"));
                data[19] = new Integer(rs.getString("oldBookValue"));
                if(rs.getString("adjustType").equals("2")){
                    data[20] = "減  "+Common.valueFormat(rs.getString("adjustBookValue"));
                }else{
                    data[20] = "增  "+Common.valueFormat(rs.getString("adjustBookValue"));
                }
                data[21] = new Integer(rs.getString("newBookValue"));
                data[22] = rs.getString("setPeriod");
                data[23] = rs.getString("commonObligee");
                data[24] = rs.getString("setPerson");
               	data[25] = Common.MaskDate(rs.getString("payDate"));
                data[26] = new Integer(rs.getString("interest"));
                data[27] = rs.getString("rent");
                data[28] = rs.getString("notes1");
                data[29] = new JRTableModelDataSource(getSubModel((String)data[0]));            
                data[30] = Common.areaFormat(obj.getSumHoldArea())+"　";
                data[31] = Common.areaFormat(obj.getSumSetArea())+"　";
                data[32] = new Integer(obj.getSumOldBookValue());
                if(Integer.parseInt(obj.getSumAdjustBookValue())<0){
                	data[33] = "減  "+Common.valueFormat((Integer.parseInt(obj.getSumAdjustBookValue())*-1)+"");
                }else{
                	data[33] = "增  "+Common.valueFormat(obj.getSumAdjustBookValue());
                }
                data[34] = new Integer(obj.getSumNewBookValue());
                if(rs.getString("cause").equals("99")){
                    data[35] = rs.getString("cause1");
                }else{
                    data[35] = rs.getString("causeName");
                }
                data[36] = kindName[k-1];
                data[37] = rs.getString("caseNo")+j;
                data[38] = Common.get(rs.getString("summonsNo"))+"　";
...
public DefaultTableModel getSubModel(String caseCode) throws Exception{
	...
	if(!(obj.getCaseNo()).equals(obj.getOldCaseNo())){
		...
		setAreaSum=0.00;
		oldBookValueSum=0;
		adjustBookValueSum=0;
		newBookValueSum=0;
		adjustBookValue=0;
		
		obj.setSumHoldArea(holdAreaSum+"");
		obj.setSumSetArea(setAreaSum+"");
		obj.setSumOldBookValue(oldBookValueSum+"");
		obj.setSumAdjustBookValue(adjustBookValueSum+"");
		obj.setSumNewBookValue(newBookValueSum+"");
	}
	obj.setOldCaseNo(obj.getCaseNo());
    
    try{
        String[] columns = new String[] { "SERIALNO1", "SIGNNO","SETOBLIGOR","HOLDAREA","SETAREA","OLDBOOKVALUE","ADJUSTBOOKVALUE","NEWBOOKVALUE"};
    	String execSQL="select a.serialNo1, b.signDesc as signName, a.signNo, a.setObligor, " +
    					" TO_CHAR(a.holdArea,'999999999.99') as holdArea, TO_CHAR(a.setArea,'999999999.99') as setArea, " +
    					" nvl(a.oldBookValue,0) as oldBookValue, nvl(a.adjustBookValue,0) as adjustBookValue, a.adjustType, nvl(a.newBookValue,0) as newBookValue " +
						" from UNTRT_AdjustDetail a, SYSCA_SIGN b "+
    	...
    	while(rs.next()){
    		...
    		data[3] = Common.areaFormat(rs.getString("holdArea"))+"　";
    		data[4] = Common.areaFormat(rs.getString("setArea"))+"　";
    		data[5] = new Integer(rs.getString("oldBookValue"));
    		if(rs.getString("adjustType").equals("2")){
    		    adjustBookValue = Integer.parseInt(rs.getString("adjustBookValue"))*-1;
    		    data[6] = "減  "+Common.valueFormat((adjustBookValue*-1)+"");
    		}else{
    		    adjustBookValue = Integer.parseInt(rs.getString("adjustBookValue"));
    		    data[6] = "增  "+Common.valueFormat(adjustBookValue+"");
    		}
    		data[7] = new Integer(rs.getString("newBookValue"));
			//合計
			holdAreaSum += rs.getDouble("holdArea");
			setAreaSum += rs.getDouble("setArea");
			oldBookValueSum += Integer.parseInt(rs.getString("oldBookValue"));
			adjustBookValueSum += adjustBookValue;
			newBookValueSum += Integer.parseInt(rs.getString("newBookValue"));
			obj.setSumHoldArea(holdAreaSum+"");
			obj.setSumSetArea(setAreaSum+"");
			obj.setSumOldBookValue(oldBookValueSum+"");
			obj.setSumAdjustBookValue(adjustBookValueSum+"");
