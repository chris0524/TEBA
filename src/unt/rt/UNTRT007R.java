...
public class UNTRT007R extends UNTRT005Q{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOF","CASENO","MANAGENO",
    									 "OWNERSHIP","PROPERTYKIND","FUNDTYPE","BUYDATE","PROPERTYNO","PROPERTYNAME",
    									 "PROPERTYNAME1","SOURCEKIND","MEAT","PROOFDOC1","HOLDNUME1_HOLDDENO2","REGISTERCAUSE",
    									 "REGISTERDATE","OLDBOOKVALUE","REDUCEBOOKVALUE","NEWBOOKVALUE","SETPERIOD","COMMONOBLIGEE",
    									 "SETPERSON","PAYDATE","INTEREST","RENT","NOTES1","subReportDataSource",
    									 "SUMAREA","SUMHOLDAREA","SUMSETAREA","SUMORIGINALBV","Q_KIND","caseNoPage","summonsNo"};
    	...
    	String execSQL="select a.enterOrg as shareEnterOrg, a.ownership as shareOwnership, a.propertyNo as sharePropertyNo, a.serialNo as shareSerialNo, " +
    					...
    					" a.meat, a.proofDoc1, a.holdNume1, a.holdDeno2, " +
    					" (select g.codeName from SYSCA_Code g where 1=1 and a.registerCause=g.codeID(+) and g.codeKindID(+) = 'RCA') as registerCause, " +
    					" a.registerDate, nvl(a.oldBookValue,0) as oldBookValue, nvl(a.reduceBookValue,0) as reduceBookValue, nvl(a.newBookValue,0) as newBookValue, a.setPeriod, a.commonObligee, a.setPerson, a.payDate, a.interest, a.rent, a.notes1, " +
    					" a.summonsNo " +
    					" from UNTRT_ReduceProof a,SYSPK_PropertyCode b, SYSCA_Organ c,SYSCA_Code d "+
...
//System.out.println("-- untrt007r getResultModel -- \n/"+execSQL);  
    	//j ,i ,k 為報表收執第幾聯之判斷
    	int j ,i ,k;
    	if("4".equals(q_kind)){
    		i = 3 ;
            ...
            while(rs.next()){
                ...
                data[9] = rs.getString("buyDate").substring(0,3)+"/"+rs.getString("buyDate").substring(3,5)+"/"+rs.getString("buyDate").substring(5,7);
                ...
                if(rs.getString("registerDate")!=null){
                	data[18] = rs.getString("registerDate").substring(0,3)+"/"+rs.getString("registerDate").substring(3,5)+"/"+rs.getString("registerDate").substring(5,7);
                }else{
                	data[18] = rs.getString("registerDate");
                }
                data[19] = new Integer(rs.getString("oldBookValue"));
                data[20] = new Integer(rs.getString("reduceBookValue"));
                ...
                if(rs.getString("payDate")!=null){
                	data[25] = rs.getString("payDate").substring(0,3)+"/"+rs.getString("payDate").substring(3,5)+"/"+rs.getString("payDate").substring(5,7);
                }else{
                	data[25] = rs.getString("payDate");
                }
                ...
                data[30] = Common.areaFormat(obj.getSumArea())+"　";
                data[31] = Common.areaFormat(obj.getSumHoldArea())+"　";
                data[32] = Common.areaFormat(obj.getSumSetArea())+"　";
                data[33] = new Integer(obj.getSumOriginalBV());
                
                data[34] = kindName[k-1];
                data[35] = rs.getString("caseNo")+j;
                data[36] = Common.get(rs.getString("summonsNo"))+"　";
...
public DefaultTableModel getSubModel(String caseCode) throws Exception{
    ...
    Database db = new Database();
	//合計價值換頁的判斷 
//System.out.println(obj.getCaseNo() +" -- "+obj.getOldCaseNo());
    ...
    try{
    	...
    	String execSQL="select a.serialNo1, b.signDesc as signName, a.signNo, a.setObligor, a.area, a.holdNume, " +
    					" a.holdDeno, TO_CHAR(a.holdArea,'999999999.99') as holdArea, TO_CHAR(a.setArea,'999999999.99') as setArea, a.bookValue " +
						" from UNTRT_ReduceDetail a, SYSCA_SIGN b "+
...
//System.out.println("-- untrt007r getSubModel -- \n/"+execSQL); 
    	...
    	while(rs.next()){
			...
			//合計
			areaSum += Double.parseDouble(rs.getString("area"));
			holdAreaSum += rs.getDouble("holdArea");
			setAreaSum += rs.getDouble("setArea");
			originalBVSum += Integer.parseInt(rs.getString("bookValue"));
			obj.setSumArea(areaSum+"");
			obj.setSumHoldArea(holdAreaSum+"");
			obj.setSumSetArea(setAreaSum+"");
			obj.setSumOriginalBV(originalBVSum+"");
