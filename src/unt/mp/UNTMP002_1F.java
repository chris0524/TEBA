...
public class UNTMP002_1F extends UNTMP001Q{
...
//		System.out.println("xx="+getPicture());
	...
	private Map getRecordMap_forDetail(String newserialNo){
		Map map = new HashMap();
		UNTCH_Tools ut = new UNTCH_Tools();
		...
		map.put("NetValue", getDetailNetValue());
...
protected String getInsertUntmpMovableDetail(){
	UNTMP002_1F obj = this;
	StringBuffer sbSQL = new StringBuffer("");
	int counter=0;
//	if(Integer.valueOf(trueOriginalUnit)==0){
...
protected String[] getUpdateSQL(){
	...
	String upValueSQL[]=null;				//shan
	int deleteMovalbeDetail = 0;
	int movableDetail = 0;
	trueOriginalAmount = originalAmount==null || "".equals(originalAmount)?"0":originalAmount;
	trueOriginalUnit = originalUnit==null || "".equals(originalUnit)?"0":originalUnit;
	trueOriginalBV = originalBV==null || "".equals(originalBV)?"0":originalBV;
	...
	if(Double.parseDouble(trueOriginalAmount) != Double.parseDouble(checkOriginalAmount)){
		String amount="1";
		if(Integer.parseInt(trueOriginalUnit)==0){
			amount = "1";
		}else{
		...
		serialNoSSQL = "(select RIGHT('0000000' + CAST(min(serialno)+1 as varchar),7) from UNTMP_MOVABLEDETAIL where enterorg = " + Common.sqlChar(enterOrg) + "and ownership = " + Common.sqlChar(ownership) + "and differencekind = " + Common.sqlChar(differenceKind) + "and propertyno = " + Common.sqlChar(propertyNo) + "and lotno = " + Common.sqlChar(lotNo) + ")";
		serialNoESQL = "(select RIGHT('0000000' + CAST(max(serialno)+1 as varchar),7) from UNTMP_MOVABLEDETAIL where enterorg = " + Common.sqlChar(enterOrg) + "and ownership = " + Common.sqlChar(ownership) + "and differencekind = " + Common.sqlChar(differenceKind) + "and propertyno = " + Common.sqlChar(propertyNo) + "and lotno = " + Common.sqlChar(lotNo) + ")";
		
		DBServerTools dbt = new DBServerTools();
		dbt._setDatabase(new Database());
		setSerialNoS(dbt._execSQL_asString(serialNoSSQL));
		
		dbt._setDatabase(new Database());
		setSerialNoE(dbt._execSQL_asString(serialNoESQL));
	...
	deleteMovalbeDetail = deleteMovalbeDetailSQL==null?0:deleteMovalbeDetailSQL.length;
	movableDetail = movableDetailSQL==null?0:movableDetailSQL.length;
//	String[] execSQLArray = new String[deleteMovalbeDetail+movableDetail+3];
	String[] execSQLArray = new String[deleteMovalbeDetail+movableDetail+3];

	for(int i=0;i<deleteMovalbeDetail;i++){
		execSQLArray[i] = deleteMovalbeDetailSQL[i];
	}
	for(int i=3;i<movableDetail+3;i++){
		execSQLArray[i] = movableDetailSQL[i-3];
	}
	

	execSQLArray[deleteMovalbeDetail] = new SQLCreator()._obtainSQLforUpdate("UNTMP_MOVABLE", getPKMap(), getRecordMap());
	
	
	String upBookvalue = "";
	if( (Integer.parseInt(trueOriginalUnit) != Integer.parseInt(checkOriginalUnit) || Integer.parseInt(trueOriginalBV) != Integer.parseInt(checkOriginalBV))){
		if ("".equals(originalUnit) || originalUnit == null ){
			upBookvalue = ", bookValue = "+ Common.sqlChar(originalBV);
		}else{
			upBookvalue = ", bookValue = "+ Common.sqlChar(originalUnit);
		}
	}else{
		//execSQLArray[deleteMovalbeDetail+2]="select 1";
	}
	Database db=new Database();
	try{
...
public UNTMP002_1F  queryOne() throws Exception{
	Database db = new Database();
	UNTMP002_1F obj = this;
	try {
		String sql=" select b.organSName as enterOrgName, " +"\n"+
					...
					" case a.otherLimitYear when null then 0 else a.otherLimitYear end as otherLimitYear," +
					" a.SourceDate, a.BuyDate, a.PropertyKind, a.FundType, a.Valuable, a.CaseSerialNo," +
					" a.SerialNoS, a.SerialNoE, a.OtherPropertyUnit, a.OtherMaterial, a.PropertyName1," +
					...
					" a.EscrowOriAccumDepr, a.OldSerialNoS, a.OldSerialNoE, a.Cause, a.Cause1, a.picture, a.caseno as caseno2," +
					" a.originalapportionendym as originalapportionendym1 " +
					...
					",a.bookamount as a_bookamount ,a.bookvalue as a_bookvalue ,a.netvalue as a_netvalue" +
					",a.notes as a_notes" +
					",d.*" +
...
//System.out.println("-- untmp002f queryOne -- "+sql);
		...
		if (rs.next()){
			...
			obj.setNotes(rs.getString("a_notes"));
			...
			obj.setBookAmount(rs.getString("a_bookamount"));
			obj.setBookValue(rs.getString("a_bookvalue"));
			obj.setNetValue(rs.getString("a_netvalue"));
