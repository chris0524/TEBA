...
public class UNTMP038R extends SuperBean{
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	...
    	String[] columns = new String[] {"OWNERSHIP","ENTERORGNAME","CASENO","PROPERTYNO","PROPERTYNAME","BUYDATE","LIMITYEAR","USEYEAR","OLDBOOKUNIT","ADJUSTBOOKAMOUNT","ADJUSTBOOKVALUE","CAUSE2","SCRAPVALUE2","DEALSUGGESTION"};
    	String execSQL="select c.organAName as enterOrgName," +  //,"SIGN"
    	...
    	execSQL1=execSQL1+" and b.cause != '01' ";
    	...
    	//int i;
    	ResultSet rs3 = db.querySQL(execSQL2);
    	while(rs3.next()){
    		amount3=rs3.getString("sum(adjustBookAmount)");
    		amount3=Common.valueFormat(amount3);
    		value3=rs3.getString("sum(adjustBookValue)");
    		value3=Common.valueFormat(value3);
    	}
    	ResultSet rs4 = db.querySQL(execSQL3);
    	while(rs4.next()){
    		amount4=rs4.getString("sum(adjustBookAmount)");
    		amount4=Common.valueFormat(amount4);
    		value4=rs4.getString("sum(adjustBookValue)");
    		value4=Common.valueFormat(value4);
    	}
    	ResultSet rs5 = db.querySQL(execSQL4);
    	while(rs5.next()){
    		amount5=rs5.getString("sum(adjustBookAmount)");
    		amount5=Common.valueFormat(amount5);
    		value5=rs5.getString("sum(adjustBookValue)");
    		value5=Common.valueFormat(value5);
    	}
        ...
        while(rs.next()){
            ...
            //data[14] = "製表       使用單位(人員)            財產管理單位(人員)           會計單位(人員)           機關首長";
