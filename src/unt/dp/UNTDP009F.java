...
public class UNTDP009F extends SuperBean{
...
public ArrayList queryAll() throws Exception{
	...
	try {
		...
		"where (a.nodeprset is null or a.nodeprset <> 'Y')  ";
		...
		"and a.lotno = b.lotno "+
		"and c.enterorg in (a.enterorg,'000000000A')"+
		"and a.propertyno = c.propertyno "+
		"and (b.nodeprset is null or b.nodeprset <> 'Y')  ";
		...
		sql += " select a.enterorg,  a.ownership, (select codename from SYSCA_CODE z where z.codekindid='OWA' and z.codeid=a.ownership) as ownershipName"
		     + ",a.differencekind, (select codename from SYSCA_CODE z where z.codekindid = 'DFK' and z.codeid = a.differencekind) as differenceKindName"
		     + ", a.buydate, a.propertyno, a.serialno,  a.propertyname1, '' as nameplate, '' as specification, '1' as bookamount, a.bookvalue, a.keepunit"
		     + ", (select unitname from UNTMP_KEEPUNIT d where d.enterorg=a.enterorg and d.unitno=a.keepunit )  as unitname, a.place1"
		     + ", (select placename from SYSCA_PLACE e where e.enterorg=a.enterorg and e.placeno=a.place1 )  as placename, c.propertyname"
		     + ", ISNULL(a.otherpropertyunit,c.propertyunit) as propertyunit, ISNULL(a.otherlimityear, c.limityear) as limityear ";
		sql += " from UNTRT_ADDPROOF a  ";
		sql += " left join SYSPK_PROPERTYCODE c on  a.propertyno = c.propertyno and c.propertyno in ( a.propertyno, '000000000A')  ";
		...
		while (rs.next()){
			...
			rowArray[1]=Common.getORGANASNNAME(getQ_enterOrg());
...
public void update(){
	...
	if(getsKeySet()!=null){
		...
		for (int i=0; i<getcut; i++) {	//將挑選的資料組成一字串
			...
			if(tempPropertyNo.length()>=3 && tempPropertyNo.substring(0,3).equals("111")){
				sqlcode[i] = " update UNTRF_ATTACHMENT set nodeprset='Y' where enterorg= "+ Common.sqlChar(strKeys[0])+" and ownership= "+ Common.sqlChar(strKeys[1])+
			...
			if(tempPropertyNo.length()>=1 && tempPropertyNo.substring(0,1).equals("2")){
				sqlcode[i] = " update UNTBU_BUILDING set nodeprset='Y' where enterorg= "+ Common.sqlChar(strKeys[0])+" and ownership= "+ Common.sqlChar(strKeys[1])+
				 " and differencekind= "+ Common.sqlChar(strKeys[2])+ " and propertyno= "+ Common.sqlChar(strKeys[3])+ " and serialno= "+ Common.sqlChar(strKeys[4]);
			}
			
			if(tempPropertyNo.length()>=1 && ( tempPropertyNo.substring(0,1).equals("3") || tempPropertyNo.substring(0,1).equals("4") || tempPropertyNo.substring(0,1).equals("5") ) ){
				sqlcode[i] = " update UNTMP_MOVABLEDETAIL set nodeprset='Y' where enterorg= "+ Common.sqlChar(strKeys[0])+" and ownership= "+ Common.sqlChar(strKeys[1])+
				 " and differencekind= "+ Common.sqlChar(strKeys[2])+ " and propertyno= "+ Common.sqlChar(strKeys[3])+ " and serialno= "+ Common.sqlChar(strKeys[4]);
			}
			
			if (tempPropertyNo.length()>=1 && tempPropertyNo.substring(0,1).equals("8")) {
				sqlcode[i] = " update UNTRT_ADDPROOF set nodeprset='Y' where enterorg= " + Common.sqlChar(strKeys[0]) + " and ownership= " + Common.sqlChar(strKeys[1]) +
						 	 " and differencekind= "+ Common.sqlChar(strKeys[2])+ " and propertyno= "+ Common.sqlChar(strKeys[3])+ " and serialno= "+ Common.sqlChar(strKeys[4]);
			}
		}
