 ...
 * <br/>--------------------------------------------------------
 * <br/>修改作者　　     修改日期　　　			修改目的
 * <br/>問題單2371 中科提出專利權獨立設置轉帳憑證黏存單的功能
 * <br/>--------------------------------------------------------
 */
public class UNTDP027R extends SuperBean {
	...
	private String getSQL() {
		...
		sql.append(" select a.enterorg, a.deprym, a.differencekind,")
		   ...
		   .append(" when '8' then '160101 權利'")
		   .append(" else null end) as propertytype1");
		
		//依照折舊月檔中的園區別代碼至園區別代碼檔查詢中文顯示
	...
	private String getF3Val(String deprYM) {
		if (!"".equals(deprYM) && deprYM.length() == 6) {
			return Datetime.changeTaiwanYYMM(deprYM, "1").substring(0, 3) + "年" + Datetime.changeTaiwanYYMM(deprYM, "1").substring(3, 5) + "月份公務無形資產攤銷";
		} else {
			return deprYM;
		}
	}
			...
			if (rows != null && rows.size() > 0) {
				if("1".equals(Common.get(getQ_changePage()))) {
					...
					for(int i =0; i < indexsArray.length; i++ ){
						rows.get((Integer)indexsArray[i])[7] = "510803 無形資產攤銷";
						rows.get((Integer)indexsArray[i])[9] = Common.moneyFormat((Integer)sumArray[i]);
					}
				}else {
					rows.get(0)[7] = "510803 無形資產攤銷";
