...
public class UNTDP017R extends SuperBean{
	...
	public DefaultTableModel getResultModel() throws Exception{
	    ...
	    try{
		    ...
		    if(!"".equals(getQ_deprYM())){
		    	...
		    	head01 = getQ_deprYM().substring(0, 3) + "年" + getQ_deprYM().substring(3,5) + "月  財產折舊表(基金財產)";
		    }
	    	...
	    	if("1".equals(getQ_reportType())){
		    	...
		    	stb.append(" select distinct '***' from SYSCA_ORGAN ");
		        stb.append(" union all ");
		    	stb.append(" select distinct " + Common.sqlChar("報表編號：untdp017r-1")+ " from SYSCA_ORGAN ");
		        ...
		        stb.append(" union all ");
		    	stb.append(" select distinct " + Common.sqlChar("報表編號：untdp017r-2")+ " from SYSCA_ORGAN ");
		        ...
		        stb.append(" union all ");
		    	stb.append(" select distinct " + Common.sqlChar("報表編號：untdp017r-3")+ " from SYSCA_ORGAN ");
		    	...
		    	stb.append(" select '部門別：'+z.deprunitname from SYSCA_DEPRUNIT z where z.enterorg = " + Common.sqlChar(Common.get(getQ_enterOrg())) + " and z.deprunitno = "+ Common.sqlChar(Common.get(getQ_deprUnit())));
		        stb.append(" union all ");
		    	stb.append(" select distinct " + Common.sqlChar("報表編號：untdp017r-4")+ " from SYSCA_ORGAN ");
	    	} else {
	    		stb.append(" union all ");
		    	stb.append(" select distinct '***' from SYSCA_ORGAN ");
		    	stb.append(" union all ");
	    	...
	    	.append(" union all ")
	    	.append(" select case when sum(a.bookvalue) is null then  '0' else convert(nvarchar(10), sum(a.bookvalue)) end from " + queryLastMonthTableName + " a where a.differencekind = '120' and a.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") + " and a.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and a.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ")  // A
	    	.append(" union all ")
			...
			.append(" where b.differencekind = '120' and b.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and b.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and b.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) + " and k.adjustdate=" + Common.sqlChar(Common.get(dbThisMonth)) + " and l.reducedate="+ Common.sqlChar(Common.get(dbThisMonth)) +" ") // B
	    	.append(" union all ")
	    	...
	    	.append(" where c.differencekind = '120'  and c.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and c.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and c.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) + " and k.adjustdate=" + Common.sqlChar(Common.get(dbThisMonth)) + " and l.reducedate="+ Common.sqlChar(Common.get(dbThisMonth)) +" ")  // C
	    	.append(" union all ")
//	    	.append(" select case when sum(d.bookvalue) is null then  '0' else convert(nvarchar(10), sum(d.bookvalue)) end from " + queryThisMonthTableName + " d where d.differencekind = '120' and d.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and d.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and d.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ")  // D=A+B-C
	    	...
	    	if(i == 1){
	    		...
	    		stb.append(" select case when sum(e.accumdepr) is null then  '0' else convert(nvarchar(10), sum(e.accumdepr)) end from " + queryLastMonthTableName + " e where e.differencekind = '120'  and e.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and e.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and e.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ");  // E
	    	}
	    	stb.append(" union all ");
	    	if(i == 1){
	    	    stb.append(" select distinct case when count(*) = 0 then  '0' else '0' end from UNTDP_MONTHDEPR f  ");  // F
	    	}else{
	    		stb.append(" select convert(nvarchar(10),sum ( case when f.deprunitcb='Y' then  convert(nvarchar(10),(f.scaledaddaccumdepr  - f.scaledreduceaccumdepr)) when f.deprunitcb='N' then  convert(nvarchar(10),(f.addaccumdepr - f.reduceaccumdepr)) else 0 end )) from UNTDP_MONTHDEPR f where f.differencekind = '120' and f.propertytype=" + Common.sqlChar(String.valueOf(i)) + " and f.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and f.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" and f.deprym="+ Common.sqlChar(Common.get(dbThisMonth))+" ");  // F
	    	}
	    	stb.append(" union all ");
	    	if(i == 1){
	    	    stb.append(" select distinct case when count(*) = 0 then  '0' else '0' end from UNTDP_MONTHDEPR g  "); // G
	    	}else{
	    		stb.append(" select case when sum ( case when g.deprunitcb='Y' then  g.scaledmonthdepr when g.deprunitcb='N' then  g.monthdepr else 0 end ) is null then  '0' else convert(nvarchar(10), sum ( case when g.deprunitcb='Y' then  g.scaledmonthdepr when g.deprunitcb='N' then  g.monthdepr else 0 end )) end  from UNTDP_MONTHDEPR g where g.differencekind = '120' and g.propertytype=" + Common.sqlChar(String.valueOf(i)) + " and g.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and g.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" and g.deprym="+ Common.sqlChar(Common.get(dbThisMonth))+" "); // G
	    	}
	    	stb.append(" union all ");
//	    	if(i == 1){
//	    		stb.append(" select distinct '0' from " + queryThisMonthTableName + " h ");  // H=E+F+G
//	    	}else{
//	    		stb.append(" select case when sum(h.accumdepr) is null then  '0' else convert(nvarchar(10), sum(h.accumdepr)) end from " + queryThisMonthTableName + " h where h.differencekind = '120'  and h.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and h.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and h.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ");  // H=E+F+G
	    		stb.append(" select distinct 'H' from " + queryThisMonthTableName + " h ");
//	    	}
	    	stb.append(" union all ");
//	    	if(i == 1){
//	    		stb.append(" select case when sum(i.bookvalue) is null then  '0' else convert(nvarchar(10), sum(i.bookvalue)) end from " + queryThisMonthTableName + " i where i.differencekind = '120'  and i.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and i.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and i.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ");  // I=D-H
//	    	}else{
//	    		stb.append(" select case when sum(i.netvalue) is null then  '0' else convert(nvarchar(10), sum(i.netvalue)) end from " + queryThisMonthTableName + " i where i.differencekind = '120'  and i.propertyno like " + Common.sqlChar(String.valueOf(i)+"%") +  " and i.enterorg=" + Common.sqlChar(Common.get(getQ_enterOrg())) + " and i.ownership="+ Common.sqlChar(Common.get(getQ_ownership())) +" ");  // I=D-H	    	
		...
		for(int i=0; i< tableName.length; i++){
			for(int j=0; j< queryMonth.length; j++){
				try{
					...
					message += "目前尚未備存" + tableChineseName[i] + "之" +queryMonth[j]+"資料!!	";
				}
			}
		}
		return message;
