...
public class UNTDP013R extends SuperBean{
	...
	private String q_netvalueCheck; 	//淨值為0
	...
	public String getQ_netvalueCheck(){ return checkGet(q_netvalueCheck); }
	public void setQ_netvalueCheck(String s){ q_netvalueCheck=checkSet(s); }
	...
	public DefaultTableModel getResultModel() throws Exception{
		...
		try{
			if(!"".equals(getQ_enterOrg()) ||  !"".equals(getOrganID())){
				...
				columns = new String[]{
					...
					String execSQL="  select * from ( select distinct  " +
					" a.enterorg, " +
					" a.differencekind, " +
					" a.deprpark, " +
					" a.deprunit, " +
		...
		//			" d.codename as 'propertytype', " +
					...
					" a.buydate , " +
					" a.scrapvalue, " +
//					" (select x.limityear from SYSPK_PROPERTYCODE x where a.propertyno = x.propertyno and x.propertytype='1' and x.enterorg in('000000000A',a.enterorg)) as limityear, " +
					" limityear, " +
					" bookamount, " +
		...
		//			" inner join SYSCA_DEPRUNIT f on a.enterorg = f.enterorg and a.deprunit = f.deprunitno " +
					...
					if("N".equals(getQ_netvalueCheck())){ //不列印淨值為0
						execSQL += " and oldnetvalue <> 0 ";
					}
					execSQL += " ) as b order by ";
					if ("1".equals(getQ_page())) { //跳頁方式:部門別
						...
						" propertyno, " +
						" serialno" ;
					} else if ("4".equals(this.getQ_page())) { // 部門別單位 + 會科
						execSQL += " enterorg, " +
								   " differencekind, " +
								   " deprpark, " +
								   " deprunit, " +
		...
		//				data[4] = rs.getString("propertytype");
						...
						try {
							...
							data[17] = rs.getDouble("bookvalue") - rs.getDouble("newaccumdepr");
						} catch(Exception e) {
							e.printStackTrace();
						}
						...
						if (TCGHCommon.getSYSCODEName("02", "ETO").equals(this.getQ_enterOrg())) {
						...
						data[20] = checkGet(deprunit1Map.get(rs.getString("deprunit1")));
