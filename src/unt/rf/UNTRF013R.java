...
public class UNTRF013R extends SuperBean{
	String enterOrg;
	String ownership;
	String caseNoS;
	String caseNoE;
...
public DefaultTableModel getResultModel() throws Exception{
    ...
    try{
    	...
    	String[] columns = new String[] {"ENTERORGNAME","WRITEDATE","WRITEUNIT","PROOFNO","CASENO"
    									,"SOURCEDATE","PROPERTYNO","PROPERTYNAME","SIGNNO","PROPERTYUNIT"
    									,"MEASURE","BOOKVALUE","CAUSE","LIMITYEAR","USEMONTH","KIND"
    									,"summonsNo","propertyKind","propertyKindName","ownershipTitle","propertyName1"};
    	
    	String execSQL = " select c.organAName as enterOrgName ,a.writeDate ,a.writeUnit ,a.proofDoc||'字第'||a.proofNo||'號' as proofNo " + "\n" 
    				   + "		 ,a.caseNO ,b.sourceDate,substr(b.propertyNo,0,7)||'-'||substr(b.propertyNo,8,3)||'-'||b.serialNo as propertyNo " + "\n" 
    				   + "		 ,d.propertyName " + "\n" 
    				   + "		 ,nvl(((select  k.signDesc||substr(h.signNo,8,4)||'-'||substr(h.signNo,12,4)||'地號'from UNTRF_Base h,SYSCA_Sign k where h.enterOrg=b.enterOrg and h.ownership=b.ownership and h.propertyNo=b.propertyNo and h.serialNo=b.serialNo and substr(h.signNo,1,7)=k.signNo and rownum=1 )),'') as signLevel " + "\n" 
    				   + "		 ,(select count(*) from UNTRF_Base x where x.enterOrg=b.enterOrg and x.ownership=b.ownership and x.propertyNo=b.propertyNo and x.serialNo=b.serialNo) signSum " + "\n" 
    				   + "		 ,d.propertyUnit,b.measure,b.bookValue,(select f.codename from SYSCA_CODE f where f.codekindid = 'CAC' and b.cause=f.codeid)cause,b.cause1,nvl(d.limitYear,b.otherLimitYear)limitYear,b.useYear||'年'||b.useMonth||'個月' as useMonth " + "\n" 
    				   + "		 ,a.summonsNo " + "\n" 
    	...
    	//System.out.println("== UNTRF013R =="+execSQL);
...
//                data[8] = rs.getString("signLevel") + "等" +rs.getString("signSum") +"筆";
//                if("等0筆".equals(data[8]))data[8]="";
//                if("1".equals(rs.getString("signSum"))){
//                	data[8] = rs.getString("signLevel");
//                }
                int t = Integer.parseInt(rs.getString("signSum"));
                
                if(t>1){          	           	               	 
               		 data[8] = rs.getString("signLevel") + "等" + rs.getString("signSum") + "筆";
               }else{
               		data[8] = Common.get(rs.getString("signLevel"));
               }
                ...
                if("其他".equals(data[12])||data[12]==null)data[12]=causetemp;
                data[13] = rs.getString("LimitYear");
                data[14] = rs.getString("useMonth");
                ...
                data[19] = Common.get(rs.getString("ownershipName"))+"財產減損單";
                data[20] = Common.get(rs.getString("propertyName1"));
