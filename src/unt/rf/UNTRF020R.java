...
public class UNTRF020R extends UNTRF001F_Q{
	...
	public UNTRF020R(){	
		setFileName("UNTRF020R");				//設定輸出EXCEL的檔名
		setTableName("QRY_UNTRF020R");			//從 fieldMapping 這張 Table 取出 tableName = getTableName() 的資料
	...
	private String getExportAllSQL(){
		String querySql=" select "+
				...
				" a.ownership, a.propertyKind, a.fundType, a.taxCredit, a.fundsSource, a.sourceKind, a.valuable,"+
				...
				" (case a.verify when 'Y' then '是' else '否' end) as verify, a.propertyNo, a.serialNo,"+
				" (select z.propertyName from SYSPK_PropertyCode z where a.propertyno=z.propertyno) as propertyName,"+
				" a.propertyName1,"+
				" (select z.material from SYSPK_PropertyCode z where a.propertyno=z.propertyno) as material,"+
				" a.buildDate,"+
				...
				" a.sourceDate,"+
				" a.sourceDoc, (select organaname from SYSCA_Organ where 1=1 and organid=a.useUnit) as useUnit1, a.measure, "+
				" (select z.propertyUnit from SYSPK_PropertyCode z where a.propertyno=z.propertyno) as propertyUnit,"+
				" a.bookValue,a.buydate,a.otherlimityear,"+
				" (Convert(varchar(4),DATEDIFF(M, a.buydate, getdate()) / 12) + '年' + convert(varchar(2),DATEDIFF(M, a.buydate, getdate()) % 12)) + '個月' AS 'useym'," +
				" b.serialNo1 as serialNo1_Base, b.area as area_Base, b.owner as owner_Base, e.organSName as manageOrg_Base,"+
				" c.serialNo1 as serialNo1_Manage, " +
				" (select codeName from SYSCA_Code where codeKindID='URE' and codeID=c.useRelation) as useRelation_Manage, " +
				" (select organaname from SYSCA_ORGAN where 1=1 and organid=c.useUnit) as useUnit_Manage, c.useDateS as useDateS_Manage, c.useDateE as useDateE_Manage,"+
				" c.measure as measure_Manage, "+
				" (d.signdesc+SubString(b.signno,8,4)+'-'+SubString(b.signno,1,12)) signNo_Base,"+
				" a.differencekind"
				...
				+ ",a.usernote"
				+ ",a.place1"
				+ ",a.place,a.engineeringno,a.oldpropertyno,oldserialno"+
				...
				",a.netvalue, a.depramount, a.accumdepr, a.monthdepr, a.monthdepr1, a.notes"+
				" from UNTRF_ATTACHMENT a"+
				" left join UNTRF_BASE b on  a.enterorg=b.enterorg and a.ownership=b.ownership and a.differencekind=b.differencekind and  a.propertyno=b.propertyno and a.serialno=b.serialno"+
				" left join UNTRF_MANAGE c on a.enterorg=c.enterorg and a.ownership=c.ownership and a.differencekind=c.differenceKind and  a.propertyno=c.propertyno and a.serialno=c.serialno"+
				" left join SYSCA_SIGN d on substring(b.signNo,1,7)=d.signNo"+
				" left join SYSCA_ORGAN e on b.manageOrg=e.organID"+
				" where 1=1"+
