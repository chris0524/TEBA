...
public class UNTRF008F extends UNTRF008Q{
	...
	private Map getRecordMap(){
		...
		if(this.checkProofyearChanged(getEnterOrg(), getCaseNo(), getProofYear())){
			proofno = MaxClosingYM.getMaxProofNo("UNTRF_REDUCEPROOF",enterOrg,ownership,this.getProofYear());
		}
...
private boolean checkProofyearChanged(String enterorg, String caseno, String newproofyear){
	...
	try{
		String sql = "select proofyear from UNTRF_REDUCEPROOF where enterorg=" + Common.sqlChar(enterorg) +
...
protected String[] getInsertSQL(){
	setProofNo(MaxClosingYM.getMaxProofNo("UNTRF_ReduceProof",enterOrg,ownership,this.getProofYear()));
	String[] execSQLArray = new String[1];
	execSQLArray[0] = new SQLCreator()._obtainSQLforInsert("UNTRF_ReduceProof", getPKMap(), getRecordMap());
	return execSQLArray;
}
...
protected String[] getUpdateSQL(){
	...
	execSQLArray[0] = new SQLCreator()._obtainSQLforUpdate("UNTRF_ReduceProof", getPKMap(), getRecordMap());
...
protected String getDBVerify(){
	...
	String sql = "select verify from UNTRF_ReduceProof "
							+ "where 1=1 "
	...
	private String updateTable(){
		...
		if(getReVerify().equals("Y")){
			sql += " update UNTRF_ReduceDetail set" +
			...
			try{
				...
				while(rs.next()){
					sql += " update UNTRF_ATTACHMENT set" +
				        	  " measure = " + Common.sqlChar(rs.getString("measure")) + "," +
...
protected String[] getDeleteSQL(){ 
		String[] execSQLArray = new String[2];
		//刪除 UNTRF_ReduceProof TABLE
		execSQLArray[0] = new SQLCreator()._obtainSQLforDelete("UNTRF_ReduceProof", getPKMap(), getRecordMap());
				
		//刪除 UNTRF_ReduceDetail TABLE
		execSQLArray[1]=" delete UNTRF_ReduceDetail where 1=1 " +
				" and enterOrg = " + Common.sqlChar(enterOrg) +
				" and ownership = " + Common.sqlChar(ownership) +
				" and caseNo = " + Common.sqlChar(caseNo) +
				" and differenceKind = " + Common.sqlChar(differenceKind) + 
...
public UNTRF008F  queryOne() throws Exception{   
	Database db = new Database();
	UNTCH_Tools ut = new UNTCH_Tools();
	UNTRF008F obj = this;	
	try {    
		String sql=" select a.enterOrg, b.organSName, a.ownership, a.caseNo, a.caseName, a.writeDate, a.writeUnit, a.proofDoc, a.proofNo, a.manageNo, a.summonsNo,a.unusableCause,a.demolishDate, a.reduceDate, a.approveOrg, a.approveDate, a.approveDoc, a.verify, a.notes, a.editID, a.editDate, a.editTime, a.editID, a.editDate, a.editTime, a.closing, a.summonsDate, a.cause, "+
			...
			" from UNTRF_ReduceProof a, SYSCA_Organ b where 1=1 " +
...
//System.out.println("-- untrf008f queryOne --\n"+sql);
...
public void approveRe()throws Exception{	
	...
	try {
		String[] execSQLArray;
		String strSQL = "";
		String sql ="select a.caseNo, b.enterOrg, b.ownership, b.propertyNo, b.serialNo, b.measure, b.bookValue," +
		...
		while (rs.next()){
			...
			if(verify.equals("Y") && reduceCount.equals("0") && adjustCount.equals("0") && reduceMax.equals("0") && adjustMax.equals("0")){				
				...
				strSQL += "update UNTRF_ReduceProof set verify ='N' " +
						" where enterOrg=" + Common.sqlChar(enterOrgQuery) +
						" and ownership=" + Common.sqlChar(ownershipQuery) +
						" and caseNo=" + Common.sqlChar(caseNoQuery) +
						":;:";				
				strSQL += "update UNTRF_ReduceDetail set verify ='N' " +
						" where enterOrg=" + Common.sqlChar(enterOrgQuery) +
						" and ownership=" + Common.sqlChar(ownershipQuery) +
						" and caseNo=" + Common.sqlChar(caseNoQuery) +
						":;:";							
