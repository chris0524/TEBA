...
<%@ page import="util.User, filter.SSOFilter"%>
<%
String account = util.Common.checkGet(request.getParameter("userID")).equals("")?"":util.Common.checkGet(request.getParameter("userID")).replaceAll("'","").replaceAll("\"","");
String pwd = util.Common.checkGet(request.getParameter("password")).equals("")?"":util.Common.checkGet(request.getParameter("password")).replaceAll("'","").replaceAll("\"","");
String organid = util.Common.checkGet(request.getParameter("organid")).equals("")?"":util.Common.checkGet(request.getParameter("organid")).replaceAll("'","").replaceAll("\"","");
...
if (!"".equals(account) && !"".equals(pwd) ){
	...
	boolean retrying = util.LoginUtil.isUserRetrying(account, request.getRemoteAddr(),retryCount);
	if(!retrying){
	    user = util.User.getVerifiedUser(account, pwd,organid);
	}
	if (user != null) {
		//取得使用者權限
		//Permission permission[] = new Permission[]{new Permission("sysap001f","M"),new Permission("sysap002f","M")};
		session.setAttribute("editID", user.getUserID());
		SYSAP003F_Permission[] permission = user.getPermission();  // modify by Timtoy.Tsai 20100125
		session.setAttribute("CurrentPermission",permission);      // modify by Timtoy.Tsai 20100125
		session.setAttribute("user",user);
		session.setAttribute(SSOFilter.SESSION_SSO_TOKEN1, null);//非SSO登入方式需清除token1資訊以免header.jsp判斷成SSO登入
		response.sendRedirect("home/frame.jsp");
	} else if(retrying){
	    script = "alert('因為重覆輸入錯誤密碼,這個帳號已被停用"+retryTimeout+"分鐘!');";
	} else {
	    util.LoginUtil.markUserRetrying(account,request.getRemoteAddr(),retryCount,retryTimeout);
	    script = "alert('帳號或密碼錯誤, 請重新登入!');";
	}
} else {
	session.removeAttribute("editID");
	//session.removeAttribute("CurrentPermission");
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" id="table6">
      ...
      <tr>
		...
		<div align="center">
            ...
            <td width="45" align="center" valign="middle">機關</td>
            <td align="left" valign="middle">
			<select class="field" type="select" name="organid">
					<%=util.View.getOption("select  organid, organsname from SYSCA_ORGAN where ismanager = 'Y' order by organid", "")%>
				</select></td>
          </tr>
          ...
          <tr>
            ...
            <td align="left" valign="middle">
			<input type="text" class="field" tabindex="1" maxlength="20" size="15" name="userID" value="" /></td>
          </tr>
          <tr>
            <td width="20" align="right" valign="middle">&nbsp;</td>
            <td width="45" align="center" valign="middle">密碼</td>
            <td height="32" align="left" valign="middle">
			<input tabindex="2" class="field" type="password" maxlength="20" size="15" value="" name="password" /></td>
          </tr>
        </table>
		</div>
		</td>
...
<table width="571" border="0" align="center" cellpadding="0" cellspacing="0" id="table8">
  <tr>
    ...
    <td>
	<a onMouseOut="MM_swapImgRestore();" onMouseOver="MM_swapImage('login_r4_c6','','image/login_r4_c6_f2.jpg',1);" onClick="form1.submit();"><input type="image" name="login_r4_c6" src="image/login_r4_c6.jpg" width="221" height="38" border="0" id="login_r4_c6" alt="登入財產管理資訊系統" /></a></td>
    <td>
