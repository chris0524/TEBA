...
<%
	...
	if("PN".equals(Common.get(sType))) {
		...
		signNo4 = Common.checkGet(Common.formatFrontZero(request.getParameter("signNo4"),5));
		signNo5 = Common.checkGet(Common.formatFrontZero(request.getParameter("signNo5"),3));         
	}
	...
	String sSQL = " select a.enterorg ,o.organsname " + "\n"
		 		...
		 		+ "        ,substr(a.signno,8,5) as signno4 ,substr(a.signno,13,3)as signno5 " + "\n"
		 		+ "        ,a.enterDate " + "\n"
		 		+ " from untbu_building a ,sysca_organ o ,syspk_propertycode p " + "\n"
		 		+ " where 1=1 " + "\n"
...
StringBuffer strXML = new StringBuffer(1000);
	...
	try {		
		...
		while (rs.next()){
			...
			strXML.append(" propertyNo=\"").append(Common.checkGet(rs.getString("propertyNo"))).append( "\"");
			strXML.append(" propertyNoName=\"").append(Common.checkGet(rs.getString("propertyNoName"))).append( "\"");
			strXML.append(" serialNo=\"").append(Common.checkGet(rs.getString("serialNo"))).append( "\"");
			
			strXML.append(" signNo1=\"").append(Common.checkGet(rs.getString("signNo1"))).append( "\"");
			strXML.append(" signNo2=\"").append(Common.checkGet(rs.getString("signNo2"))).append( "\"");
			strXML.append(" signNo3=\"").append(Common.checkGet(rs.getString("signNo3"))).append( "\"");
			strXML.append(" signNo4=\"").append(Common.checkGet(rs.getString("signNo4"))).append( "\"");
			strXML.append(" signNo5=\"").append(Common.checkGet(rs.getString("signNo5"))).append( "\"");
