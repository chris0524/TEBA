...
if (Common.get(preWord).length()>0) {
	...
	if (!"".equals(transToCE_Year(Common.get(q_bulletinDate),5)))	
		sql += " and bulletinDate = " + Common.sqlChar(transToCE_Year(Common.esapi(q_bulletinDate),5));
	if (!"".equals(transToCE_Year(Common.get(Common.esapi(q_suiteDateS)),7)))
		sql += " and suitDateS>="+Common.sqlChar(transToCE_Year(Common.esapi(q_suiteDateS),7));
	if (!"".equals(transToCE_Year(Common.get(Common.esapi(q_suiteDateE)),7)))
		sql += " and suitDateE<="+ Common.sqlChar(transToCE_Year(Common.esapi(q_suiteDateE),7));
	ResultSet rs = db.querySQL(sql+" order by bulletindate desc");	
	
	while (rs.next()) {
		StringBuffer strLink = new StringBuffer(0).append(Common.sqlChar(transToROC_Year(Common.checkGet(rs.getString("bulletindate")),6)) ).append( "," ).append(
			Common.sqlChar(transToROC_Year(Common.checkGet(rs.getString("suitdates")),7)) ).append( "," ).append(
			Common.sqlChar(transToROC_Year(Common.checkGet(rs.getString("suitdatee")),7)));				
		...
		sbHTML.append(" <td class='listTD' >").append(getDotDate(transToROC_Year(Common.checkGet(rs.getString("bulletindate")),6))).append("</td> ");
		sbHTML.append(" <td class='listTD' >").append(getDotDate(transToROC_Year(Common.checkGet(rs.getString("suitdates")),8)));
		sbHTML.append("~").append(getDotDate(transToROC_Year(Common.checkGet(rs.getString("suitdatee")),8))).append("</td> ");		
		sbHTML.append(" </tr> ");	
		count++;
	}
}
