...
String q_limitYear = Common.checkGet(request.getParameter("q_limitYear"));
...
String fromAdd = Common.checkGet(request.getParameter("fromAdd"));
...
if ("Y".equals(Common.get(isLookup))) {
	...
	"		opener.document.all.item(\"limitYear\").readOnly=true;\n"+
	...
	//"			opener.document.all.item(\"otherLimitYear\").value=limitYear;\n"+
	"		}\n"+
	"	}\n";	
}
...
String sql=" select a.propertyNo, a.propertyName, a.propertyType, a.propertyUnit, a.material, a.limitYear " +
			" from SYSPK_PropertyCode a where a.enterOrg in ('000000000A','" + user.getOrganID() + "')" ;
if ("Y".equals(fromAdd)) {
...
if (!"".equals(q_limitYear)) {
	sql+=" and a.limitYear = '" + Common.get(Common.esapi(q_limitYear)) +"' " ;
}
...
function selectProperty(propertyNo,propertyName,propertyType,propertyUnit,material,limitYear,propertyName1){
	if(limitYear.substring(0,1)==0){
		limitYear = limitYear.substring(1);
	}
	...
	// 使用年限
	if (isObj(opener.document.all.item("originalLimitYear"))) {
		if (!isNaN(limitYear)) {
			opener.document.all.item("originalLimitYear").value=limitYear;
		} else {
			opener.document.all.item("originalLimitYear").value = '';
		}
	} 
	
	// 使用年限(月)
	if (isObj(opener.document.all.item("otherLimitYear")) && '' !== limitYear) {
		if (!isNaN(limitYear)) {
			opener.document.all.item("otherLimitYear").value=limitYear * 12;
		} else {
			opener.document.all.item("otherLimitYear").value='';
		}
	}
	<%=strJavaScript%>

	// 開啟此視窗的作業不一定會有 changePropertyNo function
	try {
		//if (typeof(opener.changePropertyNo) === "function") {  IE 不支援
...
function checkField(){
	...
	columnTrim(form1.q_limitYear);
	if ((form1.q_propertyNo.value.length==0)&&(form1.q_propertyName.value.length==0)&&(form1.q_limitYear.value.length==0)){
		alert("「財產編號」或「財產名稱」或「使用年限」請至少輸入一個查詢條件!");
		return false;
	}
}
...
function openUpdateWindow(type) {
	...
	if ("ne" == type) { // 非消
		form1.action = "../unt/ne/syspk007f_s.jsp?fromAdd=" + "<%=fromAdd%>";
	} else {// 財產自用
...
<tr><td class="bg" >
	...
	<tr>		
		<td class="td_form">&nbsp;<font color="red">*</font>使用年限：</td>
		<td class="td_form_white">
			<input class="field" type="text" name="q_limitYear" size="2" maxlength="2" value="<%=Common.get(q_limitYear)%>">
		</td>		
	</tr>
	</table>
	</div>
