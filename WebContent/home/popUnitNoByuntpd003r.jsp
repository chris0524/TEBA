...
String popUnitNo2 = Common.checkGet(request.getParameter("popUnitNo2"));
String popUnitNo3 = Common.checkGet(request.getParameter("popUnitNo3"));
String popUnitNo4 = Common.checkGet(request.getParameter("popUnitNo4"));
...
String sql = " select unitNo, unitName from UNTMP_KeepUnit "+
		" where enterOrg like '" + Common.esapi(queryValue) + "%'";


if("".equals(Common.get(q_ID))){
}else{					sql += " and unitNo = '" + Common.get(Common.esapi(q_ID)) +"'";
}
if("".equals(Common.get(q_Name))){
}else{					sql += " and unitName like '%" + Common.get(Common.esapi(q_Name)) +"%'";
}
...
function selectProperty(popUnitNo, popUnitNoName,popUnitNo2,popUnitNo3,popUnitNo4){
	var popUnitNoElement = opener.document.all.item("<%=popUnitNo%>");
	if (isObj(popUnitNoElement)) {		
		popUnitNoElement.value = popUnitNo;	
		if (isObj(popUnitNoElement.onchange)) {
			try {
				popUnitNoElement.onchange();
			} catch (e) {
				// TODO 與 View.getSelectCtrlGroup  指定onchange的 可能造成迴圈
			}
		}
	}
	var popUnitNoElement2 = opener.document.all.item("<%=popUnitNo2%>");
	if (isObj(popUnitNoElement2)) {		
		popUnitNoElement2.value = popUnitNo;	
		if (isObj(popUnitNoElement2.onchange)) {
			try {
				popUnitNoElement2.onchange();
			} catch (e) {
			}
		}
	}
	if (isObj(opener.document.all.item("<%=popUnitNo3%>"))) {		
		opener.document.all.item("<%=popUnitNo3%>").value = popUnitNo;		
	}
	if (isObj(opener.document.all.item("<%=popUnitNo4%>"))) {		
		opener.document.all.item("<%=popUnitNo4%>").value = popUnitNo;		
	}
	opener.keeperChange();
	window.close();
}
...
<input type="hidden" name="popUnitNo2" value="<%=popUnitNo2%>">
<input type="hidden" name="popUnitNo3" value="<%=popUnitNo3%>">
<input type="hidden" name="popUnitNo4" value="<%=popUnitNo4%>">
