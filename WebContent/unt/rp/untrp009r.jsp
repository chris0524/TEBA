...
function checkField(){
	...
	var err2=checkAtLeastOne( [form1.originalKeepUnitQuickly, form1.originalKeeperQuickly, form1.originalUseUnitQuickly, form1.originalUserQuickly]
														,["保管單位","保管人","使用單位","使用人"] );
...
function init() {
	...
	if ('<%=user.getRoleid()%>' == '1'){		
		form1.btn_originalKeeper.disabled = true;		
		readbg="#EEEEEE";
		setFormItem("originalKeeperQuickly","R");
	}
...
<form id="form1" name="form1" method="post" action="untrp009p.jsp" onReset="form1.q_enterOrg.value='<%=user.getOrganID()%>';changeAll();" onSubmit="return checkField()" target="_blank">
...
<table class="bg" width="70%" cellspacing="0" cellpadding="0" align="center"><tr><td>
	...
	<tr>
        <td class="td_form" colspan="4" style="text-align:center">財產保管單位別清冊 <font color="red">(A4 橫式)</font></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="queryTDInput" colspan="3">
			<select class="field" type="select" name="q_propertyType">
				<%=util.View.getTextOption("1;土地;2;建物;3;土地改良;4;動產;5;權利;6;有價證券;","")%>
			</select>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="queryTDInput" >
				<%=View.getSelectCtrlGroup("select unitno, unitname from UNTMP_KEEPUNIT where enterorg = '" + user.getOrganID() + "' order by unitno ", 
			                                                       "field", "form1", "originalKeepUnit", "originalKeepUnitQuickly", user.getUnitID()) %>
				<input class="field" type="button" name="btn_originalKeepUnit" onclick="popUnitNo(form1.organID.value,'originalKeepUnit')" value="..." title="單位輔助視窗">
		</td>
		<td class="queryTDLable">保管人：</td>
		<td class="queryTDInput" >
				<%if("1".equals(user.getRoleid())){ %>
				<input class="field_RO" type="text" name="originalKeeperQuickly" value="<%=user.getUserName()%>" size="10">
				<input type="hidden" name="originalKeeper" value="<%=user.getKeeperno()%>">
				<input class="field_RO" type="button" name="btn_originalKeeper" onclick="popUnitMan(form1.organID.value,'originalKeeper')" value="..." title="人員輔助視窗">
				<%}else{ %>		
				<%=View.getSelectCtrlGroup("select a.keeperno, CASE a.incumbencyyn WHEN 'N' THEN a.keepername + '(停用)' ELSE a.keepername END AS keepername from UNTMP_KEEPER a LEFT OUTER JOIN SYSAP_EMP b ON a.enterorg = b.organid AND a.keeperno = b.empid where 1=1 and enterorg = '" + user.getOrganID() + "' order by case a.incumbencyyn when 'Y' then 'Y' else 'N' end DESC,convert(varchar,keepername) ", 
			                                                       "field", "form1", "originalKeeper", "originalKeeperQuickly", user.getKeeperno()) %>
		        <input class="field" type="button" name="btn_originalKeeper" onclick="popUnitMan(form1.organID.value,'originalKeeper')" value="..." title="人員輔助視窗">
		        <%}%>
		</td>
	</tr>	
	<tr>
		...
		<td class="queryTDInput"  >
				<%=View.getSelectCtrlGroup("select unitno, unitname from UNTMP_KEEPUNIT where enterorg = '" + user.getOrganID() + "' order by unitno ", 
			                                                       "field", "form1", "originalUseUnit", "originalUseUnitQuickly", user.getUnitID()) %>
				<input class="field" type="button" name="btn_originalUseUnit" onclick="popUnitNo(form1.organID.value,'originalUseUnit')" value="..." title="單位輔助視窗">
		</td>
		<td class="queryTDLable">使用人：</td>
		<td class="queryTDInput" >
				<%=View.getSelectCtrlGroup("select a.keeperno, CASE a.incumbencyyn WHEN 'N' THEN a.keepername + '(停用)' ELSE a.keepername END AS keepername from UNTMP_KEEPER a LEFT OUTER JOIN SYSAP_EMP b ON a.enterorg = b.organid AND a.keeperno = b.empid where 1=1 and enterorg = '" + user.getOrganID() + "' order by case a.incumbencyyn when 'Y' then 'Y' else 'N' end DESC,convert(varchar,keepername) ", 
			                                                       "field", "form1", "originalUser", "originalUserQuickly", user.getKeeperno()) %>
		        <input class="field" type="button" name="btn_originalUser" onclick="popUnitMan(form1.organID.value,'originalUser')" value="..." title="人員輔助視窗">
		 </td>
	</tr>
	<tr>
		...
		<td colspan="3" class="td_form_white">
			...
			<input class="field" type="button" name="btn_originalPlace1" onclick="popPlace(form1.organID.value,'originalPlace1','originalPlace1Name')" value="..." title="存置地點輔助視窗">
	...
	<tr>
		...
		<td colspan="3" class="td_form_white">
			<input class="field" type="text" name="engineeringNo" size="20" maxlength="20" value="<%=obj.getEngineeringNo()%>" onchange="queryEngineeringName('q_enterOrg','engineeringNo');">
			<input class="field" type="button" name="btn_engineeringNo" onclick="popEngineering(form1.organID.value,'engineeringNo','engineeringNoName');" value="..." title="工程編號輔助視窗">
			[<input class="field_PRO" type="text" name="engineeringNoName" size="30" maxlength="50" value="<%=obj.getEngineeringNoName()%>">]
       	</td>
    </tr>
    <tr>
