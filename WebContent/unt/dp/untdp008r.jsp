...
function checkField(){
	...
	if ("1" == $('input:radio:checked[name="q_kind"]').val()) { //1.重新產製  2.列印上次產製的資料
		alertStr += checkEmpty(form1.q_engineeringNo,"工程編號");
		alertStr += checkEmpty(form1.q_totalValue,"總金額");
		alertStr += checkNumber(form1.q_totalValue,"總金額");
		alertStr += checkEmpty(form1.q_expectedAccountsYM,"預計入帳年月");
		...
		alertStr += checkYYYMM(form1.q_expectedAccountsYM,"預計入帳年月");
...
<form id="form1" name="form1" action="untdp008p.jsp" method="post" onSubmit="return checkField()" target="_blank">
...
<tr><td class="bg">
	...
	<table class="table_form" width="100%" height="100%">
		...
		<tr id="tr3">
			<td class="td_form"><font color="red">*</font>總金額：</td>
			<td colspan="3" class="td_form_white">
				<input class="field_Q" type="text" name="q_totalValue" size="10" maxlength="11" value="<%=obj.getQ_totalValue() %>" onBlur="">
			</td>
		</tr>
		<tr id="tr4">
			<td class="queryTDLable"><font color="red">*</font>預計入帳年月：</td>
			<td class="queryTDInput">
				<input type="text" class="field_Q" name="q_expectedAccountsYM" size="5" maxlength="5"><font color="red">例如：103年 2月，則輸入10302</font>
			</td>
		</tr>
		<tr id="tr5">
			...
			<td class="queryTDInput">
				<select class="field_Q" type="select" name="q_differenceKind">
					<%=util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='DFK' and codeid='120' " ,obj.getQ_differenceKind())%>
				</select>
			</td>
		</tr>
		<tr id="tr6">
			<td class="queryTDLable"><font color="red">*</font>園區別：</td>
			<td class="queryTDInput">
				<select class="field_Q" type="select" name="q_deprPark">
					<%=util.View.getOption("select deprparkno, deprparkname from SYSCA_DEPRPARK where enterOrg='"+ user.getOrganID() +"'", "")%>
				</select>
				</td>
			<td class="queryTDLable"><font color="red">*</font>部門別：</td>
			<td class="queryTDInput">
				<select class="field_Q" type="select" name="q_deprUnit" onchange="deprUnitOnChange();">
					<%=util.View.getOption("select deprunitno, deprunitname from SYSCA_DEPRUNIT where enterOrg='"+ user.getOrganID() +"'","")%>
				</select>
			</td>
		</tr>
		<tr id="tr7">
			...
			<td class="queryTDInput">
				<select class="field_Q" type="select" name="q_deprUnit1">
					<%=util.View.getOption("select deprunit1no, deprunit1name from SYSCA_DEPRUNIT1 where enterOrg='"+ user.getOrganID() +"'","")%>
				</select>
			</td>
			<td class="queryTDLable"><font color="red">*</font>會計科目：</td>
			<td class="queryTDInput">
				<select class="field_Q" type="select" name="q_deprAccounts">
					<%=util.View.getOption("select a.depraccountsno, a.depraccountsname from SYSCA_DEPRACCOUNTS a,SYSCA_DEPRUNITACCOUNTS b where a.depraccountsno=b.depraccountsno and b.enterorg='"+ Common.esapi(user.getOrganID()) +"'","")%>
				</select>
			</td>
		</tr>
		<tr>
