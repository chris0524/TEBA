<!--
程式目的：單位財產系統 > > 折舊作業 > > 報表查詢及列印 > > 基金財產 > > 財產折舊攤提月報表
程式代號：untdp015
程式日期：
程式作者：Kang Da
--------------------------------------------------------
...
<jsp:useBean id="obj" scope="request" class="unt.dp.UNTDP015R">
...
<form id="form1"  name="form1" method="post" action="untdp015p.jsp" onReset="check_reset();" onSubmit="return checkField()" target="_blank">
<table class="bg" width="100%" cellspacing="0" cellpadding="0" align="center"><tr><td>
	...
	<tr>
        <td class="td_form" colspan="4" style="text-align:center">財產折舊攤提月報表(基金財產)<font color="red">(A4 橫式)</font></td>
	</tr>
	...
	<td class="queryTDInput">
		<select class="field" type="select" name="q_originalDeprPark" onchange="form1.deprPark.value = this.value;">
		<%=util.View.getOption("select deprparkno, deprparkname from SYSCA_DEPRPARK where 1=1 and enterorg = '" + Common.esapi(user.getOrganID()) + "' ", obj.getQ_originalDeprPark())%>
		</select>
	</td>
	</tr>
	<tr>
		...
		<td class='queryTDInput'>
			<select class='field_Q' name='q_groupType'>
				<%=View.getTextOption("A;部門別+會計科目;B;部門別+部門別單位+會計科目", "") %>
			</select>
		</td>
	</tr>
	<tr>
