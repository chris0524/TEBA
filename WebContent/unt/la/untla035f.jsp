...
<jsp:useBean id="obj" scope="request" class="unt.la.UNTLA035F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.la.UNTLA035F)obj.queryOne();	
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		alertStr += checkFloat(form1.area,"整筆面積(㎡)","9,2",2);
		...
		alertStr += checkFloat(form1.holdArea,"權利範圍面積(㎡)","9,2",2);
...
function init(){
	...
	}else if(parse(form1.reduceCaseNo1.value).length>0 && form1.state.value!="insertError" && form1.state.value!="updateError"){
...
function checkAddCaseNo(SPN){
	if(parse(form1.addCaseNo.value).length>0){
		getPropertyNo(SPN,'1','N');
	}else{
		getPropertyNo(SPN,'1','Y');
	}
}
...
<!-- 	<td ID=t2 CLASS="tab_border2"><a href="#" onClick="return checkURL('untla028f.jsp');">合併<br>減損單</a></td>		
 ...
 -->		
		...
		<td ID=t8 CLASS="tab_border1">分割<br>減損單明細</td>
...
<tr><td class="bg">
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
			...
			&nbsp;&nbsp;&nbsp;　電腦單號：[<input class="field_QRO" type="text" name="reduceCaseNo1" size="10" maxlength="10" value="<%=obj.getReduceCaseNo1()%>">]
		</td>
	</tr>
	<tr>
		<td class="td_form"><font color="red">*</font>財產編號：</td>
		<td class="td_form_white" colspan="3">
			<input class="field_P" type="text" name="propertyNo" size="10" maxlength="10" value="<%=obj.getPropertyNo()%>"  onChange="getProperty('propertyNo','propertyNoName','1');checkAddCaseNo('PN');getValue();checkValue();"> 
			...
			　分號：<input class="field_P" type="text" name="serialNo" size="7" maxlength="7" value="<%=obj.getSerialNo()%>" onChange="checkAddCaseNo('PN');getValue();checkValue();">
		<br>
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
			<select class="field_P" type="select" name="signNo1" onChange="changeSignNo1('signNo1','signNo2','signNo3','');checkAddCaseNo('SN');getValue();checkValue()">
			<%=util.View.getOption("select signNo, signName from SYSCA_Sign where signNo like '_000000' order by seqno",obj.getSignNo1())%>
			</select>
			<select class="field_P" type="select" name="signNo2" onChange="changeSignNo2('signNo1','signNo2','signNo3','');checkAddCaseNo('SN');getValue();checkValue()">
			...
			　地號：
			<input class="field_P" type="text" name="signNo4" size="4" maxlength="4" value="<%=obj.getSignNo4()%>" onChange="checkAddCaseNo('SN');getValue();checkValue()">
			&nbsp;-
			<input class="field_P" type="text" name="signNo5" size="4" maxlength="4" value="<%=obj.getSignNo5()%>" onChange="checkAddCaseNo('SN');getValue();checkValue()">		
		</td>
	</tr>		
	<tr>
