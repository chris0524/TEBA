...
<jsp:useBean id="obj" scope="request" class="unt.la.UNTLA004F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.la.UNTLA004F)obj.queryOne();	
...
insertDefault = new Array(
		new Array("useArea004","<%=obj.getHoldAreaFromUntla_Land()%>")
	);
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		alertStr += checkEmpty(form1.useArea004, "使用本筆土地面積");
		if(form1.useArea004.value!="")	form1.useArea004.value = roundp(form1.useArea004.value,2,"Y")
		alertStr += checkFloat(form1.useArea004,"使用本筆土地面積(m2)",7,2);
		alertStr += checkLen(form1.notes, "備註", 250);		

		alertStr += checkEmpty(form1.ownership1, "地上物權屬");		
		alertStr += checkEmpty(form1.purpose, "地上物用途");
		alertStr += checkEmpty(form1.state004, "地上物狀況");
...
<tr><td class="bg">
	...
	<tr>
		...
		<td class="td_form"><font color="red">*</font>地上物權屬：</td>
		<td class="td_form_white">
	...
	<tr>
	  ...
	  <td class="td_form_white"><input class="field" type="text" name="owner" size="15" maxlength="15" value="<%=obj.getOwner()%>">
      </td>
	</tr>
	<tr>
	  <td class="td_form"><font color="red">*</font>地上物用途：</td>
	  <td class="td_form_white"><select class="field" type="select" name="purpose">
          <%=util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='PUR' ", obj.getPurpose())%>
        </select>
      </td>
		<td class="td_form"><font color="red">*</font>地上物狀況：</td>
		<td class="td_form_white"><input class="field" type="text" name="state004" size="20" maxlength="30" value="<%=obj.getState004()%>"><font color="red">(門牌、建號或具體之使用狀況)</font>
        </td>
	</tr>
	<tr>
	  <td class="td_form"><font color="red">*</font>使用本筆土地面積(㎡)：</td>
	  <td class="td_form_white"><input class="field" type="text" name="useArea004" size="15" maxlength="15" value="<%=obj.getUseArea004()%>">
      </td>
		<td class="td_form">併用土地標示：</td>
		<td class="td_form_white"><input class="field" type="text" name="mergeUseSign" size="20" maxlength="30" value="<%=obj.getMergeUseSign()%>">
        </td>
	</tr>		
	<tr>
	  <td class="td_form">其他事項：</td>
	  <td colspan="3" class="td_form_white"><input class="field" type="text" name="notes1" size="50" maxlength="30" value="<%=obj.getNotes1()%>">
      </td>
		</tr>
	<tr>
	  <td class="td_form">備註：</td>
	  <td class="td_form_white"colspan="3"><textarea class="field" type="text" name="notes" cols="20" rows="4"><%=obj.getNotes()%></textarea></td>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',6,false);" href="#">地上物面積</a></th>
