...
<jsp:useBean id="obj" scope="request" class="unt.la.UNTLA065F">
...
if ("insertToUntla_Attachment".equals(obj.getState())) {
	obj.afterWork();
...
function init(){
	if(message=='afterWorkOver'){
		alert("新增完成");
		message='';
		opener.form1.action="untla064f.jsp";
...
function execSubmitNewInsert(){
	...
	if(serialNo1Array.length==0){
		...
		if(confirm("確定要新增資料？")){
			...
			form1.state.value="insertToUntla_Attachment";
...
$(window).unload(function(){
	if(form1.state.value=='insertToUntla_Attachment'){
...
<!--Form區============================================================-->
	<table width="100%" cellspacing="0" cellpadding="0">
		...
		<!--標籤區============================================================-->
		<tr>
			<td ID=t2 CLASS="tab_border1">土地地上物資料挑選</td>		
		</tr>
		<!--List區============================================================-->
		<tr><td class="bg">
...
-->
				...
				<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',3,false);" href="#">管理機關</a></th>
				<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',4,false);" href="#">地上物用途</a></th>
				<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',5,false);" href="#">使用本筆土地面積(㎡)</a></th>
			</thead>
			<tbody id="listTBODY">
			<%
				int counter=0;
				...
				if ("true".equals(obj.getQueryAllFlag()) && objList.size()==0){
					...
					while(it.hasNext()){
						...
						sbHEML.append(" <td class='listTD'><input type='checkbox' class='field_Q' name='sKeySet' value=\""+rowArray[0]+","+rowArray[6]+"\" " + isCheck + ">\n");
...
//						sbHEML.append(" <td class='listTD'><input class='field_RO' type='text' name='signNo5' size='4' maxlength='4' value='" + rowArray[5] + "' disabled='true' ></td>\n ");
						sbHEML.append(" <td class='listTD'><select class='field_RO' type='select' name='ownership1' disabled='true'>"
											+ util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='OWN' ORDER BY CODEID",rowArray[1]) +
											"</select></td>\n");
						
						sbHEML.append(" <td class='listTD'><input class='field' type='hidden' name='manageOrg' value='"+ rowArray[2] +"' disabled='true'><input class='field_RO' type='text' name='manageOrgName' size='20' maxlength='50' value='" + rowArray[3] + "' disabled='true'></td>\n ");
						
						sbHEML.append(" <td class='listTD'><select class='field_RO' type='select' name='purpose' disabled='true'>"
											+ util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='PUR' ", rowArray[4]) +
											"</select></td>\n");
						
						sbHEML.append(" <td class='listTD'><input class='field_RO' type='text' name='useArea' size='5' maxlength='5' value='" + rowArray[5] + "' disabled='true' ></td>\n ");
