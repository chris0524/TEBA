...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.la.UNTLA003F)obj.queryOne();	
...
insertDefault = new Array(
		new Array("useArea","<%=obj.getHoldAreaFromUntla_Land()%>"),
		new Array("useUnit","<%=(("04".equals(obj.getPropertyKind()))?"":user.getOrganID())%>"),
		new Array("useUnitName","<%=(("04".equals(obj.getPropertyKind()))?"":user.getOrganName())%>")
	);
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		if(form1.useRelation.value == '99'){
			alertStr += checkEmpty(form1.notes1,"其他事項");
		}
		if(form1.propertyKind.value == '04'){
			
		}else{
			if(form1.useUnit.value=="" && AllTrim(form1.useUnit1).length==0){
				alertStr += "使用單位與其他使用者請擇一輸入！";
		...
		if (!parseFloat(form1.useArea.value)>0) alertStr += "使用面積必須>0!\n";
		else alertStr += checkFloat(form1.useArea,"使用面積(㎡)",7,2);
		alertStr += checkLen(form1.notes, "備註", 250);
	}
	if(alertStr.length!=0){ alert(alertStr); return false; }
	
	if(form1.overTotalArea.value=='true'){
		alert("該筆土地使用面積總合＞該筆土地總面積");
		return false;
	}
...
function changeCollectfeeYN(){
		/*if(form1.collectfeeYN.value=="Y"){
			setFormItem("feeValue","O");
		}else{
			setFormItem("feeValue","R");	
...
function init() {
	...
	if (form1.enterOrg.value=="383030000D" && form1.propertyKind.value=="04") {	
		setFormItem("insert,update,delete,clear,confirm","R");	
	}
	
	if(form1.propertyKind.value == '04'){	$('#propertyKindSpan').hide();
	}else{									$('#propertyKindSpan').show();
	}
...
//	if(form1.useArea.value > form1.originalArea.value){
//	alert("使用面積＞土地登記面積");
//	}
//}
...
function getQueryCaseData(){
	...
	comment.propertyNo = form1.propertyNo.value;
	comment.serialNo = form1.serialNo.value;
	comment.useArea = form1.useArea.value;	

	comment.state = form1.state.value;
	
	//傳送JSON
	$.post('untla003f_ajax.jsp',
		comment,
		function(data){
			//將回傳資料寫入		
			data=eval('('+data+')');
		 	
			$("input[name='overTotalArea']").val(data['OVERTOTALAREA']);
	...
	<div id="formContainer">
	  <table class="table_form" width="100%" height="100%">
        <tr>
          ...
          <td class="td_form_white"><select class="field" type="select" name="useRelation" onChange="changeRelation();">
              <%=util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='URE' ORDER BY CODEID", obj.getUseRelation())%>
            </select>
          </td>
        </tr>
        <tr>
          <td class="td_form"><span id="propertyKindSpan"><font color="red">*</font></span>使用單位：</td>
          <td colspan="3" class="td_form_white"><%=util.View.getPopOrgan("field","useUnit",obj.getUseUnit(),obj.getUseUnitName(),"Y")%> 　其他使用者：
              <input class="field" type="text" name="useUnit1" size="20" maxlength="15" value="<%=obj.getUseUnit1()%>">
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="3" class="td_form_white"><input class="field" type="text" name="useArea" size="13" maxlength="13" value="<%=obj.getUseArea()%>">
          <input class="field" type="hidden" name="overTotalArea"  >
      　 其他事項：
        <input class="field" type="text" name="notes1" size="25" maxlength="50" value="<%=obj.getNotes1()%>">
          </td>
        </tr>
        <tr>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tbody id="listTBODY">
	<%
	boolean primaryArray[] = {true,true,true,true,true,false,false,false,false,false,false,false,false,false};
	boolean displayArray[] = {false,false,false,false,true,true,false,true,true,true,true,true,false,false};
