...
if(!"Y".equals(isLimit)){
	...
	if (!"Y".equals(user.getIsAdminManager())){
		if ("Y".equals(user.getIsOrganManager())){
			sql+=" and a.isManager='Y' and a.organID like '" + Common.esapi(user.getOrganID().substring(0,5)) +"%' ";
		}else{
			sql+=" and a.isManager='Y' and a.organID = '" + Common.esapi(user.getOrganID())  +"' ";
		}
	} else {
	//系統管理者
...
sql += " and IsManager='Y' ";
sql += " and( organID=decode('"+ user.getIsAdminManager() +"','Y',organID,'"+ user.getOrganID() +"') or "; 
sql += "      ( '"+ user.getIsOrganManager() +"'='Y' and organID in (select organID from SYSCA_Organ where organSuperior='"+ user.getOrganID() +"')) "; 
sql += " ) ";
}	
