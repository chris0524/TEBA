...
<jsp:useBean id="obj" scope="request" class="unt.pd.UNTPD006F">
...
<!-- untch.js不可移到function.js下方，因有相同的popProperty這裡不支援untch.js的popProperty -->
<script language="javascript" src="../ch/untch.js"></script> 
...
function checkField(){
	var alertStr="";
    var isChecked = false; 
    var sKeySet = document.getElementsByName('sKeySet');
	if (State != "querySubmit_Click") {
		for (var i=0; i<sKeySet.length; i++){
			if(sKeySet[i].checked){
				isChecked = true;
			}
		}
	...
	<table class="queryTable"  border="1">
		...
		<tr>
			...
			<td class="queryTDInput" colspan="3">
				...
				[<input class="field_RO" type="text" name="q_placeName1" size="20" maxlength="20" value="<%=obj.getQ_place1Name()%>">]
...
<tr><td class="bg">
	...
	<tr>
	    ...
	    <td class="td_form_white">
	    	...
	    	<select class="field" type="select" name="checkresult" onChange="oddsCauseChange(this);">
				<%=util.View.getTextOption("1;盤點正常;2;盤點異常", obj.getCheckresult())%>
			</select>
			<br>
			<% if("Y".equals(obj.getScrappednote())){%>
				<input type="checkbox" class="field" name="scrappednote"  value = "Y" onClick ="checkboxValue('scrappednote');" checked>	
			<% }else{%>
				<input type="checkbox" class="field" name="scrappednote" value = "N" onClick ="checkboxValue('scrappednote');">	
			<% }%>
			報廢註記	
			<br>
			<% if("Y".equals(obj.getLabelnote())){%>
				<input type="checkbox" class="field" name="labelnote" value = "Y" onClick ="checkboxValue('labelnote');"  checked>
			<% }else{%>
				<input type="checkbox" class="field" name="labelnote" value = "N"  onClick ="checkboxValue('labelnote');">
			<% }%>
			補印標籤註記
			<br>
			<% if("Y".equals(obj.getMovenote())){%>
				<input type="checkbox" class="field" name="movenote" value = "Y" onClick ="checkboxValue('movenote');" checked>
			<% }else{%>
				<input type="checkbox" class="field" name="movenote" value = "N" onClick ="checkboxValue('movenote');" >
			<% }%>
			移動註記
	    </td>  
		<td class="td_form"><font color="red">*</font>異常原因：</td>
	    <td class="td_form_white">
	    	<input class="field" type="text" name="oddscause" size="50" maxlength="16" value="<%= obj.getOddscause() %>">
	    </td>  
	</tr>	
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th class="listTH" nowrap><a class="text_link_w" onClick="return sortTable('listTBODY',1,false);" href="#">財產區分別</a></th>
		<th class="listTH" nowrap><a class="text_link_w" onclick="return sortTable('listTBODY',2,false);" href="#">財產編號</a></th>
		<th class="listTH" nowrap><a class="text_link_w" onclick="return sortTable('listTBODY',3,false);" href="#">財產分號</a></th>
		<th class="listTH" nowrap><a class="text_link_w" onclick="return sortTable('listTBODY',4,false);" href="#">財產別名</a></th>
