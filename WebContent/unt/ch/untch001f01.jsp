...
<jsp:useBean id="obj" scope="request" class="unt.ch.UNTCH001F01">
...
if ("init".equals(obj.getState())) {
	if ("Y".equals(isEngineering)) {
		obj.setEngineeringNo(p_engineeringNo);
		obj.setEngineeringNoName(p_engineeringNoName);
	} else {	
...
if(!"".equals(p_engineeringNo) && "init".equals(obj.getState())){
	obj.setQ_verify("N");
	obj.setEnterOrg(user.getOrganID());
	obj.setQ_enterOrg(user.getOrganID());
	obj.setQ_engineeringNo(p_engineeringNo);
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		alertStr += checkEmpty(form1.proofYear,"編號-年");
...
function init(){
	...
	setFormItem("printAddProof,printTransferProof,printPropertyNoCard,printLabel,printEngPropertyNo", "O");
	...
	form1.objPath.value = "功能選單 > > 單位財產系統 > > 財產管理 > > 財產增加單維護";
...
function loadUntrp012r(){
	var url = "../rp/untrp012r.jsp?"
		+ "organID=" + form1.organID.value
		+ "&q_enterOrg=" + form1.enterOrg.value 
		+ "&q_proofYear=" + form1.proofYear.value
		+ "&q_proofDoc=" + encodeURIComponent(form1.proofDoc.value)
...
<tr><td class="bg">
	...
	<tr>
		...
		<td colspan="3" class="td_form_white">		
			...
			<%=util.View._getSelectHTML("field_P", "differenceKind", obj.getDifferenceKind(),"DFK", null," and codeID<>'500' ") %>
...
<!--           <td colspan="3" class="td_form_white"> -->
<%-- 				<input class="field_RO" type="text" name="caseNo" size="15" maxlength="15" value="<%=obj.getCaseNo()%>">			 --%>
<!--           </td>	 -->
<!--    	    </tr>       -->
    	<tr>
        ...
        <tr>
			...
			<td colspan="3" class="td_form_white">
        		[<input class="field_RO" type="text" name="engineeringNo" size="10" maxlength="11" value="<%=obj.getEngineeringNo() %>" onBlur="">]
...
<tr><td class="bg" style="text-align:center">
	...
	<span id="btnUntrp012r">
		<input class="toolbar_default" type="button" followPK="true" name="printTransferProof" value="列印作價撥充（價購）" onClick="loadUntrp012r();">&nbsp;|
	</span>
	<span id="x1">
