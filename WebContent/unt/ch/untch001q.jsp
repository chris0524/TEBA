...
function checkYN(obj){
	...
	<tr id="q_title_tr">
		<td class="queryTDInput" colspan="4">
			<input name="querySelect" type="radio" class="field_Q" value="proof" <%=untbuQuery.getQueryTab1()%> onclick="initQ_Form(this.value);">
			&nbsp;<font color="green">查詢增加單資料</font>&nbsp;&nbsp;&nbsp;		
			<input name="querySelect" type="radio" class="field_Q" value="detail" <%=untbuQuery.getQueryTab2()%> onclick="initQ_Form(this.value);">
	...
	<tr name="div_q_proof">
	  ...
	  <td class="queryTDInput" >
	  	<%=util.View.getPopOrgan("field_Q","q_enterOrg",untbuQuery.getQ_enterOrg(),untbuQuery.getQ_enterOrgName(),"N")%>
	  </td>
	...
	<tr name="div_q_proof">
		...
		<td class="queryTDInput">
			<%=util.View._getSelectHTML("field_Q", "q_differenceKind", untbuQuery.getQ_differenceKind(),"DFK") %>			
		</td>
		 <td class="queryTDLable">入帳：</td>
		 <td class="queryTDInput">
		 	<select class="field_Q" type="select" name="q_verify" onChange="getEnterDate();">
	...
	<tr name="div_q_detail">
		...
		<td class="queryTDInput">
			...
			<input class="field_Q" type="button" name="btn_q_useUnit" onclick="popUnitNo(form1.q_enterOrg.value,'q_useUnit')" value="..." title="單位輔助視窗">
	   </td>
	...
	<tr name="div_q_detail">
		...
		<td class="queryTDInput">
			<%if("Y".equals(untbuQuery.getQ_originalBuildFeeCB())){ %>
				<input type="checkbox" class="field_Q" name="q_originalBuildFeeCB" onchange="checkYN(this);" checked >
			<% } else { %>
				<input type="checkbox" class="field_Q" name="q_originalBuildFeeCB" onchange="checkYN(this);">
			<% } %>
       	</td>
       	<td class="queryTDLable">部門別依比例分攤：</td>
		<td class="queryTDInput">
			<%if("Y".equals(untbuQuery.getQ_originalDeprUnitCB())){ %>
				<input type="checkbox" class="field_Q" name="q_originalDeprUnitCB" onchange="checkYN(this);" checked >
			<% } else { %>
				<input type="checkbox" class="field_Q" name="q_originalDeprUnitCB" onchange="checkYN(this);">
			<% } %>
       	</td>
	</tr>
	<tr name="div_q_detail">
	...
	<tr name="div_q_detail">
	  ...
	  <td class="queryTDLable">入帳：</td>
	   <td class="queryTDInput">
	    <select class="field_Q" type="select" name="q_verify2">
          <%=util.View.getYNOption(untbuQuery.getQ_verify2())%>
        </select>
      </td>	  
	</tr>
	<tr name="div_q_detail">
...
<!-- 		</td> -->
	...
	<tr name="div_q_proof">
		...
		<td class="queryTDInput"  colspan="3">
			<input class="field_Q" type="text" name="q_proofYear" size="3" value="<%=untbuQuery.getQ_proofYear()%>" onchange="getFrontZero(this.name,3);">
			年
			<input class="field_Q" type="text" name="q_proofDoc" size="10" maxlength="20" value="<%=untbuQuery.getQ_proofDoc()%>">
			字
			起<input class="field_Q" type="text" name="q_proofNoS" size="10" maxlength="20" value="<%=untbuQuery.getQ_proofNoS()%>" onchange="getFrontZero(this.name,7);">&nbsp;~&nbsp;
			訖<input class="field_Q" type="text" name="q_proofNoE" size="10" maxlength="20" value="<%=untbuQuery.getQ_proofNoE()%>" onchange="getFrontZero(this.name,7);">號
		</td>
	</tr>
	<tr name="div_q_detail">
	...
	<tr name="div_q_detail">
		...
		<td class="queryTDInput" colspan="3">
			起 <input class="field_Q" type="text" name="q_oldlotNoS" size="10" maxlength="30" value="<%=untbuQuery.getQ_oldlotNoS()%>">&nbsp;~&nbsp; 
			訖 <input class="field_Q" type="text" name="q_oldlotNoE" size="10" maxlength="30" value="<%=untbuQuery.getQ_oldlotNoE()%>">
		</td>
	</tr>
	<tr name="div_q_detail">
