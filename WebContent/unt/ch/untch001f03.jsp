...
String originalbv = obj.queryOriginalbv(); 
String deprshareamount_all = obj.querySumDeprShareAmount(); 

//提示user 繼續新增資料
if("insertSuccess".equals(obj.getState())){
    if(Integer.parseInt(deprshareamount_all) < Integer.parseInt(originalbv)) {
        obj.setErrorMsg("新增成功，目前分攤總金額仍小於總價，請繼續新增分攤資料");
    }
}

if("updateSuccess".equals(obj.getState())){
    if(Integer.parseInt(deprshareamount_all) < Integer.parseInt(originalbv)) {
        obj.setErrorMsg("修改成功，目前分攤總金額仍小於總價，請繼續新增分攤資料");
    }
}
...
if("111".equals(checkStr3)){
	...
	if("true".equals(obj.getIsAddProof())){	tabs = uch._createTabData(uch._MP_ADD, 6);
	}else if("query".equals(obj.getIsAddProof())){
											uch_QUERY._setupViewType(uch_QUERY._queryOrMaintenance);
	...
	if("true".equals(obj.getIsAddProof())){	tabs = uch._createTabData(uch._RT_ADD, 5);
	}else if("query".equals(obj.getIsAddProof())){
											uch_QUERY._setupViewType(uch_QUERY._queryOrMaintenance);
											tabs = uch_QUERY._createTabData(uch_QUERY._RT_ADD, 4);
	}else if("_query".equals(obj.getIsAddProof())){
											uch._setupViewType(uch._query);
...
}else if("1".equals(checkStr1) && !"111".equals(checkStr3)) {
	if("true".equals(obj.getIsAddProof())){	tabs = uch._createTabData(uch._LA_ADD, 10);
	}else if("query".equals(obj.getIsAddProof())){
											uch_QUERY._setupViewType(uch_QUERY._queryOrMaintenance);
											tabs = uch_QUERY._createTabData(uch_QUERY._LA_ADD, 9);
	}else if("_query".equals(obj.getIsAddProof())){
											uch._setupViewType(uch._query);
...
var originalbv = "<%=originalbv%>";
var deprshareamount_all = "<%=deprshareamount_all%>";
var deprshareamount_forUpdate = 0;
...
function checkField(){
	...
	if(alertStr.length!=0){ 
	    ...
	    if(form1.isHistory.value === 'N'){
		    var maxAcceptValue = originalbv - (deprshareamount_all - deprshareamount_forUpdate);
		    if(form1.deprShareamount.value > maxAcceptValue) {
		        alert("分攤總金額大於總價，請確認分攤金額。");
		        return false;
		    }
	    }
	    beforeSubmit();
...
function calDeprPercent(){
    var deprShareamount = form1.deprShareamount.value;
    var calPercent = deprShareamount / originalbv * 100;
    form1.deprPercent.value = calPercent.toFixed(2);
}
</script>
</head>
...
<tr><td class="bg">
	...
	<table class="table_form" width="100%" height="100%">
		...
		<tr>
			<td class="td_form"><font color="red">*</font>分攤金額：</td>
			<td class="td_form_white" colspan="3">
				<input type="text" class="field" name="deprShareamount" value="<%=obj.getDeprShareamount() %>" size="10" maxlength="10" onchange="calDeprPercent();">
				&nbsp;&nbsp;&nbsp;
				<font color="red">*</font>歷史資料：
				<select class="field" type="select" name="isHistory">
				　　<%=util.View.getYNOption(obj.getIsHistory())%>
				</select>
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">*</font>分攤百分比(%)：
				[<input type="text" class="field_RO" name="deprPercent" value="<%=obj.getDeprPercent() %>" size="10" maxlength="10">]
			</td>
		</tr>	
...
localButtonFireListener.whatButtonFireEvent = function(buttonName){
	switch (buttonName)	{
		case "insert":
			...
			break;
		case "update":
		    deprshareamount_forUpdate = form1.deprShareamount.value;
		    break;
	}
}
</script>
