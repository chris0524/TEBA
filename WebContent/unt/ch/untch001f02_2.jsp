...
<jsp:useBean id="obj" scope="request" class="unt.ch.UNTCH001F02_2">
...
obj.setIsAddProof(uch._maintenance);
	...
	if(form1.state.value=="queryAll"){
		...
		if(alertStr.length == 0){
			...
			if("111" == checkStr3){				
				...
				alertStr += checkEmpty(form1.areaLa,"土地總面積");
...
function init(){
	...
	if (form1.enterOrg.value!="<%=user.getOrganID()%>") {		
		setFormField(new Array("insert,update","delete"),"R");
	}	
	if (checkVerify == "Y") {
	...
	setDisplayItem("spanInsert,spanNextInsert,spanDelete","H");
...
function changePropertyNo(propertyNo){
	...
	if("111" == checkStr3){
		...
		$("#div_amount").show();
		$("#div_move").show();
		if(form1.keepUnit.value==""){
			form1.keepUnit.value="<%=user.getUnitID()%>";
		
		}
	    if(form1.useUnit.value==""){
	    	form1.useUnit.value=form1.keepUnit.value;
	    }
		if(form1.keeper.value==""){
			form1.keeper.value="<%=user.getKeeperno()%>";
		}
		if(form1.userNo.value==""){

            form1.userNo.value=form1.keeper.value;
		}
		$("#div_depr").show();
		$("#div_depr2").show();
		$("#div_pic").show();
		...
		if(form1.holdNumeLa.value == ""){
			form1.holdNumeLa.value = "1";
		}

		if(form1.holdDenoLa.value == ""){
			form1.holdDenoLa.value = "1";	
		}
...
function changeArea(check){
	var holdAreaLa;
	form1.holdNumeLa.value = Math.round(form1.holdNumeLa.value);
	form1.holdDenoLa.value = Math.round(form1.holdDenoLa.value);	
	holdAreaLa = roundp((form1.areaLa.value * form1.holdNumeLa.value / form1.holdDenoLa.value),'2',"Y") ;
	if(holdAreaLa==null)	holdAreaLa=0;
	if(holdAreaLa=="")		holdAreaLa=0;
	form1.holdAreaLa.value = holdAreaLa;	
...
function checkDeprMethod(value){
	...
	if(form1.originalDeprMethod.value == '01'){
		...
		if(form1.buyDate.value == ''){
			...
			// 問題單 1065  有些財產大類(土改, 建物 無法輸入單價 就要抓總價)
			if (form1.originalUnit.disabled) {
...
function calValue(name){
	...
	if(checkStr3 != '111' && checkStr1 == '1'){
		...
		if(name == 'originalUnit'){
			...
			form1.originalBV.value = roundp((form1.holdAreaLa.value * form1.originalUnit.value),'0',"Y").replace(".","");
			
		//輸入總價	總價 /權利面積  四捨五入至元
		}else if(name == 'originalBV'){
			form1.originalBV.value = roundp(form1.originalBV.value,'0',"Y").replace(".","");
			form1.originalUnit.value = roundp((form1.originalBV.value / form1.holdAreaLa.value),'2',"Y");
	...
	//動產
		...
		if(name == 'originalUnit'){
			...
			if(form1.originalUnit.value < 10000 && form1.propertyNo.value.substring(0,3) != '503'){//財產_財編503 為圖書類不需判斷單價小於一萬
		...
		//輸入總價
			...
			if(form1.originalBV.value < 10000 && form1.propertyNo.value.substring(0,3) != '503'){//財產_財編503 為圖書類不需判斷單價小於一萬
...
<div id="queryContainer" style="width:1100px;height:400px;display:none">
	...
	<jsp:include page="untch001q02_2.jsp" />
	</table>
</div>

<TABLE STYLE="width:100%;" CELLPADDING=0 CELLSPACING=0 valign="top">
	<tr><td ID=t1 CLASS="tab_border1" HEIGHT="25">財產明細</td>
</TABLE>
	...
	<table class="table_form" width="100%" height="100%">
		...
		<tr>
		  ...
		  <td colspan="3" class="td_form_white">
		  	...
		  	<font color="red">*</font>財產取得日期：[<%=util.View.getPopCalndar("field_QRO","sourceDate",obj.getSourceDate())%>] 　
			文號：[<input class="field_QRO" type="text" name="sourceDoc" size="25" maxlength="40" value="<%=obj.getSourceDoc()%>">]
		  </td>	
		</tr>
		...
		<tr>
			...
			<td colspan="3" class="td_form_white">
				<%=util.View._getSelectHTML("field_PRO", "propertyKind", obj.getPropertyKind(),"PKD") %>
				&nbsp;&nbsp;&nbsp;
				基金財產
		...
		<tr>
			...
			<td class="td_form_white" colspan="3">
				...
				<span id="div_limitYear">					
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
	        		使用年限(月)：
	        		[<input class="field_RO" type="text" name="otherLimitYear" size="3" maxlength="3" value="<%=obj.getOtherLimitYear()%>">]
	        	</span>
			</td>			
		</tr>
		...
		<tr id="div_spec">
			...
			<td class="td_form_white" colspan="3">
				...
				<input class="field_RO" type="button" name="btn_storeNo" onclick="popStore('storeNo','storeNoName')" value="..." title="廠商輔助視窗">
				<input class="field_RO" type="button" name="clear_storeNo" onclick="clearStoreNo();" value="清除">
				&nbsp;&nbsp;&nbsp;	
				品名：
				<input name="articleName" type="text" class="field_RO" value="<%=obj.getArticleName()%>" size="20" maxlength="10">
			</td>
		</tr>		
		...
		<tr id="div_laSignNo">
			...
			<td class="td_form_white">
				<select class="field_RO" type="select" name="laSignNo1" onChange="changeSignNo1('laSignNo1','laSignNo2','laSignNo3','');refreshTime();">
					<%=util.View.getOption("select signNo, signName from SYSCA_Sign where signNo like '_000000' order by seqno", obj.getLaSignNo1())%>
				</select>　
				<select class="field_RO" type="select" name="laSignNo2" onChange="changeSignNo2('laSignNo1','laSignNo2','laSignNo3','');refreshTime();">
		...
		<tr id="div_door">
			...
			<td class="queryTDInput" colspan="3">
				<select class="field_RO" type="select" name="doorPlate1" onChange="changeSignNo1('doorPlate1','doorPlate2','doorPlate3','');" onblur="setDoorplateAll()">
					<%=util.View.getOption("select signNo ,signName from SYSCA_Sign where signNo like '_000000' order by seqno",obj.getDoorPlate1())%>
				</select>
				<select class="field_RO" type="select" name="doorPlate2" onChange="changeSignNo2('doorPlate1','doorPlate2','doorPlate3','');" onblur="setDoorplateAll()">
					
				</select>
				<span style="display:none"><select class="field" type="select" name="doorPlate3"></select></span>
				
	        	[<input class="field_RO" type="text" name="doorPlatevillage1"   size="5" maxlength="5" value="<%=obj.getDoorPlatevillage1() %>" onblur="setDoorplateAll()"/>]
				...
				[<input class="field_RO" type="text" name="doorplateRd1"   size="15" maxlength="20" value="<%=obj.getDoorplateRd1() %>" onblur="setDoorplateAll()"/>]
				<select class="field_RO" type="select" name="doorplateRd2" onchange="" onblur="setDoorplateAll()">
	          		<%=util.View.getTextOption("1;路;2;街;3;大道",obj.getDoorplateRd2())%>
	        	</select>
				&nbsp;
				[<input class="field_RO" type="text" name="doorplateSec"   size="3"  maxlength="4"  value="<%=obj.getDoorplateSec() %>"   onblur="setDoorplateAll()"  />]段
			  	[<input class="field_RO" type="text" name="doorplateLn"    size="2"  maxlength="3"  value="<%=obj.getDoorplateLn() %>"    onblur="setDoorplateAll()"  />]巷
			  	[<input class="field_RO" type="text" name="doorplateAly"   size="2"  maxlength="3"  value="<%=obj.getDoorplateAly() %>"   onblur="setDoorplateAll()"  />]弄
			  	[<input class="field_RO" type="text" name="doorplateNo"   size="2"  maxlength="3"  value="<%=obj.getDoorplateNo() %>"   onblur="setDoorplateAll()"  />]號
			  	[<input class="field_RO" type="text" name="doorplateFloor1" size="2"  maxlength="3"  value="<%=obj.getDoorplateFloor1() %>" onblur="setDoorplateAll()" />]樓
			  	[<input class="field_RO" type="text" name="doorplateFloor2" size="15"  maxlength="25"  value="<%=obj.getDoorplateFloor2() %>" onblur="setDoorplateAll()" />]<br>
			  	總稱：
		...
		<tr id="div_laArea">
			...
			<td class="td_form_white" colspan="3">
				...
				總面積：
				[<input name="areaLa" type="text" class="field_RO" value="<%=obj.getAreaLa() %>" size="12" maxlength="10" onChange="changeArea('areaLa');calValue('');">]
				<br>
				權利分子：
				[<input class="field_RO" type="text" name="holdNumeLa" size="9" maxlength="10" value="<%=obj.getHoldNumeLa() %>" onChange="changeArea('holdNumeLa');calValue('');">]
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				權利分母：
				[<input class="field_RO" type="text" name="holdDenoLa" size="9" maxlength="10" value="<%=obj.getHoldDenoLa() %>" onChange="changeArea('holdDenoLa');calValue('');">]
				&nbsp;&nbsp;
				權利範圍面積(㎡)：
		...
		<tr id="div_buArea">
			...
			<td colspan="3" class="td_form_white">
				...
				[<input class="field_RO" type="text" name="originalCArea" size="9" maxlength="10" value="<%=obj.getOriginalCArea() %>" onChange="changeArea('originalCArea');calValue('');">]
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				附屬面積：
				[<input class="field_RO" type="text" name="originalSArea" size="9" maxlength="10" value="<%=obj.getOriginalSArea() %>" onChange="changeArea('originalSArea');calValue('');">]
	　			&nbsp;
				總面積：
				[<input class="field_RO" type="text" name="originalAreaBu" size="12" value="<%=obj.getOriginalAreaBu() %>">]<br>
				權利分子：
				[<input class="field_RO" type="text" name="originalHoldNumeBu" size="9" maxlength="10" value="<%=obj.getOriginalHoldNumeBu() %>" onChange="changeArea('originalHoldNumeBu');calValue('');">]
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				權利分母：
				[<input class="field_RO" type="text" name="originalHoldDenoBu" size="9" maxlength="10" value="<%=obj.getOriginalHoldDenoBu() %>" onChange="changeArea('originalHoldDenoBu');calValue('');">]
				&nbsp;&nbsp;
				權利範圍面積(㎡)：
			...
			<td class="td_form_white" colspan="3"> 
				<table  border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td>
							其它說明：
							[<input class="field_QRO" type="text" name="fundsSource1" size="12" maxlength="12" value="<%=obj.getFundsSource1()%>" readonly="true">]
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							會計科目：
							[<input class="field_QRO" type="text" name="accountingTitle" size="10" maxlength="20" value="<%=obj.getAccountingTitle()%>">]
						</td>
					</tr>
				</table>
			</td>
		...
		<tr id="div_rfArea">
			<td class="td_form">土地改良物：</td>
	  	    <td colspan="3" class="td_form_white">
	      		...
	      		[<input name="originalMeasure" type="text" class="field_PRO" onChange="changeArea();" value="<%=obj.getOriginalMeasure() %>" size="15" maxlength="15">]
	      		&nbsp;&nbsp;&nbsp;
		...
		<tr id="div_amount">
			...
			<td class="td_form_white" colspan="3">
				...
				[<input class="field_RO" type="text" name="originalAmount" size="10" maxlength="10" value="<%=obj.getOriginalAmount() %>" onchange="calValue(this.name);">]
				&nbsp;&nbsp;&nbsp;
				單價：
				[<input class="field_RO" type="text" name="originalUnit" size="10" maxlength="10" value="<%=obj.getOriginalUnit() %>" onchange="calValue(this.name);checkDeprMethod('');">]
				&nbsp;&nbsp;&nbsp;
				<font color="red">*</font>總價：
				[<input class="field_RO" type="text" name="originalBV" size="10" maxlength="10" value="<%=obj.getOriginalBV() %>" onchange="calValue(this.name);checkDeprMethod('');">]
				<br>
				會計科目：
				[<input type="text" class="field_RO" name="accountingTitle" value="<%=obj.getAccountingTitle() %>" size="20">]
				&nbsp;&nbsp;&nbsp;
				帳務摘要：
				[<input type="text" class="field_RO" name="originalNote" value="<%=obj.getOriginalNote() %>" size="20">]
			</td>
		</tr>	
		<tr id="div_move">
			...
			<td class="td_form_white" colspan="3">
				...
				<br/>
				使用註記：
				<input type="text" class="field" name="userNote" value="<%=obj.getUserNote() %>" size="20" maxlength="100">		        
		        <br>
		        <div style="display:none">
				...
				&nbsp;&nbsp;&nbsp;		
				存置地點說明：
				<input class="field" type="text" name="place" size="45" maxlength="45" value="<%=obj.getPlace() %>">
			</td>
		</tr>
		<tr id='div_accDetail'>
		...
		<tr id="div_depr">
			...
			<td class="td_form_white" colspan="3">
				...
				<%=util.View._getSelectHTML_withFunction("field_PRO", "originalDeprMethod", obj.getOriginalDeprMethod(),"DEP", "checkDeprMethod('');queryDeprUnitData();") %>
				&nbsp;&nbsp;
				<input class="field_PRO" type="checkbox" name="cb_originalBuildFeeCB" size="10" maxlength="10" onChange="defaultDU();"onclick="checkOriginalBuildFeeCB();selectOriginalBuildFeeCB();" <%=("Y".equals(obj.getOriginalBuildFeeCB())?"checked":"")%>>
				<input class="field_PRO" type="hidden" name="originalBuildFeeCB" size="10" maxlength="10" value="<%=obj.getOriginalBuildFeeCB()%>">
				屬公共設施建設費：
				&nbsp;&nbsp;
				<input class="field_PRO" type="checkbox" name="cb_originalDeprUnitCB" size="10" maxlength="10" onclick="checkOriginalDeprUnitCB();checkDeprMethod('');" <%=("Y".equals(obj.getOriginalDeprUnitCB())?"checked":"")%>>
				<input class="field_PRO" type="hidden" name="originalDeprUnitCB" size="10" maxlength="10" value="<%=obj.getOriginalDeprUnitCB()%>">
				部門別依比例分攤：
				<br>
				園區別：
				<select class="field_PRO" type="select" name="originalDeprPark" onchange="form1.deprPark.value = this.value;">
				...
				&nbsp;&nbsp;&nbsp;
				部門別：
				<select class="field_PRO" type="select" id="originalDeprUnit" name="originalDeprUnit" onchange="queryDeprUnitData();">
				...
				<br>
				殘值：
				[<input class="field_PRO" type="text" id="originalScrapValue" name="originalScrapValue" size="10" maxlength="10" value="<%=obj.getOriginalScrapValue()%>" onchange="checkDeprMethod('');">]
				&nbsp;&nbsp;
				應攤提折舊總額：
				...
				&nbsp;&nbsp;
				攤提壽月
				[<input class="field_PRO" type="text" name="originalApportionMonth" size="10" maxlength="3" value="<%=obj.getOriginalApportionMonth()%>" onChange="changeArea();checkDeprMethod(this.value);">]
				月提折舊金額
...
localButtonFireListener.whatButtonFireEvent = function(buttonName){
	switch (buttonName)	{
		case "update":
			var checkStr1 = form1.propertyNo.value.substring(0,1);
			if (checkStr1 == "2") {
				var propertyno = form1.propertyNo.value.substring(0, 5);
				if ((form1.buSignNo1.value == ""  && form1.buSignNo2.value == "" && form1.buSignNo3.value == "" && form1.buSignNo4.value == "" && form1.buSignNo5.value == "") 
						&& (propertyno >= 20101 && propertyno <= 20106)) {
					setFormItem("buSignNo1,buSignNo2,buSignNo3,buSignNo4,buSignNo5", "O");
				} else {
					setFormItem("buSignNo1,buSignNo2,buSignNo3,buSignNo4,buSignNo5", "R");
				}		
			}
