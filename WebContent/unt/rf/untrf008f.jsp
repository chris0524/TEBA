...
<!--
程式目的：土地改良物減少作業－減損單資料
程式代號：untrf008f
程式日期：0941005
程式作者：judy
--------------------------------------------------------
修改作者　　修改日期　　　修改目的
Jim Chou 0951229     add the button for untrf022r.jsp
--------------------------------------------------------
-->

<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8"%>
<%@ include file="../../home/head.jsp" %>
<jsp:useBean id="obj" scope="request" class="unt.rf.UNTRF008F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.rf.UNTRF008F)obj.queryOne();	
...
insertDefault = new Array(
	...
	new Array("proofDoc","<%=new unt.ba.UNTBA002F().getDefaultProofDoc(user.getOrganID(),"C2")%>"),
	new Array("closing","N")
);
<!-- 檢查區，譬如：日期、空白... -->
function checkField(){
...
function gountrf011r(){
	var url = "untrf011p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
		"&q_kind=4";
	window.open(url);
}

function gountrf022r(){
	var url = "untrf022p.jsp?q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
		"&q_kind=4";
	window.open(url);
}

function gountrf01213r(){
	if(form1.ownership.value==1){
		var url = "untrf012p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=1" +
			"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
			"&q_kind=4";
	}else if(form1.ownership.value==2){
		var url = "untrf013p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=2" +
			"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
			"&q_kind=4";
	}
	window.open(url);
...
function loadUntrf012r(){
	var url = "untrf012p.jsp?"
...
function loadUntrf011r(){
	var url = "untrf011p.jsp?"
			+ "organID=" + form1.organID.value
			+ "&q_enterOrg=" + form1.enterOrg.value 
			+ "&q_ownership=" + form1.ownership.value
			+ "&q_caseNoS=" + form1.caseNo.value
...
<TABLE STYLE="width:100%;" CELLPADDING=0 CELLSPACING=0 valign="top">
	<tr>
		...
		<td ID=t2 CLASS="tab_border2"><a href="#" onClick="return checkURL('untrf009f.jsp');">減損單明細資料</a></td>	
	</tr>
	<tr>
...
<input class="field_RO" type="text" name="proofNo" size="10" maxlength="10" value="<%=obj.getProofNo()%>">
	...
	<tr>
		<td class="td_form">報廢拆除日期：</td>
		<td colspan="3" class="td_form_white">
			<table width="100%" height="100%" border="0">
              <tr>
                <td width="29%"><%=util.View.getPopCalndar("field","demolishDate",obj.getDemolishDate())%></td>
                <td width="18%" align="right">原因： </td>
                <td><input class="field" type="text" name="unusableCause" size="50" maxlength="50" value="<%=obj.getUnusableCause()%>"></td>
              </tr>
            </table>
			</td>
	</tr>
...
<tr><td class="bg" style="text-align:center">
	...
	<span id="spanUntrf012r">
		<input class="toolbar_default" type="button" followPK="true" name="untrf012r" value="列印減損單" onClick="loadUntrf012r();">&nbsp;|
	</span>	
	<span id="spanUntrf011r">
		<input class="toolbar_default" type="button" followPK="true" name="untrf011r" value="列印移交清冊" onClick="loadUntrf011r();">&nbsp;|
	</span>
	<span id="spanID">
		<br>&nbsp;|
