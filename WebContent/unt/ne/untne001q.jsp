...
<table class="queryTable"  border="1">
	<tr>
	  ...
	  <input name="querySelect" type="radio" class="field_Q" value="addProof" <%=untneAddProofQuery.getQueryType1()%> checked="checked">
	  ...
	  <input name="querySelect" type="radio" class="field_Q" value="nonexpDetail" <%=untneAddProofQuery.getQueryType3()%>>
		...
		<td class="td_form_white" colspan="3">
			保管單位
				<%=View.getSelectCtrlGroup("select unitno, unitname from UNTMP_KEEPUNIT where enterorg = '" + untneAddProofQuery.getQ_enterOrg() + "' order by unitno ", 
			                                                       "field_Q", "form1", "q_keepUnit", "q_keepUnitQuickly",  untneAddProofQuery.getQ_keepUnit()) %>
				...
				<%=View.getSelectCtrlGroup("select a.keeperno, CASE a.incumbencyyn WHEN 'N' THEN a.keepername + '(停用)' ELSE a.keepername END AS keepername from UNTMP_KEEPER a LEFT OUTER JOIN SYSAP_EMP b ON a.enterorg = b.organid AND a.keeperno = b.empid where 1=1 and enterorg = '" + untneAddProofQuery.getQ_enterOrg() + "' order by case a.incumbencyyn when 'Y' then 'Y' else 'N' end DESC,convert(varchar,keepername) ", 
			                                                       "field_Q", "form1", "q_keeper", "q_keeperQuickly", untneAddProofQuery.getQ_keeper()) %>
		        <input class="field_Q" type="button" name="btn_q_keeper" onclick="popUnitMan(form1.organID.value,'q_keeper')" value="..." title="人員輔助視窗">
				<br>
				使用單位
				<%=View.getSelectCtrlGroup("select unitno, unitname from UNTMP_KEEPUNIT where enterorg = '" + untneAddProofQuery.getQ_enterOrg() + "' order by unitno ", 
			                                                       "field_Q", "form1", "q_useUnit", "q_useUnitQuickly",  untneAddProofQuery.getQ_useUnit()) %>
