...
<jsp:useBean id="obj" scope="request" class="unt.ne.UNTNE022F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.ne.UNTNE022F)obj.queryOne();	
...
<html>
<head>
<title>物品廢品處理現有資料明細新增輔助視窗</title>
...
function initC(){
	...
	document.all.item("nextInsert").style.display="none";
...
<!--Query區============================================================-->
<div id="queryContainer" style="width:1100px;height:200px;display:none">
	...
	<tr>
		...
		<td class="queryTDInput" colspan="3">
			...
			<select class="field_Q" type="select" name="q_userNo" onchange="form1.q_keeper.value = this.value">
				<%=util.View.getOption("select a.keeperno, CASE a.incumbencyyn WHEN 'N' THEN a.keepername + '(停用)' ELSE a.keepername END AS keepername from UNTMP_KEEPER a LEFT OUTER JOIN SYSAP_EMP b ON a.enterorg = b.organid AND a.keeperno = b.empid where 1=1 and enterorg = '" + obj.getEnterOrg() + "' order by case a.incumbencyyn when 'Y' then 'Y' else 'N' end DESC,convert(varchar,keepername) ",obj.getQ_userNo())%>
	        </select>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	if ("true".equals(obj.getQueryAllFlag()) && objList.size()==0){
		...
		while(it.hasNext()){
			...
			//sbHEML.append(" <td class='listTD' ><a href='#' " + tempJS + ">"+rowArray[0]+"</a></td>\n ");
