...
function selectProperty(propertyNo,propertyName,propertyType,propertyUnit,material,limitYear, memo, editID, editDate){
	if (isObj(opener.document.all.item("up_propertyNo"))) {
		opener.document.all.item("up_propertyNo").value=propertyNo;
	}
	if (isObj(opener.document.all.item("up_propertyNoName"))) {
		opener.document.all.item("up_propertyNoName").value=propertyName;
	}
	if (isObj(opener.document.all.item("up_propertyType"))) {
		opener.document.all.item("up_propertyType").value=propertyType;
	}
	if (isObj(opener.document.all.item("up_propertyUnit"))) {
		opener.document.all.item("up_propertyUnit").value=propertyUnit;
	}
	if (isObj(opener.document.all.item("up_material"))) {
		opener.document.all.item("up_material").value=material;
	}
	if (isObj(opener.document.all.item("up_limitYear"))) {
		opener.document.all.item("up_limitYear").value=limitYear;
	}
	if (isObj(opener.document.all.item("up_otherLimitYear"))) {
		if (parseInt(limitYear)>0) {
			opener.document.all.item("up_otherLimitYear").readOnly=true;
		}
	}
	if (parseInt(limitYear)<=0) {
		limitYear = "";
