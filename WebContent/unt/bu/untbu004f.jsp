...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.bu.UNTBU004F)obj.queryOne();	
...
if("true".equals(obj.getIsAddProof())){
	...
	tabs = uch_QUERY._createTabData(uch_QUERY._BU_ADD, 4);
}else if("_query".equals(obj.getIsAddProof())){
	uch._setupViewType(uch._query);
	tabs = uch._createTabData(uch._BU_ADD, 4);
...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		alertStr += checkEmpty(form1.floor,"樓層");
...
function openBatchInsertWindow() {
	var alertStr = "";	
	alertStr += checkEmpty(form1.enterOrg,"入帳機關");
	alertStr += checkEmpty(form1.ownership,"權屬");
	alertStr += checkEmpty(form1.propertyNo,"財產編號");
	alertStr += checkEmpty(form1.serialNo,"財產分號");
	popWin = window.open('untbu004f_batch.jsp?enterOrg='+form1.enterOrg.value+
		'&ownership='+form1.ownership.value+'&propertyNo='+form1.propertyNo.value+
		"&serialNo="+form1.serialNo.value+"&differenceKind="+form1.differenceKind.value,'MyWindow','scrollbars=1, resizable=yes, status=yes, toolbar=no,menubar=no,width=500,height=600');
}
...
<tr><td class="bg">
	...
	<tr>
		<td class="td_form">樓層次序：</td>
		<td class="td_form_white">
			[<input class="field_RO" type="text" name="serialNo1" size="5" maxlength="3" value="<%=obj.getSerialNo1()%>">]
        </td>
		<td class="td_form"><font color="red">*</font>樓層：</td>
		<td class="td_form_white">
			<select class="field" type="select" name="floor">
			<%=util.View.getOption("select codeID, codeName from SYSCA_Code where codeKindID='FLC' ", obj.getFloor())%>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td class="td_form_white">
			<input class="field" type="text" name="purpose" size="20" maxlength="50" value="<%=obj.getPurpose()%>">
		</td>
		<td class="td_form"><font color="red">*</font>面積(㎡)：</td>
		<td class="td_form_white">
			<input class="field" type="text" name="area" size="10" maxlength="10" value="<%=obj.getArea()%>">
		</td>
	</tr>
	<tr>
	  <td class="td_form">備註：</td>
	  <td class="td_form_white" colspan="3"><textarea name="notes" cols="30" rows="5" class="field"><%=obj.getNotes()%></textarea>
      </td>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',1,false);" href="#">樓層次序</a></th>
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',2,false);" href="#">樓層</a></th>
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',3,false);" href="#">用途</a></th>
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',4,false);" href="#">面積(㎡)</a></th>
		</thead>
	<tbody id="listTBODY">
	<%
