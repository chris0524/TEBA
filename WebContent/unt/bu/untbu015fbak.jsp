...
function checkField(){
	...
	if(form1.state.value=="queryAll"){
		...
		if(form1.approveOrg.value=="02" || form1.approveOrg.value=="03" || form1.approveOrg.value=="04"){
			alertStr += checkEmpty(form1.approveDate,"核准日期");
			alertStr += checkEmpty(form1.approveDoc,"核准文號");
		}		
	}
    var approveOrg = form1.approveOrg.value;
    var approveDate = form1.approveDate.value;
    var approveDoc = form1.approveDoc.value;
    if(approveOrg!= "01"){
       if((approveDate==""||approveDate<=0)&&(approveDoc==""||approveDoc<=0)){
           alert("「核准機關」為「本府、審計處、內政部、財政部、行政院」，「核准日期、核准文號」為必填欄位。");
       }
    }
...
[
			  <input class="field_PRO" type="text" name="enterOrgName" size="20" maxlength="50" value="<%=obj.getEnterOrgName()%>">
                ...
                <td width="13%" align="right"><font color="red">*</font>權屬：</td>
                <td width="13%"><select class="field_P" type="select" name="ownership">
                  <%=util.View.getOnwerOption(obj.getOwnership())%>
                </select></td>
                <td width="18%" align="right">填單日期：</td>
                <td width="100"><%=util.View.getPopCalndar("field","writeDate",obj.getWriteDate())%></td>
              </tr>
            </table>
			</td>
	</tr>
...
<input class="field_RO" type="text" name="proofNo" size="10" maxlength="10" value="<%=obj.getProofNo()%>">
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
			<table width="100%" height="100%" border="0">
              <tr>
                <td width="20%"><select class="field_RO" type="select" name="verify">
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
			<select class="field" type="select" name="approveOrg" onChange="changeApproveOrg();" >
                ...
                &nbsp;&nbsp;核准文號：<input class="field" type="text" name="approveDoc" size="20" maxlength="20" value="<%=obj.getApproveDoc()%>" obblur="changeApproveOrg()">
		</td>
	</tr>
	<tr>
