...
<jsp:useBean id="obj" scope="request" class="unt.bu.UNTBU027F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.bu.UNTBU027F)obj.queryOne();	
	...
	<div id="formContainer">
	  <table class="table_form" width="100%" height="100%">
        <tr>
          ...
          <td colspan="3" class="td_form_white"><input class="field_PRO" type="hidden" name="enterOrg" value="<%=obj.getEnterOrg()%>" >
            [<input class="field_PRO" type="text" name="enterOrgName" size="20" maxlength="50" value="<%=obj.getEnterOrgName()%>">]　 <font color="red">*</font>權屬：<select class="field_P" type="select" name="ownership" onChange="getUNTBUBuilding();">
  <%=util.View.getOnwerOption(obj.getOwnership())%>
</select>
　 資料狀態：[<input name="dataStateName" type="text" class="field_RO" value="<%=obj.getDataStateName()%>" size="10">]
<input name="dataState" type="hidden" value="<%=obj.getDataState()%>"></td>
          </tr>
        <tr>
          <td align="right" class="td_form"><font color="red">*</font>建物標示：</td>
          <td class="td_form_white" colspan="3"><select class="field_P" type="select" name="signNo1" onChange="changeSignNo1('signNo1','signNo2','signNo3','');getUNTBUBuilding('SN','1','Y','N','Y');">
              <%=util.View.getOption("select signNo, signName from SYSCA_Sign where signNo like '_000000' order by seqno",obj.getSignNo1())%>
            </select>　
              <select class="field_P" type="select" name="signNo2" onChange="changeSignNo2('signNo1','signNo2','signNo3','');getUNTBUBuilding('SN','1','Y','N','Y');">
                <script>changeSignNo1('signNo1','signNo2','signNo3','<%=obj.getSignNo2()%>');</script>
              </select>　
              <select class="field_P" type="select" name="signNo3" onChange="getUNTBUBuilding('SN','1','Y','N','Y');">
                <script>changeSignNo2('signNo1','signNo2','signNo3','<%=obj.getSignNo3()%>');</script>
              </select>              
               　建號：&nbsp;
               <input class="field_P" type="text" name="signNo4" size="5" maxlength="5" value="<%=obj.getSignNo4()%>" onBlur="getUNTBUBuilding('SN','1','Y','N','Y');">
&nbsp;-
    <input class="field_P" type="text" name="signNo5" size="3" maxlength="3" value="<%=obj.getSignNo5()%>" onBlur="getUNTBUBuilding('SN','1','Y','N','Y');">
          </td>
        </tr>
        <tr>
          <td align="right" class="td_form">財產編號：</td>
          <td colspan="3" class="td_form_white">[<input name="propertyNo" type="text" class="field_RO" value="<%=obj.getPropertyNo()%>" size="12">] 
          　分號：[<input class="field_PRO" type="text" name="serialNo" size="7" maxlength="7" value="<%=obj.getSerialNo()%>">]　 財產名稱：[<input class="field_RO" type="text" name="propertyName" value="<%=obj.getPropertyName()%>">]<br>
    ...
    　其他說明：
    <input class="field" type="text" name="drawCause1" size="40" maxlength="40" value="<%=obj.getDrawCause1()%>">
          </td>
        </tr>
        <tr>
