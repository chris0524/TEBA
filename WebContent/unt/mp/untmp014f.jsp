...
<jsp:useBean id="obj" scope="request" class="unt.mp.UNTMP014F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.mp.UNTMP014F)obj.queryOne();	
...
insertDefault = new Array(
	...
	new Array("proofDoc","<%=new unt.ba.UNTBA002F().getDefaultProofDoc(user.getOrganID(),"D2")%>"),
	new Array("closing","N")
);
<!-- 檢查區，譬如：日期、空白... -->
function checkField(){
...
function gountmp017r(){
	var url = "untmp017p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
		"&q_kind=4";
	window.open(url);
}
 
function gountmp018r(){
	var url = "untmp018p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value ;
	window.open(url);
}

function gountmp019r(){
	var url = "untmp019p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		"&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
		"&q_kind=4";
	window.open(url);
}

function gountmp020r(){
	var url = "untmp020p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value
			+ "&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value
			+ "&q_kind=4";
			
	for(i=0; i<form1.q_detail.length; i++){ 
		if(form1.q_detail[i].checked){ 
			url +="&q_detail="+form1.q_detail[i].value;
		} 
	}
	window.open(url);
}

function gountmp038r(){
	var url = "untmp038p.jsp?organID="+form1.organID.value+"&q_enterOrg=" + form1.enterOrg.value + "&q_ownership=" + form1.ownership.value +
		      "&q_caseNoS=" + form1.caseNo.value + "&q_caseNoE="+form1.caseNo.value +
			  "";
	window.open(url);
}
...
function loadUntmp020r(){
	var url = "untmp020p.jsp?"
...
<TABLE STYLE="width:100%;" CELLPADDING=0 CELLSPACING=0 valign="top">
	<tr>
		...
		<td ID=t2 CLASS="tab_border2"><a href="#" onClick="return checkURL('untmp015f.jsp');">減損單明細資料</a></td>	
	</tr>
	<tr>
...
<div id="print020r" style="position:absolute;z-index: 25;left:0;top :0;width:300px;height:80px;display:none">
	<iframe id="printFrom"  style="position:absolute;z-index: -1;left:0;top :0;width:100%;height:100%;"></iframe>	
	<div class="queryTitle">核章明細表視窗</div>
	<table class="queryTable"  border="1">	
	<tr> 
		<td class="queryTDLable" width="45%">核章明細表列印：</td>
		<td class="queryTDInput">
			<input class="field_Q" type="radio" name="q_detail" size="7"  value="Y">是
			<input class="field_Q" type="radio" name="q_detail" size="7"  value="N" CHECKED>否
		</td>
	</tr>
	<tr>
		<td class="queryTDInput" colspan="2" style="text-align:center;">
			<input class="toolbar_default" followPK="false" type="submit" name="querySubmit" value="確    定" onClick="gountmp020r();">
			<input class="toolbar_default" followPK="false" type="button" name="queryCannel" value="取　　消" onClick="queryHidden('print020r');init();">
		</td>
	</tr>
	</table>
</div>
...
<tr><td class="bg" style="text-align:center">
	...
	<span id="spanUntmp020r">
		<input class="toolbar_default" type="button" followPK="true" name="untmp020r" value="列印減損單" onClick="loadUntmp020r();">&nbsp;|
	</span>
