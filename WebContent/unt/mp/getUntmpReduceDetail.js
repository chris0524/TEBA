//*********************************************
//函數功能：找出UNTMP_ReduceDetail 資料表
//參　　數：無
//*********************************************
function getUntmpReduceDetail(){
	var alertStr = "";
	var i = 0, j=0;
	var sFlag = false;
	alertStr += checkEmpty(form1.enterOrg,"入帳機關");
	alertStr += checkEmpty(form1.ownership,"權屬");
	alertStr += checkEmpty(form1.caseNo,"減損單號");
	...
	if(alertStr.length!=0){
		...
		var queryValue = "";	
		queryValue = "enterOrg="+sEnterOrg+"&ownership="+sOwnership+"&caseNo="+sCaseNo+"&propertyNo="+sPropertyNo+"&serialNo="+sSerialNo;

		var xmlDoc=document.createElement("xml");		
		xmlDoc.async=false;			
		if(xmlDoc.load("xmlUntmpReduceDetail.jsp?"+queryValue)){
			for(i=0; i<xmlDoc.documentElement.childNodes.length; i++){
				...
				if (isObj(form1.accumDepr1)) {
						form1.accumDepr1.value=xmlDoc.documentElement.childNodes.item(i).getAttribute("accumDepr1");
				}
				if (isObj(form1.scrapValue1)) {
						form1.scrapValue1.value=xmlDoc.documentElement.childNodes.item(i).getAttribute("scrapValue1");
				}
