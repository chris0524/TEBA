...
<jsp:useBean id="obj" scope="request" class="unt.mp.UNTMP027F">
...
if ("queryAll".equals(obj.getState())) {
	...
	obj = (unt.mp.UNTMP027F)obj.queryOne();	
...
<html>
<head>
<title>動產增減值作業現有資料明細新增輔助視窗</title>
...
<!--批次設定區============================================================-->
<div id="gountmp027f" style="position:absolute;z-index: 25;left:0;top :0;width:300px;height:100px;display:none">
	...
	<tr> 
		...
		<td class="queryTDInput">
			<input class="field_Q" type="text" name="q_cause" size="12" maxlength="12" value="<%=obj.getQ_cause()%>">
		</td>
	</tr>
	<tr>
	...
	<tr>
		<td class="queryTDInput" colspan="4" style="text-align:center;">
			...
			<input class="toolbar_default" followPK="false" type="button" name="queryCannel" value="取　　消" onClick="queryHidden('gountmp027f');initB();">
		</td>
	</tr>
	</table>
</div>
...
<!--Query區============================================================-->
<div id="queryContainer" style="width:750px;height:250px;display:none">
...
<tr><td class="bg"> 
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
	  		[<input class="field_QRO" type="text" name="propertyNo" size="10" maxlength="11" value="<%=obj.getPropertyNo()%>"  onBlur="getUNTMPMovable();">]
	   		&nbsp;分號：[<input class="field_RO" type="text" name="serialNo" size="7" maxlength="7" value="<%=obj.getSerialNo()%>" onBlur="getUNTMPMovable();">]
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">		
			數量：[<input class="field_RO" type="text" name="bookAmount" size="15" maxlength="7" value="<%=obj.getBookAmount()%>">]
			&nbsp;&nbsp;　取得日期：[<input type="text" name="sourceDate" class="field_RO"  size="7" maxlength="7" value="<%=obj.getSourceDate()%>">]
		<br>
			金額總價：[<input class="field_RO" type="text" name="bookValue" size="15" maxlength="15" value="<%=obj.getBookValue()%>">]
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="td_form_white" colspan="3">
			保管單位：[<input class="field_RO" type="text" name="keetUnitName" size="30" value="<%=obj.getKeepUnitName()%>">]
					<input class="field_RO" type="hidden" name="keepUnit" value="<%=obj.getKeepUnit()%>">
			&nbsp;　保管人：[<input class="field_RO" type="text" name="keeperName" size="10" value="<%=obj.getKeeperName()%>">]
							<input class="field_RO" type="hidden" name="keeper" value="<%=obj.getKeeper()%>">
		<br>
			使用單位：[<input class="field_RO" type="text" name="useUnitName" size="30" value="<%=obj.getUseUnitName()%>">]
					<input class="field_RO" type="hidden" name="useUnit" value="<%=obj.getUseUnit()%>">
			&nbsp;　使用人：[<input class="field_RO" type="text" name="userName" size="10" value="<%=obj.getUserName()%>">]
							<input class="field_RO" type="hidden" name="userNo" value="<%=obj.getUserNo()%>">
		<br>
			存置地點：[<input class="field_RO" type="text" name="place" size="25" value="<%=obj.getPlace()%>">]
		</td>
	</tr>
	</table>
	</div>
...
<tr><td class="bg" style="text-align:center">
	...
	<input class="toolbar_default" type="button" followPK="false" name="batchInsertBut" value="批次設定" onClick="queryShow('gountmp027f');">&nbsp;|
</td></tr>
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',4,false);" href="#">帳面數量</a></th>		
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',5,false);" href="#">帳面價值</a></th>
	</thead>
	<tbody id="listTBODY">	
	<%
	...
	if ("true".equals(obj.getQueryAllFlag()) && objList.size()==0){
		...
		while(it.hasNext()){
			...
			sbHEML.append(" <td class='listTD'>"+rowArray[6]+"</td></tr>\n ");
		}
	}
