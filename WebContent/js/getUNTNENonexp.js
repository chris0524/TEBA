//*********************************************
//函數功能：找出UNTNE_Nonexp 和 UNTNE_NonexpDetail 資料表
//參　　數：無
//*********************************************
function getUNTNENonexp(){
	var alertStr = "";
	var i = 0, j=0;
	var sFlag = false;
	...
	**/
		...
		queryValue = "enterOrg="+senterOrg+"&ownership="+sownership+"&propertyNo="+sPropertyNo+"&propertyNoName="+encodeURIComponent(sPropertyName)+"&serialNo="+sSerialNo+"&differenceKind="+sDifferenceKind;
...
//		var xmlDoc=document.createElement("xml");		
		xmlDoc.async=false;
		if(xmlDoc.load("../../home/xmlUNTNENonexp.jsp?"+queryValue)){
			for(i=0; i<xmlDoc.documentElement.childNodes.length; i++){
				...
				if (isObj(form1.oldPlaceName)) {
					form1.oldPlaceName.value=xmlDoc.documentElement.childNodes.item(i).getAttribute("oldPlaceName");
			    }	
			}
			if (!sFlag) {
				if (isObj(form1.propertyNo)) { form1.propertyNo.value=""; }		
				if (isObj(form1.propertyNoName)) { form1.propertyName1.value=""; }
				if (isObj(form1.serialNo)) { form1.serialNo.value=""; }			
				return false;	
			}
		}else{
